...
El instalador de Smart Node de Rocket Pool puede transformar su máquina en un nodo Ethereum completo, ya que requiere tanto clientes de Ejecución como de Consenso para funcionar correctamente.
...
Para usuarios que desean ponerse en marcha rápidamente, el instalador de Smart Node proporciona una opción de `Cliente Aleatorio` que puede ser la mejor opción.
...
Rocket Pool admite cinco clientes de Ejecución: **Geth**, **Besu**, **Nethermind**, **Reth** y **Erigon**.
...
Su requisito mínimo de RAM es de 8 GB, pero se recomiendan 16 GB.
...
[Erigon](https://erigon.tech/) es un cliente de Ejecución escrito en [Go](https://go.dev/). Utiliza una arquitectura de staged-sync y puede descargar instantáneas por BitTorrent en el primer arranque, lo que hace que la sincronización inicial sea comparativamente rápida si el puerto torrent es accesible.

En el Administrador de Configuración, Erigon expone un **Torrent Port** (por defecto `42069`, TCP y UDP) además del puerto P2P habitual (`30303`). Redirige ambos si quieres que la sincronización por instantáneas y el emparejamiento funcionen bien desde fuera de tu LAN.

Erigon admite **Rolling History Expiry** (descartar cuerpos y recibos de más de un año de antigüedad aproximadamente). **No** admite la History Expiry limitada a pre-merge; si eliges ese modo, conserva el historial de bloques restante con la poda de estado del modo completo. Si lo ejecutas como **nodo completo**, conserva los recibos.

Como con los demás clientes, las necesidades de RAM son flexibles y la calidad del disco importa. Abre los puertos torrent y P2P en tu router (y en el firewall del host) igual que harías para el `30303` de Geth.
...
_Datos de uso de RAM obtenidos de [eth-docker](https://ethdocker.com/Usage/ResourceUsage/). El uso real puede variar según la configuración y las condiciones de la red._
...
Esto le permite funcionar bien en sistemas embebidos y dispositivos con recursos restringidos. Si planeas ejecutar un par de clientes de Ejecución y Consenso con 16GB de RAM en total en tu nodo, se recomienda nimbus.
...
| Cliente    | Uso de RAM | Tiempo de Sincronización                  |
| ---------- | ---------- | ----------------------------------------- |
| Lighthouse | ~14 GB     | Instantáneo con sincronización checkpoint |
| Lodestar   | ~12 GB     | Instantáneo con sincronización checkpoint |
| Nimbus     | ~5 GB      | Instantáneo con sincronización checkpoint |
| Prysm      | ~9 GB      | Instantáneo con sincronización checkpoint |
| Teku       | ~10 GB     | Instantáneo con sincronización checkpoint |
