...
En este punto, deberías tener la infraestructura completa de Rocket Pool funcionando, incluyendo el stack del Smart Node, un cliente de Ejecución y un cliente de Consenso.
...
<div className="p-3">
  <Tabs>
    <Tab label="Modo Docker (Predeterminado)">
      Ahora que tienes el Smart Node instalado, iniciar el stack es fácil.
      ...
      Si es así, entonces el stack del Smart Node ha sido inicializado exitosamente y ahora está funcionando.
    ...
    <Tab label="Modo Nativo">
      En modo nativo, ya configuraste e iniciaste los servicios del Smart Node como parte de la sección [Configurando un Nodo Rocket Pool Nativo sin Docker](./native).
...
La primera línea te dirá si tu Smart Node está configurado para la mainnet de Ethereum o para la testnet Hoodi.
...
Ahora que los servicios del Smart Node se han iniciado, es una buena idea verificar que estén funcionando correctamente sin errores.
  ...
  <Tabs>
    <Tab label="Modo Docker (Predeterminado)">
      ...
      <p className="rspress-directive warning">
        ...
        ¡Esto está bien!
        El contenedor del validador fallará hasta que se haya creado un nuevo validador usando el comando `rocketpool node deposit`, que cubriremos en la sección [Creación de un Megapool (Validador)](/node-staking/megapools/create-megapool-validator).
    ...
    <Tab label="Modo Nativo">
      La forma más fácil de verificar el estado y los registros en modo nativo es usar los scripts de registro que creaste cuando configuraste por primera vez los servicios del Smart Node.
