...
# Configuración del Stack de Smart Node (Docker / Modo Híbrido)
...
Afortunadamente, Smart Node está diseñado para ocultar toda esa complejidad, por lo que es rápido y fácil de configurar, mientras te da la libertad de personalizar todo si así lo deseas.

En esta sección, repasaremos los distintos métodos para configurar Smart Node si estás usando la **configuración basada en Docker** o una **configuración híbrida** donde te conectas a clientes de Ejecución o Consenso administrados externamente (por ejemplo, clientes que administras fuera de Smart Node para staking en solitario).
...
- A través de la [Interfaz del Administrador de Configuración](#configuración-a-través-del-administrador-de-configuración) - esto te da acceso a todas las configuraciones de Smart Node para que puedas personalizar todo tanto como quieras.
- Sin interfaz a través de la [Línea de Comandos](#configuración-a-través-de-la-línea-de-comandos) - esta es una opción para personas que ejecutan Smart Node en un entorno sin interfaz (no interactivo) y necesitan configurarlo automáticamente.
...
::: tip NOTA
Si ya has configurado Smart Node, en su lugar serás recibido con el [Administrador de Configuración](#configuración-a-través-del-administrador-de-configuración).
...
::: tip NOTA
Las redes oficiales (Mainnet, Hoodi Testnet y Devnet) provienen de `networks-default.yml`. Puedes añadir o sobrescribir redes en `~/.rocketpool/networks-extra.yml` sin recompilar Smart Node. Consulta [Redes personalizadas y de sobrescritura](./advanced-config#redes-personalizadas-y-de-sobrescritura) para más detalles.
:::
...
Úsalo si aún no tienes un par de clientes y quieres que Smart Node administre uno por ti.
Al elegir esto, Smart Node creará, configurará y administrará un par de clientes de Ejecución y Consenso como contenedores de Docker.
...
Al elegir esto, Smart Node simplemente se conectará a tus clientes existentes y no ejecutará uno propio.
...
<div className="p-3">
  <Tabs>
    <Tab label="Administrado Localmente">
      Si quieres que Smart Node administre un cliente de Ejecución por ti, la siguiente pantalla te pedirá que elijas un cliente:

      <img src={ECSelection} width="100%" height="auto"/>

      Consulta la sección [Elegir tus clientes ETH](/es/node-staking/eth-clients#clientes-de-ejecución) para una descripción de cada opción.
      Una vez que hayas hecho tu elección, haz clic en la pestaña apropiada a continuación para aprender cómo configurarlo:

      Si eliges **Geth**, **Besu**, **Nethermind**, **Reth** o **Erigon**, el Asistente manejará toda la configuración por ti.
      ...
      Si eliges **Erigon**, redirige también su **Torrent Port** (por defecto **42069**, TCP y UDP) además del puerto P2P **30303**, para que la sincronización por instantáneas y el emparejamiento funcionen. El Asistente sigue usando valores predeterminados sensatos para todo lo demás.
      ...
      <p className="rspress-directive warning">
        <p className="rspress-directive-title">NOTA</p>
        Debido a que Smart Node se ejecutará en su propio contenedor de Docker, usará la red interna de Docker.
        No podrás usar nombres de host como `localhost` o `127.0.0.1` aquí; si tu cliente de Ejecución está ejecutándose en la misma máquina que Smart Node, necesitarás proporcionar la dirección IP LAN de la máquina en su lugar.
      </p>
    ...
    <Tab label="Externo">
      ...
      Smart Node usará la URL HTTP para comunicarse con él y realizar actividades de blockchain, como consultar el estado de la cadena y enviar transacciones.
      **Si aún no tienes habilitado el puerto API de tu cliente y accesible desde tu máquina Smart Node, necesitarás configurarlo ahora.**
      ...
      La URL de Websocket solo es relevante si la necesitas para tu propio uso; Smart Node no la usa.
      Puedes dejarla en blanco si no la necesitas.

      <p className="rspress-directive warning">
        <p className="rspress-directive-title">NOTA</p>
        Debido a que Smart Node se ejecutará en su propio contenedor de Docker, usará la red interna de Docker.
        No podrás usar nombres de host como `localhost` o `127.0.0.1` aquí; si tu cliente de Ejecución de respaldo está ejecutándose en la misma máquina que Smart Node, necesitarás proporcionar la dirección IP LAN de la máquina en su lugar.
      </p>
...
<div className="p-3">
  <Tabs>
    <Tab label="Administrado Localmente">
      Si quieres que Smart Node administre un cliente de Consenso por ti, la siguiente pantalla te pedirá que elijas un cliente:
      ...
      **La opción preferida** para la salud general y diversidad de la red es la opción **Random (Recommended)**, que elegirá aleatoriamente entre uno de los cuatro clientes de Consenso compatibles por ti.
      ...
      Si prefieres elegir un cliente explícito, consulta la sección [Elegir tus clientes ETH](./eth-clients#clientes-de-consenso) para una descripción de cada opción para que puedas tomar una decisión informada.
        ...
        <Tabs>
          ...
          <Tab label="Nimbus">
            ...
            **Ten en cuenta que la longitud máxima del graffiti es de 16 caracteres.**
            ...
            Nimbus admite gossip P2P sobre el [protocolo QUIC](https://en.wikipedia.org/wiki/QUIC). En el Gestor de Configuración, establece **P2P QUIC Port** en los ajustes de Nimbus (por defecto **8001**, UDP), reenvía ese puerto a tu nodo en la configuración de port forwarding de tu router y ábrelo en el firewall del host si usas uno (`ufw status`). Evita colisiones con otros nodos en la misma máquina (o con nodos de respaldo / testnet).
    ...
    <Tab label="Administrado Externamente">
      Debido a que cada cliente de Consenso tiene un comportamiento ligeramente diferente, Smart Node necesita saber cuál estás usando externamente para que pueda adaptar su propio comportamiento en consecuencia.
      ...
      <div className="p-3">
        <Tabs>
          <Tab label="Lighthouse">
            ...
            Smart Node usará la URL HTTP para comunicarse con él y realizar actividades de blockchain, como consultar el estado de la cadena y enviar mensajes.
            **Si aún no tienes habilitado el puerto API y accesible desde tu máquina Smart Node, necesitarás configurarlo ahora.**
            ...
            <p className="rspress-directive warning">
              <p className="rspress-directive-title">NOTA</p>
              Debido a que Smart Node se ejecutará en su propio contenedor de Docker, usará la red interna de Docker.
              No podrás usar nombres de host como `localhost` o `127.0.0.1` aquí; si tu cliente está ejecutándose en la misma máquina que Smart Node, necesitarás proporcionar la dirección IP LAN de la máquina en su lugar.
            </p>
          ...
          <Tab label="Prysm">
            ...
            **Si aún no tienes los puertos API habilitados y accesibles desde tu máquina Smart Node, necesitarás configurarlos ahora.**
...
A partir de la versión 1.5.0 del stack de Smart Node, puedes proporcionar un par de cliente de Ejecución y cliente de Consenso de "respaldo" que puede tomar el control de tus clientes principales si alguna vez se desconectan (como porque usas Geth y necesitas podarlo).
...
A partir de Smart Node v1.7.0, MEV ahora es _opt-out_, por lo que su configuración se presenta como parte de la configuración inicial, como ves en la siguiente pantalla:
...
Después de esta pregunta, ¡has terminado de configurar Smart Node!
Verás el siguiente cuadro de diálogo:

<img src="./images/tui-finished.png" width="100%" height="auto" />

Si estás satisfecho con tu configuración y estás listo para iniciar Smart Node, haz clic en `Save and Exit` aquí y ve a la sección [Asegurar tu Nodo](./securing-your-node) a continuación.
...
1. La **Lista de Categorías**, que te permite profundizar en las configuraciones de cada categoría del stack de Smart Node
...
3. El botón **Open the Config Wizard**, que te llevará al [Asistente de Configuración](#configuración-a-través-del-asistente) si prefieres esa interfaz
...
Por ejemplo, aquí está la pantalla para la categoría **Smart Node and TX Fees**:
...
¡Se te anima a explorar todas las configuraciones para aprender sobre ellas y ver lo que tu Smart Node puede hacer!
:::
...
Si usas Smart Node en un entorno sin interfaz donde no puedes interactuar con la interfaz de terminal, puedes configurar tu nodo a través de la línea de comandos en su lugar.
