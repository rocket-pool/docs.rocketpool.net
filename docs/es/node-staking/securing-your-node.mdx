...
::: warning NOTA
Si usas tu Smart Node localmente (iniciando sesión físicamente en él con un teclado y monitor conectados directamente), entonces **esta sección no es relevante para ti - puedes omitirla**.
:::

La mayoría de los operadores de Smart Node interactúan con su nodo remotamente conectándose a su terminal desde otra computadora usando `ssh`:
...
Una de las cosas más importantes que puedes hacer para asegurar tu Smart Node es **mantener segura tu máquina cliente**.
...
Ya sea que ejecutes tu Smart Node en casa o uses un VPS en un datacenter remoto, es probable que o bien accedas a él a través de SSH, o que SSH esté habilitado **incluso si no lo usas**.
...
En general, tu máquina solo debería aceptar tráfico de red en puertos que tu cliente de Ejecución, cliente de Consenso y stack de Smart Node usen.
...
Los siguientes comandos configurarán `ufw` con una buena configuración predeterminada para tu Smart Node.
...
Los clientes de consenso de Smart Node que admiten gossip P2P sobre QUIC escuchan en el puerto UDP **8001** por defecto. Si habilitas esa función, permite el puerto en el firewall de tu host:
