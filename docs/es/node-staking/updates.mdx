...
## Actualizando el Stack del Smart Node

Ocasionalmente, Rocket Pool lanzará una nueva versión del stack del Smart Node.
...
::: tip CONSEJO
Cuando hayas completado la actualización del Smart Node, el panel de Grafana todavía indicará que hay una actualización disponible.
...
Los pasos para actualizar dependen del modo que use tu nodo. La mejor forma de actualizar el Smart Node es usando el comando de actualización integrado de la CLI. Selecciona de las opciones a continuación:

<div className="p-3">
  <Tabs>
    <Tab label="Comando de actualización de la CLI (Modo Docker - Linux/macOS)">

      Para actualizar el Smart Node usando el comando de actualización integrado, ejecuta:

      ```shell
      rocketpool update
      ```
      Este comando:
      1. Descarga y verifica el binario más reciente de la CLI
      2. Opcionalmente detiene, actualiza y reinicia el servicio automáticamente

      Sigue las indicaciones interactivas para completar la actualización.
      ```
      ...
      Después, el Smart Node descargará y verificará el binario más reciente de la CLI:
      ...
      y
      ```
      En este punto, puedes seleccionar `y` en todas las indicaciones interactivas para que el Smart Node te guíe por el resto de los pasos.
      Cuando termine, verás un mensaje confirmando que la actualización fue exitosa:
      ```
    ...
    <Tab label="Actualización manual - Linux (Modo Docker o Híbrido)">
      ...
      Descarga la nueva CLI del Smart Node:
      ...
      Finalmente, verifica la versión para asegurarte de que la CLI y el stack del Smart Node estén actualizados:
    ...
    <Tab label="Actualización manual - Linux (Modo Nativo)">
      <p className="rspress-directive warning">
        <p className="rspress-directive-title">NOTA</p>
        Esto solo actualizará el stack del Smart Node en sí - **no** actualizará tus clientes de Ejecución o Consenso.
      ...
      Descarga la nueva CLI del Smart Node y respalda los binarios antiguos de la CLI y el daemon por si necesitas recuperarlos:
    ...
    <Tab label="Actualización manual - macOS (Modo Docker o Híbrido)">
      Detén los servicios de Rocket Pool:
...
Cada nuevo lanzamiento del Smart Node incluye digests de imágenes Docker anclados para los contenedores de Ejecución, Consenso y del propio Smart Node (`repo:tag@sha256:…` en `~/.rocketpool/mainnet.env`). `rocketpool service install` sobrescribe esos archivos de anclaje oficiales. En algunos casos podrías querer ejecutar una imagen de cliente diferente _antes_ de esperar un nuevo lanzamiento del Smart Node. Esta sección te muestra cómo.
...
<div className="p-3">
  <Tabs>
    <Tab label="Modo Docker">
      ...
      El Smart Node ahora ancla por digest las imágenes de los clientes y del propio Smart Node.
      Cambiar **Container Tag** en la TUI te sigue permitiendo ejecutar un tag o digest diferente antes del próximo lanzamiento del Smart Node.
      Después de `rocketpool service install` o `rocketpool update`, los anclajes oficiales de `mainnet.env` se reemplazan; una anulación hecha en la TUI sigue prevaleciendo hasta que la elimines.
      ...
      El Smart Node ofrecerá reiniciar todos los contenedores afectados automáticamente.

      ::: tip NOTA
      Los anclajes oficiales están en `~/.rocketpool/mainnet.env`. No edites ese archivo para hacer cambios duraderos; la instalación lo sobrescribe. Las anulaciones opcionales por red van en `~/.rocketpool/testnet.env` o `~/.rocketpool/devnet.env` (solo las claves que definas reemplazan el anclaje de mainnet). Un **Container Tag** que establezcas en el Administrador de Configuración anula el anclaje para ese servicio. Déjalo en el valor por defecto para conservar el digest oficial.
      :::
    </Tab>
