...
::: danger ADVERTENCIA
La ruta de CoW Swap no es parte del protocolo de Rocket Pool. Las cotizaciones y los solvers son determinados por sistemas de **terceros** que están fuera del control o la visibilidad del protocolo de Rocket Pool. Los usuarios deben tener en cuenta que el uso de CoW Swap puede exponerlos a riesgos adicionales como slippage, impacto en el precio, solvers que enrutan fondos incorrectamente, etc. Para un control detallado,
recomendamos encarecidamente que utilice CoW Swap directamente.
:::
