...
Lee el [ciclo de vida de una propuesta](./pdao#ciclo-de-vida-de-una-propuesta-del-pdao) antes de continuar. Explicará las diferencias entre todos los períodos de votación y las acciones que se pueden tomar durante cada período.
...
- Si respondes con `delegate`, tendrás la oportunidad de ahorrar un poco de gas al inicializar la votación y [establecer un delegado](./participate#delegando-el-poder-de-voto) dentro de la misma transacción.
...
La versión `1.13.8` del Smart Node introduce una nueva característica **Inicialización automática del poder de voto**, que está diseñada para inicializar automáticamente el poder de voto en nodos que aún no lo han hecho. El poder de voto inicializado automáticamente es auto-delegado. Esta característica puede configurarse en la configuración del Smart Node ejecutando `rocketpool service config` y navegando a la sección **Smart Node and TX Fees**.
...
- Incluido en el snapshot (ya sea [inicializando la votación](./participate#inicializando-la-votación) o registrándose después de Houston)
- Debe tener al menos un minipool
- Tiene poder de voto distinto de cero
- Ha permitido el [bloqueo de RPL](./participate#permitiendo-el-bloqueo-de-rpl)
- Tiene una participación de RPL (menos cualquier RPL ya bloqueado) mayor que el bono de propuesta

Las propuestas existen para cambiar parámetros y ejecutar código a nivel del protocolo! Debe haber discusión y consenso a través del proceso de [gobernanza](./participate#proceso-de-gobernanza) antes de que se cree una propuesta on-chain.
...
Cada uno de estos comandos te pedirá una lista de entradas para crear tu propuesta deseada. Los comandos de propuesta de configuración aceptan `--to-json` para escribir o añadir un cambio de configuración a un archivo JSON en lugar de enviar una transacción. Cuando el archivo esté listo, envíalo con `rocketpool pdao propose submit-batch`.

En esta guía, invitaremos a un nodo al consejo de seguridad para servir como ejemplo. Para presentar una propuesta para invitar a un nodo al consejo de seguridad, usarías el comando:
...
Después de que esto se incluya en un bloque, se creará una propuesta del pDAO! La propuesta entrará en el [período de retraso de votación](./pdao#período-de-retraso-de-votación) al momento de la creación.
...
<div className="p-3">
  <Tabs>
    <Tab label="Viendo una lista de propuestas">
      ...
      los [retadores](/es/pdao/pdao#proceso-de-desafío) pueden disputar la validez del
      ...
      propuesta](./pdao#ciclo-de-vida-de-una-propuesta-del-pdao) para un repaso.
    </Tab>
