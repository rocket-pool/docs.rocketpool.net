...
Una proposta dovrebbe essere preannunciata dal [processo di governance](./participate#processo-di-governance) prima che finisca on-chain. Consiste di 4 periodi, tutti parametri [controllabili dalla pDAO](https://rpips.rocketpool.net/RPIPs/RPIP-33#parameter-table):
...
Dopo che la proposta ha superato i periodi di voto, il proponente può [richiedere](./participate#richiedere-garanzie-e-ricompense) la sua garanzia RPL bloccata, a meno che la proposta non sia stata sconfitta da una sfida o sottoposta a veto.
...
Qualsiasi nodo può partecipare al tracciamento e alla verifica della correttezza delle proposte. Per aderire a questa responsabilità, usa il comando `rocketpool service config`, naviga nel menu **Smart Node and TX Fee Settings** e seleziona la casella `Enable PDAO Proposal Checker`.

<img src={proposal_checker} width="100%" height="auto" />

Quando questa impostazione è abilitata, il nodo controllerà le nuove proposte, verificherà la loro correttezza e invierà sfide alle proposte non valide. L'unico prerequisito è che il [blocco RPL](./participate#consentire-il-blocco-rpl) sia abilitato.
...
Ora che la proposta è sconfitta, il nostro nodo (lo sfidante) può [richiedere](./participate#richiedere-garanzie-e-ricompense) la sua garanzia RPL originale e la garanzia RPL del proponente come ricompensa per aver sconfitto una proposta fraudolenta.
