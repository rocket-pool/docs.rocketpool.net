...
Se un validator in prelaunch non riesce a fare lo stake entro 365 giorni, il validator verrà dissolto. 1 ETH (di un bond da 4 ETH) inviato alla Beacon Chain durante il processo di prelaunch **non è recuperabile**.
Il node operator riceve i restanti 3 ETH dal suo bond (meno una penale di dissoluzione di 0.05 ETH applicata come debito) come rimborso nel suo megapool, per un rimborso netto di 2.95 ETH.
Questo può essere richiesto utilizzando `rocketpool megapool claim` o `rocketpool claims claim-all`.
...
- 32 ETH depositati nella Beacon Chain
- Assegnato un numero di indice del validator
- In attesa di attivazione sulla Beacon Chain
