...
Dopo che il tuo validator `Prelaunch` è stato elaborato dalla Beacon Chain e il deposito iniziale di 1 ETH è stato accreditato, il tuo nodo eseguirà automaticamente una
transazione `stake` per completare il deposito completo di 32 ETH sulla Beacon Chain. La transazione `stake` farà progredire il tuo validator `Prelaunch`
verso un validator `Staking`. A questo punto il tuo validator `Staking` ha:
