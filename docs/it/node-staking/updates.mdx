...
## Aggiornamento dello Stack Smart Node

Occasionalmente, Rocket Pool rilascerà una nuova versione dello stack Smart Node.
...
::: tip SUGGERIMENTO
Quando hai completato l'aggiornamento dello Smart Node, il dashboard Grafana indicherà ancora che è disponibile un aggiornamento.
...
I passaggi per aggiornare dipendono dalla modalità che il tuo nodo utilizza. Il modo migliore per aggiornare lo Smart Node è usare il comando di aggiornamento integrato della CLI. Seleziona dalle opzioni seguenti:

<div className="p-3">
  <Tabs>
    <Tab label="Comando di aggiornamento CLI (Modalità Docker - Linux/macOS)">

      Per aggiornare lo Smart Node usando il comando di aggiornamento integrato, esegui:

      ```shell
      rocketpool update
      ```
      Questo comando:
      1. Scarica e verifica il binario più recente della CLI
      2. Facoltativamente arresta, aggiorna e riavvia il servizio automaticamente

      Segui i prompt interattivi per completare l'aggiornamento.
      ```
      ...
      Successivamente, lo Smart Node scaricherà e verificherà il binario più recente della CLI:
      ...
      y
      ```
      A questo punto, puoi selezionare `y` a tutti i prompt interattivi per lasciare che lo Smart Node ti guidi nei passaggi rimanenti.
      Al termine, vedrai un messaggio che conferma il buon esito dell'aggiornamento:
      ```
    ...
    <Tab label="Aggiornamento manuale - Linux (Modalità Docker o Hybrid)">
      ...
      Scarica la nuova CLI Smart Node:
      ...
      Infine, controlla la versione per assicurarti che la CLI e lo stack Smart Node siano entrambi aggiornati:
    ...
    <Tab label="Aggiornamento manuale - Linux (Modalità Native)">
      <p className="rspress-directive warning">
        <p className="rspress-directive-title">NOTA</p>
        Questo aggiornerà solo lo stack Smart Node stesso - **non** aggiornerà i tuoi client Execution o Consensus.
      ...
      Scarica la nuova CLI Smart Node e fai un backup dei vecchi binari CLI e daemon nel caso tu debba ripristinarli:
    ...
    <Tab label="Aggiornamento manuale - macOS (Modalità Docker o Hybrid)">
      Ferma i servizi Rocket Pool:
...
Ogni nuovo rilascio dello Smart Node include digest di immagini Docker bloccati per i container Execution, Consensus e Smart Node (`repo:tag@sha256:…` in `~/.rocketpool/mainnet.env`). `rocketpool service install` sovrascrive quei file di pin ufficiali. In alcuni casi potresti voler eseguire un'immagine del client diversa _prima_ di aspettare un nuovo rilascio dello Smart Node. Questa sezione ti mostra come.
...
<div className="p-3">
  <Tabs>
    <Tab label="Modalità Docker">
      ...
      Lo Smart Node ora blocca tramite digest le immagini dei client e dello Smart Node.
      Modificare **Container Tag** nella TUI ti permette comunque di eseguire un tag o un digest diverso prima del prossimo rilascio dello Smart Node.
      Dopo `rocketpool service install` o `rocketpool update`, i pin ufficiali in `mainnet.env` vengono sostituiti; una sovrascrittura impostata nella TUI continua a prevalere finché non la rimuovi.
      ...
      Lo Smart Node si offrirà di riavviare automaticamente tutti i container interessati.

      ::: tip NOTA
      I pin ufficiali si trovano in `~/.rocketpool/mainnet.env`. Non modificare quel file per cambiamenti duraturi; l'installazione lo sovrascrive. Le sovrascritture facoltative per rete vanno in `~/.rocketpool/testnet.env` o `~/.rocketpool/devnet.env` (solo le chiavi che imposti sostituiscono il pin di mainnet). Un **Container Tag** impostato nel Settings Manager prevale sul pin per quel servizio. Lascialo al valore predefinito per mantenere il digest ufficiale.
      :::
    </Tab>
