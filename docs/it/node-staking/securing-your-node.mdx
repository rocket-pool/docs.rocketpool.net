...
::: warning NOTA
Se usi il tuo Smart Node localmente (collegandoti fisicamente con tastiera e monitor direttamente collegati), allora **questa sezione non è rilevante per te - puoi saltarla**.
:::

La maggior parte degli operatori Smart Node interagisce con il proprio nodo da remoto connettendosi al suo terminale da un altro computer usando `ssh`:
...
Una delle cose più importanti che puoi fare per proteggere il tuo Smart Node è **mantenere sicura la tua macchina client**.
...
Che tu gestisca il tuo Smart Node a casa o usi un VPS in un datacenter remoto, è probabile che tu vi acceda tramite SSH, o che SSH sia abilitato **anche se non lo usi**.
...
In generale, la tua macchina dovrebbe accettare solo traffico di rete sulle porte che il tuo client Execution, client Consensus e stack Smart Node usano.
...
I seguenti comandi configureranno `ufw` con una buona configurazione predefinita per il tuo Smart Node.
...
I client di consenso di Smart Node che supportano il gossip P2P su QUIC ascoltano sulla porta UDP **8001** per impostazione predefinita. Se abiliti questa funzione, consenti la porta nel firewall del tuo host:
