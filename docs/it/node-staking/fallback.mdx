...
A partire dalla versione **1.5.0** dello stack Smart Node, puoi fornire una coppia "di fallback" di client Execution e Consensus che può subentrare ai tuoi client primari se mai dovessero andare offline (come ad esempio perché [usi Geth e hai bisogno di fare il pruning](./pruning)).
...
Una volta che i client primari del tuo nodo sono di nuovo online, lo Smart Node e il tuo client Validator torneranno a usarli automaticamente.
...
Questo esempio ti mostra come creare un secondo Smart Node su una macchina diversa usando Docker mode, che può servire come nodo di fallback.
...
2. Una volta che la macchina è pronta, [installa lo stack Smart Node](./docker).
3. Esegui `rocketpool service config` per specificare quali client desideri utilizzare.
   ...
   6. Salva le impostazioni e avvia lo Smart Node.
...
5. Facoltativamente, spunta **Prefer Fallback Clients** (disattivato per impostazione predefinita). Se abilitato, lo Smart Node invierà per prime le richieste di API, nodo e watchtower ai tuoi client Execution e Consensus di fallback, e userà la coppia primaria solo se il fallback non è disponibile. In questo modo il carico dello Smart Node resta lontano dai client primari, che possono concentrarsi sui compiti di validazione. Il tuo Validator Client non è interessato e continua a usare prima la coppia primaria.
...
Nota che questo fornirà solo allo **Smart Node stesso** (il servizio daemon) il supporto per il fallback; **dovrai aggiornare manualmente gli argomenti del servizio del tuo client Validator per dargli accesso ai client di fallback.**
:::
