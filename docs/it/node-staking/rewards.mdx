...
Il Merkle Tree viene costruito in un file JSON e ospitato su [un repository GitHub che manteniamo](https://github.com/rocket-pool/rewards-trees/). Lo Smart Node può anche caricare gli **eventi** di intervallo delle ricompense da quel repository GitHub, quindi puoi ancora richiedere le ricompense dopo che una scadenza aggressiva della cronologia ha eliminato i vecchi log del livello Execution.
...
- Se il tuo client di Execution primario non è un nodo archive, puoi aggiungere un URL per un nodo archive separato nella casella `Archive-Mode EC URL`.
  ...
  - L'Archive EC verrà utilizzato solo per la generazione dell'albero; non verrà utilizzato per altre funzioni dello Smart Node.
...
Ora il tuo Smart Node costruirà l'albero completamente indipendentemente dall'Oracle DAO, utilizzando solo i dati forniti dai tuoi client di Execution e Consensus.
...
Se **non** sei iscritto allo Smoothing Pool, la parte del livello di Execution delle tue ricompense dalle proposte di blocchi (incluse le commissioni di transazione e MEV) verrà inviata al contratto [Fee Distributor](./fee-distrib-sp#fee-distributor-e-lo-smoothing-pool) del tuo nodo.
