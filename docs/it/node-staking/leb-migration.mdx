...
Si noti che, come dimostrato nella sezione [matematica di esempio](./create-validator.mdx), un minipool da 8 ETH al 14% è _ancora più redditizio_ di un minipool da 16 ETH al 20%, quindi mantenere un tasso di commissione elevato non è una ragione convincente per mantenere un bond da 16 ETH.
:::
...
Una volta selezionato uno o più minipool da ridurre, lo Smart Node verificherà se quei minipool sono idonei per la riduzione del bond.
...
Questo credito può essere utilizzato per [creare un altro minipool da 8 ETH](./create-validator.mdx#deposito-di-eth-e-creazione-di-un-minipool) gratuitamente (nessun ETH richiesto dal portafoglio del nodo, tranne che per il gas)!
