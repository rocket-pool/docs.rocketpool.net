...
Nel passaggio **Add Tags**, dovreste fare clic sul pulsante **click to add a Name tag** e assegnargli un nome facile da ricordare che potete usare per identificare il server in seguito; qualcosa come **Rocket Pool Smart Node** funzionerà se avete bisogno di ispirazione.
...
Inserite un nome e una descrizione come **Smart Node** e **Smart Node Group**.
...
Assegnategli un nome memorabile come **Smart Node Key**, quindi fate clic su **Download Key Pair**.
...
Lasciate il tipo di risorsa impostato su **Instance** e selezionate l'istanza Rocket Pool Smart Node che avete appena creato dall'elenco.
