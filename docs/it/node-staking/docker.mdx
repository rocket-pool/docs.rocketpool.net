...
In questa sezione, vedremo il processo di installazione dello stack Rocket Pool Smart Node utilizzando la configurazione standard basata su [Docker](https://www.docker.com/resources/what-container).
Questo installerà e configurerà tutto il necessario per eseguire un nodo completo, inclusi:

- Il software **Smart Node** di Rocket Pool
...
1. Usa la CLI per installare lo stack Smart Node
1. Configura lo stack Smart Node con un'interfaccia utente facile da usare nel terminale
1. Fatto!
...
L'installer di Ubuntu ottiene docker dal gestore di pacchetti snap, che entrerà in conflitto con la versione di docker installata da Smart Node.
...
## Installazione dello Stack Smart Node

Ora che hai installato la CLI, puoi distribuire lo stack Smart Node.
Questo preparerà il tuo sistema con Docker, [docker-compose](https://docs.docker.com/compose/) e caricherà i file Smart Node in modo che siano pronti per l'uso.
Non eseguirà ancora nulla; questo arriverà dopo.

Per distribuire lo stack Smart Node, dovrai eseguire il seguente comando sulla macchina del nodo (effettuando l'accesso localmente o connettendoti da remoto come tramite SSH):

```shell
rocketpool service install
```

Questo scaricherà l'ultima versione dello stack Smart Node e lo configurerà.
...
::: warning NOTA
Nota che il programma di installazione di Smart Node non può installare `docker` e `docker-compose` su tutte le piattaforme automaticamente.
...
Una volta terminato, lo stack Smart Node sarà pronto per essere eseguito.
...
## Configurazione dello Stack Smart Node
