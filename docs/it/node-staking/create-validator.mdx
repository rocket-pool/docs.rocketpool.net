...
per fare staking da esso. Puoi farlo tramite il seguente comando Smart Node:
...
Se desideri rimuovere un indirizzo dalla tua whitelist di stake-on-behalf, puoi farlo con il seguente comando Smart Node:
...
Tuttavia, lo Smart Node offre la possibilità di cercare un **indirizzo vanity** personalizzato per il minipool.
...
Vedrai prima una nota che depositare un nuovo minipool **distribuirà automaticamente** qualsiasi saldo nel contratto [fee distributor](./fee-distrib-sp) del tuo nodo (utilizzato per catturare le ricompense MEV se non hai optato per il [Smoothing Pool](./fee-distrib-sp#lo-smoothing-pool)):
