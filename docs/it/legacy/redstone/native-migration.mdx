...
Prima di aggiornare alla v1.5.0 e versioni successive dello Smart Node, segui questa checklist per assicurarti di essere pronto:
...
Ti mostreremo come configurare il tuo client Validator in modo che utilizzi automaticamente il fee recipient corretto richiesto dal software Smart Node qui sotto.
...
Segui semplicemente [le normali istruzioni qui](../../node-staking/updates#aggiornamento-dello-stack-smartnode).
...
- Controlla i tuoi script di log per il client di Esecuzione, il client di Consenso, il client Validator e il daemon Smart Node (il servizio `rp-node`) per assicurarti che funzionino tutti normalmente senza errori.
...
Come descritto nell'[articolo panoramica](./whats-new#fee-recipient-e-il-tuo-distributor), questo può essere uno di due valori:
...
In modalità Nativa, hai la scelta di lasciare che lo Smart Node gestisca questo per te se utilizzi il servizio daemon Smart Node, `rp-node`, o gestirlo tu stesso se non utilizzi il daemon.

#### Gestione Automatica tramite il Daemon

Il daemon Smart Node determinerà automaticamente il fee recipient appropriato per il tuo nodo e lo gestirà nel caso cambi (come quando aderisci o esci dallo Smoothing Pool).
Questa è l'opzione **più sicura**, perché lo Smart Node assicurerà sempre che sia impostato su un valore che **previene la [penalizzazione](https://github.com/rocket-pool/rocketpool-research/blob/master/Penalties/penalty-system)**.
...
<div className="p-3">
  <Tabs>
    <Tab label="Lighthouse">
      ...
      Il tuo VC ora utilizzerà il file gestito dal daemon Smart Node, e sarà automaticamente riavviato ogni volta che il fee recipient cambia.
    </Tab>
    <Tab label="Nimbus">
      Modifica il tuo servizio Client Validator aggiungendo la seguente riga _prima_ della riga `ExecStart`:
...
<div className="p-3">
  <Tabs>
    <Tab label="Lighthouse">
      ...
      - L'[indirizzo rETH](/it/protocol/contracts-integrations/#contratti-dei-token) **prima** che l'aggiornamento Redstone venga distribuito (es., `0xae78736Cd615f374D3085123A210448E74Fc6393` su Mainnet)
      ...
      - L'[indirizzo dello Smoothing Pool](/it/protocol/contracts-integrations/#contratti-principali-del-protocollo) se aderisci allo Smoothing Pool
...
Lo Smart Node ha ancora la capacità di fornire un client di Esecuzione di fallback (e ora anche un client di Consenso di fallback), ma ora dovrai utilizzare client di Esecuzione e Consenso che controlli.
...
Se non hai intenzione di aderire allo [Smoothing Pool](./whats-new#smoothing-pool) e richiedere tutte le tue commissioni prioritarie e ricompense MEV al tuo [contratto distributore di commissioni](./whats-new#fee-recipient-e-il-tuo-distributor), dovrai eventualmente inizializzarlo (creare l'istanza del contratto sulla catena) per poter richiedere le ricompense al tuo indirizzo di prelievo.
...
L'aggiornamento Redstone sostituisce il vecchio sistema di ricompense costoso e problematico con un [sistema completamente nuovo](./whats-new#nuovo-sistema-di-ricompense) che è molto più economico, supporta il restaking automatico di RPL (sia importi parziali che completi), e - soprattutto - **ti permette di richiedere le tue ricompense quando vuoi**.

Poiché non c'è più un limite di tempo per richiedere le ricompense, e poiché è più economico richiedere molti intervalli di ricompense in una volta, la funzionalità di richiesta automatica delle ricompense dello Smart Node **è stata rimossa**.
