...
Prima di aggiornare alla v1.5.0 e versioni successive dello Smart Node, esamina la seguente checklist per assicurarti di essere preparato:
...
[Questa guida](../../node-staking/change-clients#cambiare-execution-client) fornisce una procedura dettagliata del processo.

## Aggiornamento alla v1.5.0

L'aggiornamento dello stack Smart Node alla v1.5.0 non è diverso da qualsiasi altro aggiornamento.
Basta seguire [le normali istruzioni qui](../../node-staking/updates#aggiornamento-dello-stack-smartnode).

## Cose che lo Smart Node Gestisce Automaticamente

In modalità Docker, lo Smart Node si occuperà automaticamente della maggior parte delle modifiche necessarie per supportare Redstone e il Merge una volta aggiornato alla v1.5.0.
...
Lo Smart Node gestirà automaticamente la configurazione del token di autenticazione e dell'Engine API sia sul tuo client di Esecuzione che su quello di Consenso.
...
Lo Smart Node gestirà la configurazione dell'indirizzo corretto quando aggiorni alla v1.5 e controllerà costantemente che tu stia utilizzando quello corretto in modo da non essere [penalizzato accidentalmente](https://github.com/rocket-pool/rocketpool-research/blob/master/Penalties/penalty-system).

Se hai optato per il [Smoothing Pool](./whats-new#smoothing-pool), questo diventerà il tuo fee recipient.
Se non l'hai fatto, il tuo [contratto di distribuzione delle commissioni](./whats-new#fee-recipient-e-il-tuo-distributor) diventerà il fee recipient.
...
Rocket Pool ha MEV-Boost integrato nello Smart Node e configura automaticamente il tuo nodo per utilizzarlo, così le tue proposte ottengono la massima quantità di ricompense.

## Cose che Dovresti Fare Dopo l'Aggiornamento

Anche se lo Smart Node gestisce la maggior parte delle modifiche per te, ci sono alcune cose aggiuntive che dovresti fare manualmente:
...
Lo Smart Node ha ancora la capacità di fornire un client di Esecuzione di fallback (e ora anche un client di Consenso di fallback), ma ora dovrai utilizzare client di Esecuzione e Consenso che controlli tu.
...
Se non prevedi di aderire al [Smoothing Pool](./whats-new#smoothing-pool) e richiedere tutte le tue commissioni prioritarie e ricompense MEV al tuo [contratto di distribuzione delle commissioni](./whats-new#fee-recipient-e-il-tuo-distributor), alla fine dovrai inizializzarlo (creare l'istanza del contratto sulla catena) per poter richiedere le ricompense da esso al tuo indirizzo di prelievo.
...
L'aggiornamento Redstone sostituisce il vecchio sistema di ricompense costoso e problematico con uno [completamente nuovo](./whats-new#nuovo-sistema-di-ricompense) che è molto più economico, supporta il restaking automatico di RPL (sia importi parziali che completi) e - cosa più importante - **ti consente di richiedere le tue ricompense quando vuoi**.

Poiché non c'è più un limite di tempo per richiedere le ricompense e poiché è più economico richiedere molti intervalli di ricompense contemporaneamente, la funzione di richiesta automatica delle ricompense dello Smart Node **è stata rimossa**.
...
Se, per qualsiasi motivo, qualcosa non è di tuo gradimento e desideri ripristinare la versione precedente dello Smart Node, puoi farlo facilmente.
Lo Smart Node esegue automaticamente il backup delle impostazioni della versione precedente quando lo aggiorni, quindi basta ottenere la versione precedente (qui stiamo dimostrando la **v1.4.3**) e sostituire le impostazioni con il backup:
...
1. Se sembra a posto, avvia lo stack Smart Node:
