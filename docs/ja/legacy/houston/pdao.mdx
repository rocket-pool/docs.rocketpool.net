...
提案は、オンチェーンに至る前に[ガバナンスプロセス](../houston/participate#ガバナンスプロセス)によって予測されるべきです。これは4つの期間で構成され、すべてがpDAO[制御可能なパラメータ](https://rpips.rocketpool.net/RPIPs/RPIP-33#parameter-table)です：
...
チャレンジャーは、不正であると主張しているMerkle-sumツリーへのインデックスを提供します。任意のノードオペレーターは不正な提案への挑戦に参加できます（そうすることで報酬を得ることができます）。参加を希望する場合は、[pDAOチャレンジプロセス](../houston/pdao#チャレンジプロセス)について自由に読んでください。投票遅延期間の終わりまでにチャレンジで敗北しなかった提案は投票段階に入ります。
...
提案が投票期間を通過した後、提案者はチャレンジによって敗北したか拒否されたのでない限り、ロックされたRPL担保を[請求](../houston/participate#ボンドと報酬の請求)できます。
...
任意のノードは、提案の正確性の追跡と検証に参加できます。この責任を選択するには、コマンド`rocketpool service config`を使用し、**Smart Node and TX Fee Settings**メニューに移動し、`Enable PDAO Proposal Checker`のチェックボックスをオンにします。

<img src={proposal_checker} width="100%" height="auto" />

この設定が有効になると、ノードは新しい提案をチェックし、その正確性を検証し、無効な提案に対するチャレンジを提出します。唯一の前提条件は、[RPLロック](../houston/participate#rplロックの許可)が有効になっていることです。
...
提案が敗北したので、ノード（チャレンジャー）は元のRPL担保と、不正な提案を敗北させたことに対する報酬として提案者のRPL担保を[請求](../houston/participate#ボンドと報酬の請求)できます。
