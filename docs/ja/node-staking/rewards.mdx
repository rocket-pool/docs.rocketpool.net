...
Merkle TreeはJSONファイルに構築され、[私たちが管理するGitHubリポジトリ](https://github.com/rocket-pool/rewards-trees/)でホストされます。Smart Nodeは報酬インターバルの**イベント**をそのGitHubリポジトリから読み込むこともできるため、積極的な履歴削除によって古いExecutionレイヤーのログが削除された後でも請求できます。
...
- プライマリのExecution clientがアーカイブノードでない場合は、`Archive-Mode EC URL`ボックスに別のアーカイブノードのURLを追加できます。
  ...
  - Archive ECはツリー生成にのみ使用されます。他のSmart Nodeの職務には使用されません。
...
これで、Smart NodeはOracle DAOから完全に独立してツリーを構築し、Execution clientとConsensus clientによって提供されるデータのみを使用します。
...
Smoothing Poolにオプトイン**していない**場合、ブロック提案からのExecution-layer部分の報酬（トランザクション手数料とMEVを含む）は、代わりにノードの[Fee Distributor](./fee-distrib-sp#fee-distributorとsmoothing-pool)コントラクトに送信されます。
...
Fee Distributorの残高にアクセスするには、それを**分配**します（したがって、名前は*Fee Distributor*です）。
