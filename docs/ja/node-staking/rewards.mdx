...
Merkle TreeはJSONファイルに構築され、[私たちが管理するGitHubリポジトリ](https://github.com/rocket-pool/rewards-trees/)でホストされます。Smartnodeは報酬インターバルの**イベント**をそのGitHubリポジトリから読み込むこともできるため、積極的な履歴削除によって古いExecutionレイヤーのログが削除された後でも請求できます。
