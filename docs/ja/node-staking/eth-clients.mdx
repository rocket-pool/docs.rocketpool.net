...
Rocket PoolのSmart Nodeインストーラーは、お使いのマシンを完全なEthereumノードに変換できます。適切に動作するためには、ExecutionクライアントとConsensusクライアントの両方が必要です。
...
迅速にセットアップして稼働させたいユーザーのために、Smart Nodeインストーラーは`Random Client`オプションを提供しており、これが最適な選択である可能性があります。
