...
Smart Node設定プロセス中にメトリクスを有効にすることを選択すると、ノードに次のプロセスが追加されます。
...
デフォルト設定では、Smart Nodeの他のDockerコンテナと並んで存在するこれらすべてのサービスを含むDockerコンテナが作成されます。
...
<div className="p-3">
  <Tabs>
    <Tab label="Docker">
      ...
      このチェック中に、インストールされているRocket Pool Smart Nodeバージョンと最新リリースも比較し、新しいリリースが利用可能な場合は通知します。
...
Smart Nodeインストールモードについては、以下のタブを参照してください。
...
**Import via grafana.com**ボックスでダッシュボードIDの入力を求められたら、`24900`を入力するか、完全なURL([https://grafana.com/grafana/dashboards/24900-rocket-pool-dashboard-v1-4-0/](https://grafana.com/grafana/dashboards/24900-rocket-pool-dashboard-v1-4-0/))を使用して、**Load**ボタンを押します。
...
::: tip 注意
Smart Node v1.23.0では、メトリクススタックとともに**Rocket Pool Dashboard v1.4.2**が提供されます。このリビジョンでは、ステータス別のノード単位のminipoolおよびmegapool validator数が追加され、削除されたsupply-collectorメトリクスに依存していたネットワーク全体のmegapoolステータスパネルが廃止されました。Grafanaに古いダッシュボードを既にインポートしている場合は、再インポート（またはSmart Nodeに同梱されているJSONを読み込み）して、これらのパネルがノードのエクスポートするメトリクスと一致するようにしてください。
:::
...
過去の報酬を適切に追跡できるSmart Nodeの将来のバージョンで再び有効になります。
:::
...
Smart Nodeインストール中にDockerのデフォルトの場所を別のドライブに変更しなかった場合、追跡するディスクはOSがインストールされているディスクになります。
