...
[サンプル計算](./create-validator.mdx)セクションで示されているように、14%の8-ETH minipoolは20%の16-ETH minipoolよりも*依然として収益性が高い*ため、高い手数料率を保持することは、16 ETH bondを維持する説得力のある理由ではありません。
:::
...
削減する1つ以上のminipoolを選択すると、Smart Nodeはそれらのminipoolがbond reductionの資格があるかどうかを確認します。
...
このクレジットを使用して、[別の8-ETH minipoolを作成](./create-validator.mdx#ethのデポジットとminipoolの作成)できます(ノードウォレットからのETHは不要で、ガス代のみ必要です)。
