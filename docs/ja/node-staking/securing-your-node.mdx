...
::: warning 注記
Smart Nodeをローカルで使用している（キーボードとモニターを直接接続して物理的にログインしている）場合、**このセクションは関係ありません - スキップできます**。
:::

ほとんどのSmart Nodeオペレーターは、`ssh`を使用して別のコンピューターからターミナルに接続することで、ノードとリモートでやり取りします。

- 接続*先*のマシン（この場合、ノードマシン）は**サーバー**と呼ばれます。
- 接続*元*のマシン（ラップトップ、デスクトップ、さらにはスマートフォンなど）は**クライアント**と呼ばれます。

Smart Nodeを保護するために最も重要なことの1つは、**クライアントマシンのセキュリティを確保する**ことです。
...
自宅でSmart Nodeを運用している場合でも、リモートデータセンターでVPSを使用している場合でも、SSH経由でアクセスするか、**使用していなくてもSSHが有効になっている**可能性があります。
...
一般的に、マシンはExecution client、Consensus client、およびSmart Nodeスタックが使用するポートでのみネットワークトラフィックを受け入れる必要があります。
...
次のコマンドは、Smart Nodeの適切なデフォルト構成で`ufw`を設定します。
**ノードマシンでこれらを実行してください**。
...
QUICによるP2P gossipをサポートするSmart Nodeのconsensus clientは、デフォルトでUDPポート**8001**をリッスンします。この機能を有効にする場合は、ホストファイアウォールでそのポートを許可してください:
