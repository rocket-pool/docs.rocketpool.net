...
時折、Rocket PoolはSmart Nodeスタックの新しいバージョンをリリースします。
...
::: tip ヒント
Smart Nodeのアップグレードが完了した後も、Grafanaダッシュボードには引き続きアップデートが利用可能であることが表示されます。
...
アップグレードの手順は、ノードが使用するモードによって異なります。Smart Nodeを更新する最良の方法は、CLIの組み込み更新コマンドを使用することです。以下のオプションから選択してください。

<div className="p-3">
  <Tabs>
    <Tab label="CLI更新コマンド (Docker Mode - Linux/macOS)">

      組み込みの更新コマンドを使用してSmart Nodeを更新するには、次を実行します:

      ```shell
      rocketpool update
      ```
      このコマンドは次を行います:
      1. 最新のCLIバイナリをダウンロードして検証します
      2. 必要に応じてサービスを自動的に停止、アップグレード、再起動します

      対話式のプロンプトに従って更新を完了してください。
      ```
      ...
      その後、Smart Nodeが最新のCLIバイナリをダウンロードして検証します:
      ...
      y
      ```
      この時点で、すべての対話式プロンプトで`y`を選択すると、Smart Nodeが残りの手順を案内します。
      完了すると、アップグレードが成功したことを確認するメッセージが表示されます:
      ```
    ...
    <Tab label="手動更新 - Linux (Docker or Hybrid Mode)">
      ...
      最後に、バージョンを確認して、CLIとSmart Nodeスタックの両方が最新であることを確認します。
    ...
    <Tab label="手動更新 - Linux (Native Mode)">
      <p className="rspress-directive warning">
        <p className="rspress-directive-title">注意</p>
        これはSmart Nodeスタック自体のみを更新します - Execution ClientまたはConsensus Clientは**更新されません**。
      ...
      新しいSmart Node CLIをダウンロードし、必要に応じて復元できるように古いCLIとデーモンバイナリをバックアップします。
    ...
    <Tab label="手動更新 - macOS (Docker or Hybrid Mode)">
      Rocket Poolサービスを停止します。
...
Smart Nodeの各新リリースには、Execution、Consensus、Smart Nodeの各コンテナ用にピン留めされたDockerイメージのダイジェストが同梱されます（`~/.rocketpool/mainnet.env` 内の `repo:tag@sha256:…`）。`rocketpool service install` はこれらの公式ピン留めファイルを上書きします。場合によっては、新しいSmart Nodeのリリースを待つ*前に*別のクライアントイメージを実行したいことがあります。このセクションではその方法を説明します。
...
<div className="p-3">
  <Tabs>
    <Tab label="Docker Mode">
      ...
      Smart Nodeは現在、クライアントイメージとSmart Nodeイメージをダイジェストでピン留めしています。
      TUIで **Container Tag** を変更すれば、次のSmart Nodeリリースを待つ前に別のタグやダイジェストを実行できます。
      `rocketpool service install` または `rocketpool update` を実行すると、`mainnet.env` 内の公式ピン留めは置き換えられます。TUIでの上書きは、解除するまで引き続き優先されます。
      ...
      **Beacon Node Container Tag**設定を変更します。
      ...
      Smart Nodeは影響を受けるすべてのコンテナを自動的に再起動することを提案します。

      ::: tip 注意
      公式のピン留めは `~/.rocketpool/mainnet.env` にあります。永続的な変更のためにこのファイルを編集しないでください。インストール時に上書きされます。ネットワークごとの任意の上書きは `~/.rocketpool/testnet.env` または `~/.rocketpool/devnet.env` に記述します（設定したキーのみがmainnetのピン留めを置き換えます）。設定マネージャーで設定した **Container Tag** は、そのサービスのピン留めを上書きします。公式のダイジェストを使い続けるには、デフォルトのままにしてください。
      :::
    </Tab>
