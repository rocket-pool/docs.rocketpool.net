...
このセクションでは、Smart Nodeをインストールして設定した後、Rocket Poolでステーキングするためにノードをプロビジョニングする方法の詳細について説明します。
ステーキングに関する多くの情報があるため長い内容となっていますが、**最初のMegapoolバリデーターを作成する前に各ガイドをよくお読みください！**
...
[Rocket Poolの起動](../starting-rp)では、各モードでSmart Nodeサービスを起動する方法と、ExecutionクライアントとConsensusクライアントの同期進行状況を確認する方法を説明します。

[新しいウォレットの作成](../wallet-init)では、初めてノードをセットアップする場合にSmart Nodeで新しいウォレットを作成するプロセスを説明します。
...
[MEV、MEV-Boost、MEV報酬](../mev.mdx)では、**Maximum-Extractable Value**（MEV）、ステーキングエコシステムにおけるその役割、そしてSmart Nodeを使用してお好みに応じて設定する方法について説明します。
