...
[プライマリ引き出しアドレス](../../node-staking/prepare-node#primary-withdrawal-addressの設定)とRPL引き出しアドレスの違いを理解することが重要です。デフォルトでは、RPL引き出しアドレスは未設定であり、プライマリ引き出しアドレスは、すべてのRPLチェックポイント報酬、ステーキングされたRPL、およびBeacon Chain ETHが送信される場所です。
