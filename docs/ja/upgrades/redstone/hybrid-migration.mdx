...
::: tip ヒント
SmartnodeにそれぞれのExecution ClientとConsensus Clientを維持させたいが、独自のValidator Client（たとえば、独自のソロステーキングバリデータキーが接続されている場合など）を制御し続けたい場合は、まさにそれを実現する[Reverse Hybridモード](../../node-staking/advanced-config#外部バリデータークライアントをsmartnodeに接続できるようにする)を検討することをお勧めします！
:::
