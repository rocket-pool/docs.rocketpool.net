...
Devam etmeden önce lütfen [bir teklifin yaşam döngüsünü](./pdao#bir-pdao-teklifinin-yaşam-döngüsü) okuyun. Tüm oylama dönemleri arasındaki farkları ve her dönemde alınabilecek eylemleri açıklayacaktır.
...
- `delegate` ile yanıt verirseniz, aynı işlem içinde oylamayı başlatarak ve [bir delege belirleyerek](./participate#oy-gücünü-devretme) biraz gaz tasarrufu yapma fırsatınız olacaktır.
...
Smart Node sürüm `1.13.8`, henüz bunu yapmamış düğümlerde oy gücünü otomatik olarak başlatmak üzere tasarlanmış yeni bir özellik olan **Auto Initialize Vote Power**'ı tanıtır. Otomatik olarak başlatılan oy gücü kendine devredilir.
Bu özellik, `rocketpool service config` çalıştırılarak ve **Smart Node and TX Fees** bölümüne gidilerek Smart Node ayarlarında yapılandırılabilir.
...
Teklif önerisinde bulunma ve itiraz etme için RPL kilitleme gereklidir. Varsayılan olarak, herhangi bir amaç için RPL kilitleme devre dışı bırakılacaktır. Düğüm operatörleri, düğümlerinden veya birincil çekim adreslerinden RPL'nin kilitlenmesini etkinleştirerek yönetişim faaliyetlerini gerçekleştirmeyi kabul edeceklerdir. Smart Node'da bu komutu kullanarak bunu yapabilirsiniz:
...
- Snapshot'a dahil edilmiş (ya [oylamayı başlatarak](./participate#oylamayı-başlatma) ya da Houston sonrası kaydolarak)
- En az bir minipool'a sahip olmalı
- Sıfır olmayan oy gücüne sahip olmalı
- [RPL kilitlemeye](./participate#rpl-kilitlemeye-i̇zin-verme) izin vermiş olmalı
- Teklif bonusundan daha büyük bir RPL stake'ine (zaten kilitli olan RPL hariç) sahip olmalı

Teklifler, parametreleri değiştirmek ve protokol düzeyinde kod yürütmek için vardır! Bir teklif zincirde oluşturulmadan önce [yönetişim](./participate#yönetişim-süreci) süreci aracılığıyla tartışma ve konsensüs olmalıdır.
...
Bu komutların her biri, istediğiniz teklifi oluşturmak için size bir girdi listesi isteyecektir. Ayar teklifi komutları, bir işlem göndermek yerine bir ayar değişikliğini bir JSON dosyasına yazmak veya eklemek için `--to-json` seçeneğini kabul eder. Dosya hazır olduğunda, `rocketpool pdao propose submit-batch` ile gönderin.

Bu rehberde, bir örnek olarak güvenlik konseyine bir düğümü davet edeceğiz. Güvenlik konseyine bir düğümü davet etmek için şu komutu kullanırsınız:
...
Bu bir bloka dahil edildikten sonra, bir pDAO teklifi oluşturulacaktır! Teklif, oluşturulduğunda [oy gecikmesi süresine](./pdao#oy-gecikmesi-dönemi) girecektir.
...
<div className="p-3">
  <Tabs>
    <Tab label="Tekliflerin Listesini Görüntüleme">
      ...
      [itiraz edenler](/tr/pdao/pdao#i̇tiraz-süreci), teklif sahibi tarafından sağlanan
      ...
      teklif aktif oylama aşamalarına geçecektir. Yenilemek için [bir teklifin yaşam döngüsünü](./pdao#bir-pdao-teklifinin-yaşam-döngüsü) gözden geçirmekten çekinmeyin.
    </Tab>
