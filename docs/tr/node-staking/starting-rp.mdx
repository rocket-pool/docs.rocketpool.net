...
Bu noktada, Smart Node yığını, bir Yürütüm ve bir Fikir Birliği istemcisi dahil olmak üzere eksiksiz Rocket Pool altyapısına sahip olmalısınız.
...
<div className="p-3">
  <Tabs>
    <Tab label="Docker (Varsayılan) Modu">
      Artık Smart Node yüklü olduğuna göre, yığını başlatmak kolaydır.
      ...
      Eğer öyleyse, Smart Node yığını başarıyla başlatılmış ve şu anda çalışıyor demektir.
    ...
    <Tab label="Yerel Mod">
      Yerel modda, [Docker Olmadan Yerel Rocket Pool Düğümü Yapılandırma](./native) bölümünün bir parçası olarak Smart Node hizmetlerini zaten kurdunuz ve başlattınız.
...
İlk satır, Smart Node'unuzun Ethereum mainnet'i için mi yoksa Hoodi testnet'i için mi yapılandırıldığını söyleyecektir.
...
Artık Smart Node hizmetleri başlatıldığına göre, hatasız bir şekilde doğru çalıştıklarını kontrol etmek iyi bir fikirdir.
  ...
  <Tabs>
    <Tab label="Docker (Varsayılan) Modu">
      ...
      <p className="rspress-directive warning">
        ...
        Bu normaldir!
        Doğrulayıcı konteyneri, [Megapool Oluşturma (Doğrulayıcı)](/node-staking/megapools/create-megapool-validator) bölümünde ele alacağımız `rocketpool node deposit` komutu kullanılarak yeni bir doğrulayıcı oluşturulana kadar başarısız olacaktır.
    ...
    <Tab label="Yerel Mod">
      Yerel modda durum ve günlükleri kontrol etmenin en kolay yolu, Smart Node hizmetlerini ilk kurduğunuzda oluşturduğunuz günlük betiklerini kullanmaktır.
