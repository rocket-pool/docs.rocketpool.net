...
# Smart Node Stack'i Yapılandırma (Native Modu)

Bu bölümde, Docker kullanmayan **Native kurulum** kullanıyorsanız Smart Node'u yapılandırmanın çeşitli yöntemlerini ele alacağız.
...
- [Sihirbaz UI](#sihirbaz-ile-yapılandırma) ile - bu en kolay yoldur. Sadece birkaç temel soru sorar ve geri kalanı için iyi test edilmiş varsayılanları kullanır. İlk kez `rocketpool service config` komutunu çalıştırdığınızda size sunulacak olan budur.
- [Ayarlar Yöneticisi UI](#ayarlar-yöneticisi-ile-yapılandırma) ile - bu size Smart Node'un tüm ayarlarına erişim sağlar, böylece istediğiniz kadar her şeyi özelleştirebilirsiniz.
- [Komut Satırı](#komut-satırı-ile-yapılandırma) ile başsız olarak - bu, Smart Node'u başsız (etkileşimsiz) bir ortamda çalıştıran ve otomatik olarak yapılandırması gereken kişiler için bir seçenektir.
...
Smart Node'u yapılandırmak için yapılandırma komutunu çalıştırın:
...
::: tip NOT
Smart Node'u zaten yapılandırdıysanız, bunun yerine [Ayarlar Yöneticisi](#ayarlar-yöneticisi-ile-yapılandırma) ile karşılaşacaksınız.
...
Smart Node ile aynı makinede bir Execution client çalıştırmayı planlıyorsanız buraya `http://127.0.0.1:8545` girin (bu kılavuzun geri kalanında, bunu yapmayı planladığınızı varsayacağız).
...
Her Consensus client biraz farklı davrandığı için, Smart Node kendi davranışını buna göre uyarlayabilmesi için hangisini kullandığınızı bilmesi gerekir.
...
Consensus client'ınızın Beacon Node'unu Smart Node ile aynı makinede çalıştırmayı planlıyorsanız buraya `http://127.0.0.1:5052` girin (bu kılavuzun geri kalanında, bunu yapmayı planladığınızı varsayacağız).
...
Smart Node kurulumunuz için `/srv/rocketpool` dışında bir dizin kullanmaya karar vermediyseniz, bunları varsayılan seçenekler olarak bırakın.

### Yedek Client'lar

Smart Node stack'in 1.5.0 sürümünden itibaren, birincil client'larınız herhangi bir zamanda çevrimdışı olursa (örneğin Geth kullanıyorsanız ve onu budamanız gerekiyorsa) onların yerini alabilecek bir "yedek" Execution client ve Consensus client çifti sağlayabilirsiniz.
...
Smart Node v1.7.0 ile başlayarak, MEV artık _devre dışı bırakma seçeneği_ olduğundan, yapılandırması hakkında bir bildirim, bir sonraki ekranda gördüğünüz gibi ilk kurulumun bir parçası olarak sunulur:
...
Bu sorudan sonra, Smart Node yapılandırmasını kurmayı tamamladınız.
Aşağıdaki iletişim kutusunu göreceksiniz:

<img src={tui_native_finished} width="100%" height="auto" />

Kurulumunuzdan memnunsanız ve Smart Node'u başlatmaya hazırsanız, burada `Save and Exit`'e tıklayın.
...
Tüm ayarları gözden geçirmek ve Sihirbaza dahil edilmeyen birçok ek ayarı özelleştirmek istiyorsanız, `Review All Settings`'e tıklayın ve [sonraki bölüme](#ayarlar-yöneticisi-ile-yapılandırma) gidin.
...
1. **Kategori Listesi**, Native modu ile ilgili Smart Node stack'inin her kategorisi için ayarları detaylandırmanıza olanak tanır
...
3. **Yapılandırma Sihirbazını Aç** düğmesi, bunun yerine o arayüzü tercih ederseniz sizi [Kurulum Sihirbazı](#sihirbaz-ile-yapılandırma)'na götürecektir
...
Örneğin, işte **Smart Node ve TX Ücretleri** kategorisi için ekran:
...
Smart Node'u Terminal UI ile etkileşime giremeyeceğiniz başsız bir ortamda kullanıyorsanız, bunun yerine node'unuzu komut satırı aracılığıyla yapılandırabilirsiniz.
