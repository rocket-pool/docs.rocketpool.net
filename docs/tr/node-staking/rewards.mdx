...
Merkle Ağacı bir JSON dosyasına yerleştirilir ve [bakımını yaptığımız bir GitHub deposunda](https://github.com/rocket-pool/rewards-trees/) barındırılır. Smart Node ayrıca ödül aralığı **olaylarını** bu GitHub deposundan yükleyebilir, böylece agresif geçmiş sona erdirme eski Execution katmanı günlüklerini sildikten sonra bile talep edebilirsiniz.
...
- Birincil Execution istemciniz bir arşiv node'u değilse, `Archive-Mode EC URL` kutusuna ayrı bir arşiv node'u için bir URL ekleyebilirsiniz.
  ...
  - Archive EC yalnızca ağaç oluşturma için kullanılacaktır; diğer Smart Node görevleri için kullanılmayacaktır.
...
Artık Smart Node'unuz, Oracle DAO'dan tamamen bağımsız olarak, yalnızca Execution ve Consensus istemcileriniz tarafından sağlanan verileri kullanarak ağacı oluşturacaktır.
...
Smoothing Pool'a katılmadıysanız, blok tekliflerinden gelen ödüllerinizin Execution-layer kısmı (işlem ücretleri ve MEV dahil) bunun yerine node'unuzun [Fee Distributor](./fee-distrib-sp#ücret-dağıtıcıları-ve-smoothing-pool) sözleşmesine gönderilecektir.
