...
gerçekleştirecektir. `stake` işlemi, `Prelaunch` doğrulayıcınızı `Staking` doğrulayıcısına dönüştürecektir. Bu noktada `Staking` doğrulayıcınız: - Beacon Chain'e 32 ETH yatırılmış - Bir doğrulayıcı
dizin numarası atanmış - Beacon Chain'de etkinleştirme bekliyor
