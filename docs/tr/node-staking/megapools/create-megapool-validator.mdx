...
Doğrulayıcınızı [ekspres kuyruktan çıkarmayı](./create-megapool-validator#rocket-pool-deposit-kuyruğundan-doğrulayıcı-çıkarma) seçerseniz, düğümünüze bir ekspres kuyruk bileti iade edilecektir.
...
Bir prelaunch doğrulayıcısı 365 gün içinde stake yapamazsa, doğrulayıcı çözülür. Prelaunch süreci sırasında Beacon Chain'e gönderilen 1 ETH (4 ETH bond'dan), **geri alınamaz**.
Düğüm operatörü, bond'undan kalan 3 ETH'yi (0,05 ETH çözülme cezası borç olarak düşülerek) megapool'unda geri ödeme olarak alır ve net geri ödeme 2,95 ETH'dir.
Bu, `rocketpool megapool claim` veya `rocketpool claims claim-all` kullanılarak talep edilebilir.

[Başarılı bir stake'i nasıl onaylayacağınız hakkında daha fazla bilgi edinin](./create-megapool-validator#başarılı-bir-stakei-onaylama)
...
Bir sonraki istem, [Rocket Pool deposit kuyruğunun](/tr/node-staking/megapools/create-megapool-validator#rocket-pool-deposit-kuyruğu-ve-ekspres-kuyruk) durumunu gösterecektir.
...
Doğrulayıcınıza [Rocket Pool deposit kuyruğundan](/tr/node-staking/megapools/create-megapool-validator#rocket-pool-deposit-kuyruğu-ve-ekspres-kuyruk) ETH atandığında, `Prelaunch` durumuna taşınacaktır. Bu noktada, megapool bakiyenizden 1 ETH Beacon Chain'e yatırılacaktır. Doğrulayıcınızın pubkey'i de Beacon Chain'e kaydedilecektir; bu, `Prelaunch` doğrulayıcınızın durumunu https://beaconcha.in/ (veya testnet kullanıyorsanız https://hoodi.beaconcha.in/) gibi bir gezginde görüntüleyebileceğiniz anlamına gelir.
...
`Prelaunch` doğrulayıcınız Beacon Chain tarafından işlenip başlangıç 1 ETH yatırımı alındıktan sonra. Düğümünüz, tam 32 ETH Beacon Chain yatırımını tamamlamak için otomatik olarak bir `stake` işlemi
gerçekleştirecektir. `stake` işlemi, `Prelaunch` doğrulayıcınızı `Staking` doğrulayıcısına dönüştürecektir. Bu noktada `Staking` doğrulayıcınız:
