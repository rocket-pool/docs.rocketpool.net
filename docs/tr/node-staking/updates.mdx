...
## Smart Node Yığınını Güncelleme

Zaman zaman, Rocket Pool Smart Node yığınının yeni bir sürümünü yayınlayacaktır.
...
::: tip İPUCU
Smart Node yükseltmesini tamamladığınızda, Grafana panosu hala bir güncellemenin mevcut olduğunu gösterecektir.
...
Yükseltme adımları düğümünüzün hangi modu kullandığına bağlıdır. Smart Node'u güncellemenin en iyi yolu CLI'nin yerleşik güncelleme komutunu kullanmaktır. Aşağıdaki seçeneklerden birini seçin:

<div className="p-3">
  <Tabs>
    <Tab label="CLI Güncelleme Komutu (Docker Modu - Linux/macOS)">

      Yerleşik güncelleme komutunu kullanarak Smart Node'u güncellemek için şunu çalıştırın:

      ```shell
      rocketpool update
      ```
      Bu komut şunları yapar:
      1. En son CLI ikili dosyasını indirir ve doğrular
      2. İsteğe bağlı olarak servisi otomatik olarak durdurur, yükseltir ve yeniden başlatır

      Güncellemeyi tamamlamak için etkileşimli istemleri izleyin.
      ```
      ...
      Ardından Smart Node en son CLI ikili dosyasını indirip doğrular:
      ...
      y
      ```
      Bu noktada, Smart Node'un kalan adımlarda size rehberlik etmesi için tüm etkileşimli istemlerde `y` seçebilirsiniz.
      İşlem bitince, yükseltmenin başarılı olduğunu doğrulayan bir mesaj görürsünüz:
      ```
    ...
    <Tab label="Manuel Güncelleme - Linux (Docker veya Hibrit Mod)">
      ...
      Yeni Smart Node CLI'yi indirin:
      ...
      Son olarak, CLI ve Smart Node yığınının her ikisinin de güncel olduğundan emin olmak için sürümü kontrol edin:
    ...
    <Tab label="Manuel Güncelleme - Linux (Yerel Mod)">
      <p className="rspress-directive warning">
        <p className="rspress-directive-title">NOT</p>
        Bu yalnızca Smart Node yığınının kendisini güncelleyecektir - Yürütüm veya Fikir Birliği istemcilerinizi **güncellemeyecektir**.
      ...
      Yeni Smart Node CLI'yi indirin ve geri getirmeniz gerekebilecek eski CLI ve daemon ikili dosyalarını yedekleyin:
    ...
    <Tab label="Manuel Güncelleme - macOS (Docker veya Hibrit Mod)">
      Rocket Pool hizmetlerini durdurun:
...
Her yeni Smart Node sürümü, Execution, Consensus ve Smart Node konteynerleri için sabitlenmiş Docker imaj digest'leriyle birlikte gelir (`~/.rocketpool/mainnet.env` içinde `repo:tag@sha256:…`). `rocketpool service install` bu resmi sabitleme dosyalarının üzerine yazar. Bazı durumlarda, yeni bir Smart Node sürümünü beklemeden _önce_ farklı bir istemci imajı çalıştırmak isteyebilirsiniz. Bu bölüm bunu nasıl yapacağınızı gösterir.
...
<div className="p-3">
  <Tabs>
    <Tab label="Docker Modu">
      ...
      Smart Node artık istemci imajlarını ve Smart Node imajlarını digest ile sabitliyor.
      TUI'de **Container Tag** değerini değiştirmek, bir sonraki Smart Node sürümünden önce farklı bir etiket veya digest çalıştırmanıza yine de olanak tanır.
      `rocketpool service install` veya `rocketpool update` sonrasında `mainnet.env` içindeki resmi sabitlemeler değiştirilir; TUI'de yaptığınız geçersiz kılma, siz temizleyene kadar yine de geçerli kalır.
      ...
      **Beacon Node Container Tag** ayarını değiştirin:
      ...
      Smart Node, etkilenen tüm konteynerleri otomatik olarak yeniden başlatmayı teklif edecektir.

      ::: tip NOT
      Resmi sabitlemeler `~/.rocketpool/mainnet.env` dosyasında bulunur. Kalıcı değişiklikler için bu dosyayı düzenlemeyin; kurulum dosyanın üzerine yazar. İsteğe bağlı ağ bazlı geçersiz kılmalar `~/.rocketpool/testnet.env` veya `~/.rocketpool/devnet.env` dosyalarına yazılır (yalnızca belirlediğiniz anahtarlar mainnet sabitlemesinin yerine geçer). Ayarlar Yöneticisi'nde belirlediğiniz bir **Container Tag**, o servis için sabitlemeyi geçersiz kılar. Resmi digest'i korumak için varsayılan değerde bırakın.
      :::
    </Tab>
    <Tab label="Yerel Mod">
