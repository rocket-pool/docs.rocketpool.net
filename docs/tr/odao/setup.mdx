...
- Smart Node CLI
- Smart Node API daemon
- Smart Node `node` daemon
- Smart Node `watchtower` daemon
...
Bu bileşenlerin her biri standart kurulumda Docker container'ları olarak, "Native Mode" kurulumunda `systemd` servisleri olarak veya Smart Node'un ilgili API rotaları aracılığıyla bağlanabileceği harici olarak yönetilen varlıklar olarak çalıştırılabilir.
...
Node kaydı, iyi bir güvenlik duruşu oluşturma, node'un sağlığını ve performansını izleme ve bir güncellemeden sonra Smart Node'u güncelleme gibi diğer adımlar hala sizin için geçerlidir ve devam etmeden önce bunları dikkatle incelemelisiniz.
...
<div className="p-3">
  <Tabs>
    ...
    <Tab label="Doğrudan Config Dosyası">
      Smart Node dizininizdeki (`~/.rocketpool` varsayılan olarak) `user-settings.yml` dosyasını bir metin düzenleyicide açın.
...
## Oracle DAO Smart Node Komutları

Smart Node CLI'nin `odao` komut grubu, Oracle DAO contract'ları ve görevleriyle zincir üstünde etkileşim kurmak için kullanılır:
