...
Smart Node version `1.13.8` introduces a new feature **Auto Initialize Vote Power**, which is designed to automatically
initialize voting power on nodes who have not yet done so. Automatically initialized voting power is self-delegated.
This feature can be configured in the Smart Node settings by running `rocketpool service config` and navigating to the \*
\*Smart Node and TX Fees\*\* section.
...
withdrawal address. You can do so using this command in the Smart Node:
...
Each of these commands will prompt you with a list of inputs to create your desired proposal. Setting propose commands accept `--to-json` to write or append a setting change to a JSON file instead of submitting a transaction. When the file is ready, submit it with `rocketpool pdao propose submit-batch`.
