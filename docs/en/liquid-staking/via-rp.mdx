...
::: danger WARNING
The CoW Swap route is not part of the Rocket Pool protocol. Quotes & solvers are determined by **third party** systems that are outside the control or visibility of the Rocket Pool protocol. Users should note that using CoW Swap may expose them to additional risks such as slippage, price impact, solvers that route funds incorrectly, etc. For fine grain control
we highly recommend that you use CoW Swap directly.
:::
