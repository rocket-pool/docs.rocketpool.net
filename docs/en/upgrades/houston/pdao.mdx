...
Any node can participate in tracking and verifying the correctness of proposals. To opt into this responsibility, use the command `rocketpool service config`, navigate to the **Smart Node and TX Fee Settings** menu, and check the box `Enable PDAO Proposal Checker`.
