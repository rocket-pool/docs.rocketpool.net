...
migrate existing RPL stake from legacy minipools, claim your rewards, and withdraw your RPL when needed. It'll also go over the mechanics of how 
ETH protocol revnue is distributed to RPL stakers via voter share rewards. 

::: tip NOTE

Moving forward, legacy RPL previously staked on minipools continue earning inflation rewards and contribute to voting power. 
RPL staked to your megapool contributes to your share of voter rewards
:::


## What's Changed in Saturn 1

In Saturn 1, staking RPL to your megapool provides several benefits:

1. **ETH Revenue Share**: Vote-eligible RPL earns you a proportional share of pfrotocol ETH revenue through the voter share mechanism
...
```
Maximum vote-eligible RPL = (Total bonded ETH × 1.5) / RPL price
```
...
::: tip IMPORTANT
The voting system treats all staked RPL equally when calculating your vote weight. 
...
So in the example above the node's `total stake of 5400.964614 RPL` is vote-eligible as it's falls within the 150% bonded ETH max. The node's `4383.275814 megapool staked RPL`
...
| **pDAO Treasury to the protocol treasury** | 0% | Yes |
...
## How Voter Share is Distributed

The voter share (9% of protocol revenue) is distributed proportionally among all node operators based on their share of total vote-eligible RPL staked. Every 28 days, the oracle
DAO will take a snapshot of the total ETH accumulated in the voter share pool as well a record of each node operator's share based on their balance of vote-eligible megapool staked RPL. 
...
There's no limit on how much megapool staked RPL you can unstake, which means you can unstake and withdraw all of it any any time. 
...
- You may initiate new withdrawal during the 28 day period, but all your unstaking RPL will refresh to 28 days in unstaking. It's recommended 
...
- If you're in the process of migrating legacy RPL, select option the third option `request to unstake legacy RPL`. 
...
Your node has 1017.688800 RPL on its legacy stake (previously associated to minipools) and 3383.275814 RPL staked on its megapool.
Your node currently has 0.000000 RPL locked on pDAO proposals.
Your node's RPL withdrawal address is <RPL-withdrawal-addressss>.
...
learn how you can apply updates, monitor your node, and brush up on how to keep your node in tip top shape throughout it's life. 
