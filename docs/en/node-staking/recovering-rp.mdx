...
If you already have a wallet you'd like to use for your node, or if you're recovering a wallet you already created with the Smart Node earlier, this guide will help you through the import / recovery process.

Please select the appropriate option from the sections below.

## Recovering a Smart Node Wallet

If you generated your node wallet using the Smart Node and are simply recovering it on a new machine, the process is quite simple.
Make sure you've already installed the Smart Node software, and then simply run the following command once it's installed:
...
If you do not specify this flag, the Smart Node will attempt to recover the validator keys for your minipools or megapool validators. That step needs synced Execution and Beacon clients.

With `-k`, recovering the **wallet only** does not need to wait for synced clients. You can do that while they are still syncing.
:::

By default, the Smart Node generates a random password to encrypt the recovered wallet.
To choose your own password instead, pass `-p` / `--password`.

::: tip NOTE
You do not need to know or remember this password. The **24-word node mnemonic** is enough to fully recover your wallet.
:::
...
If you have an address you'd like to use for a node, but _didn't_ create it with the Smart Node (e.g., you created it with MetaMask or a hardware wallet), then this is the section to follow.
...
This capability is only available with Smart Node **v1.4.3 or higher**.
...
If your address has already been registered as a Rocket Pool node wallet (such as via a service like **Allnodes**) and already has active minipools or megapool validators, and you'd like to import them into the Smart Node stack along with your address, you will need to provide the private keystore files for each of their corresponding validators.
...
<div className="p-3">
  <Tabs>
    <Tab label="Docker and Hybrid Mode">
      First, ensure you've started the Smart Node with `rocketpool service start`.
    ...
    <Tab label="Native Mode">
      Create a folder called `custom-keys` inside of your Smart Node's `data` folder (for example, `/srv/rocketpool/data/custom-keys`).
    </Tab>
...
The Smart Node will look in this directory for any keystore files you have placed here in the next steps.
...
You **cannot** use this process to import other validator keys, such as for solo staking validators, into the Validator client managed by the Smart Node stack.
...
The Smart Node will automatically search through the most popular derivation paths (e.g. the ones used by Ledger Live, MyEtherWallet, Trezor, and the Smart Node stack) and address indicies to find the settings that correspond to the address you provided.
...
::: danger WARNING
If your address is a Rocket Pool node that is _not_ managed by your own self-hosted Smart Node (e.g. hosted by an external service like **Allnodes**), **IT IS CRITICAL** that you coordinate with the service running your node **before you begin this import process** and inform them that you would like to migrate to your own self-hosted system.
...
The Smart Node will require you to confirm that you have done this before allowing you to proceed with the import process.
:::
...
When you run this command, the Smart Node generates a random password to encrypt your imported node wallet unless you pass `-p` / `--password`.
...
Once you've entered all of this information, the Smart Node will recover your address and (if not disabled) the custom validator keys for your minipools and megapool validators:
...
The Smart Node will have imported them already and assigned randomized passwords to them, so those keystore files are no longer necessary.
...
The Smart Node will build this temporary file only during the recover process, and will remove it automatically regardless of whether or not the recovery process succeeded; if for any reason it failed to remove it, it will alert you in the console output.
