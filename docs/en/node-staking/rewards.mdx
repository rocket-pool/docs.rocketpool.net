...
The Merkle Tree is built into a JSON file and hosted on [a GitHub repository that we maintain](https://github.com/rocket-pool/rewards-trees/). The Smart Node can also load rewards-interval **events** from that GitHub repo, so you can still claim after aggressive history expiry has dropped old Execution-layer logs.
...
- If your primary Execution client is not an archive node, you can add a URL for a separate archive node in the `Archive-Mode EC URL` box.
  ...
  - The Archive EC will only be used for tree generation; it will not be used for any other Smart Node duties.
...
Now your Smart Node will build the tree completely independently from the Oracle DAO, using only the data provided by your Execution and Consensus clients.
