...
The Merkle Tree is built into a JSON file and hosted on the [InterPlanetary File System (IPFS)](https://en.wikipedia.org/wiki/InterPlanetary_File_System) and mirrored on [a GitHub repository that we maintain](https://github.com/rocket-pool/rewards-trees/). The Smartnode can also load rewards-interval **events** from that GitHub repo, so you can still claim after aggressive history expiry has dropped old Execution-layer logs.
