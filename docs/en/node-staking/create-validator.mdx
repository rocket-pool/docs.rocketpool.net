...
in order to stake from it. You can do this via the following Smart Node command:
...
If you ever want to remove an address from your stake-on-behalf whitelist, you may do so with the following Smart Node command:
...
However, the Smart Node provides the ability to search for a custom **vanity address** for the minipool.
