...
The `pdao` group is for Protocol DAO voting and proposals. Day-to-day voting is `rocketpool pdao proposals vote`. To change **several** Protocol DAO settings in a single proposal, write them to a JSON file with `--to-json` on the setting propose commands, then submit with:
...
Do not use `migrate-eth1` — that string is not the command name.

For the full downtime / fallback notes, see [Pruning the Execution Client](./pruning).
