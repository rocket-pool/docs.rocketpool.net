...
The `pdao` group is for Protocol DAO voting and proposals. Voting is done with `rocketpool pdao proposals vote`. To change **several** Protocol DAO settings in a single proposal, write them to a JSON file using the `--to-json` flag on the setting propose commands, then submit with:

```shell
rocketpool pdao propose submit-batch
```

Governance walkthrough: [Participating in on-chain pDAO Proposals](/pdao/participate).
...
Geth operators who still have a Pebble v1 database can migrate it in place with:

```shell
rocketpool service migrate-geth
```

This **shuts Geth down**, migrates the database from Pebble v1 to Pebble v2, then restarts it. Plan for Execution-client downtime (a [fallback node](./fallback) keeps you attesting). Confirm with `y`, or pass `--yes` to skip the prompt.
...
To replace the CLI binary (and optionally stop, upgrade, and restart the stack), run:

```shell
rocketpool update
```

This is the usual way to update a Docker or Hybrid node. The walkthrough lives on [Checking for Updates](./updates).

## Node Commands

The `node` group involves operations on your Rocket Pool node.
We'll cover these more in-depth in the next section where we create a megapool validator, but it may be helpful to see them all at a glance.
...
::: warning NOTE
The `set-withdrawal-creds` and `import-key` commands only apply to minipools that were created from solo validator migrations before Saturn 1.
[Solo validator migration](./solo-staker-migration) is no longer possible, and the corresponding `create-vacant-minipool` and `promote` commands have been removed from the CLI.
:::
...
The Smart Node uses a priority fee of 2 gwei by default, so a value of `2.2` is usually sufficient for an override.
...
The Smart Node stack will automatically check to make sure that the `nonce` you have provided is valid (it refers to a pending transaction) before sending it and wasting your gas accidentally.
...
In the next section, we'll walk through how to create a megapool validator and start validating on the Beacon Chain.
