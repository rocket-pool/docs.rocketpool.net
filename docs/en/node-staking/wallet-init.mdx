...
If the Smart Node services are up and running, the next step is to set up an Ethereum wallet for your node.
...
<div className="p-3">
  <Tabs>
    <Tab label="Creating a New Wallet">
      ...
      The Smart Node CLI provides a way to do this easily:

      ```shell
      rocketpool wallet init
      ```

      By default, the Smart Node generates a random password to encrypt your wallet's private key (stored in the password file below).
    ...
    <Tab label="Recovering or Importing an Existing Wallet">
      ...
      If you already have a node wallet that you created in a previous installation of the Smart Node, or if you want to import an address that you created elsewhere (such as with **Allnodes**), please follow the instructions in the [Importing an Existing Wallet for your Node](./recovering-rp) guide.
