...
## Updating the Smart Node Stack

Occasionally, Rocket Pool will release a new version of the Smart Node stack.
...
::: tip TIP
When you have completed the Smart Node upgrade, the Grafana dashboard will still indicate there is still an update available.
...
<div className="p-3">
  <Tabs>
    ...
    <Tab label="Manual Update - Linux (Docker or Hybrid Mode)">
      ...
      Download the new Smart Node CLI:
      ...
      Finally, check the version to make sure the CLI and Smart Node stack are both up to date:
    ...
    <Tab label="Manual Update - Linux (Native Mode)">
      <p className="rspress-directive warning">
        ...
        This will only update the Smart Node stack itself - it **will not** update your Execution or Consensus clients.
      ...
      Download the new Smart Node CLI and back up the old CLI and daemon binaries if you need to bring them back:
...
Each new Smart Node release ships pinned Docker image digests for the Execution, Consensus, and Smart Node containers (`repo:tag@sha256:…` in `~/.rocketpool/mainnet.env`). `rocketpool service install` overwrites those official pin files. In some cases you might want to run a different client image _before_ waiting for a new Smart Node release. This section shows you how.
...
<div className="p-3">
  <Tabs>
    <Tab label="Docker Mode">
      ...
      Smart Node now pins client and Smart Node images by digest.
      Changing **Container Tag** in the TUI still lets you run a different tag or digest before the next Smart Node release.
      After `rocketpool service install` or `rocketpool update`, the official pins in `mainnet.env` are replaced; a TUI override still wins until you clear it.
      ...
      The Smart Node will offer to restart all of the affected containers automatically.

      ::: tip NOTE
      Official pins live in `~/.rocketpool/mainnet.env`. Do not edit that file for lasting changes; install overwrites it. Optional per-network overrides go in `~/.rocketpool/testnet.env` or `~/.rocketpool/devnet.env` (only keys you set replace the mainnet pin). A **Container Tag** you set in the Settings Manager overrides the pin for that service. Leave it at the default to keep the official digest.
      :::
    </Tab>
