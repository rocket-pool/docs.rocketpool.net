...
Each new Smart Node release ships pinned Docker image digests for the Execution, Consensus, and Smart Node containers (`repo:tag@sha256:…` in `~/.rocketpool/mainnet.env`). `rocketpool service install` overwrites those official pin files. In some cases you might want to run a different client image *before* waiting for a new Smart Node release. This section shows you how.
