...
1. Besu does _not_ need Geth-style offline pruning; Rolling History Expiry (the default Pruning Mode) drops bodies and receipts older than about one year while the client stays online
...
In the Settings Manager, Erigon exposes a **Torrent Port** (default `42069`, TCP and UDP) in addition to the usual Execution P2P port (`30303`). Forward both if you want snapshot sync and peering to work well from outside your LAN.
...
| Reth       | Full | ~14 GB    |
| Erigon | Full | See vendor docs; disk is the main constraint |
