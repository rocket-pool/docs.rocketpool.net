...
Rocket Pool's Smart Node installer can transform your machine into a full Ethereum node, as it requires both Execution and Consensus clients in order to operate properly.
...
For users that would like to get up and running quickly, the Smart Node installer provides a `Random Client` option which may be the best choice.
...
Rocket Pool supports five Execution clients: **Geth**, **Besu**, **Nethermind**, **Reth**, and **Erigon**.
...
[Erigon](https://erigon.tech/) is an Execution client written in [Go](https://go.dev/). It uses a staged-sync architecture and can pull snapshots over BitTorrent on first start, which makes initial sync comparatively fast if the torrent port is reachable.

In the Settings Manager, Erigon exposes a **Torrent Port** (default `42069`, TCP and UDP) in addition to the usual P2P port (`30303`). Forward both if you want snapshot sync and peering to work well from outside your LAN.

Erigon supports **Rolling History Expiry** (drop bodies and receipts older than about one year). It does **not** support pre-merge-only History Expiry; if you pick that mode, it keeps remaining block history with full-mode state pruning. If you run it as a **full node**, it keeps receipts.

Like the other clients, RAM needs are flexible and disk quality matters. Open the torrent and P2P ports in your router (and host firewall) the same way you would for Geth's `30303`.
