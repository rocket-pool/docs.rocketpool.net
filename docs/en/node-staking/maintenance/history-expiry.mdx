...
Execution clients can drop old block bodies and receipts so the chain data stays a manageable size. The Smart Node **Pruning Mode** setting (Execution Client → Pruning Mode) controls this.
...
- **History Expiry** — drop _pre-merge_ bodies and receipts only (partial [EIP-4444](https://eips.ethereum.org/EIPS/eip-4444) expiry). Erigon does not support pre-merge-only expiry.
