...
**Rolling History Expiry** is the default. Nethermind, Besu, Reth, and Erigon drop bodies and receipts older than about one year. Geth does not implement that mode yet and uses pre-Prague history expiry instead (older pre-merge / pre-Prague bodies and receipts).
...
- **Archive** — keep history and historical state; much more disk.
...
- **Nethermind** — resync after changing the mode (`rocketpool service resync-eth1`).
...
- **Besu** — stays online; no extra history-expiry flags.
- **Reth** — stays online. On **Mainnet** with Rolling History Expiry, Reth downloads the official rolling-history snapshot the first time it starts.
- **Erigon** — Rolling History Expiry drops bodies/receipts older than about one year.
