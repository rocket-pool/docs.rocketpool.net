...
Dieser Abschnitt behandelt die Details, wie Sie Ihren Node für das Staking mit Rocket Pool bereitstellen, nachdem Sie den Smart Node installiert und konfiguriert haben.
Es ist ein langer Abschnitt, da es viele Informationen rund um das Staking zu behandeln gibt, also **lesen Sie bitte jeden Leitfaden durch, bevor Sie Ihren ersten Megapool-Validator erstellen!**
...
- Lassen Sie den Smart Node darauf [installiert](../installing/overview) und [konfiguriert](../config/overview)

## Leitfäden

[Starting Rocket Pool](../starting-rp) zeigt Ihnen, wie Sie die Smart Node-Services für jeden Modus starten und wie Sie den Synchronisationsfortschritt Ihrer Execution- und Consensus-Clients überprüfen.

[Creating a New Wallet](../wallet-init) führt Sie durch den Prozess der Erstellung eines brandneuen Wallets mit dem Smart Node, wenn Sie zum ersten Mal einen Node einrichten.
...
[MEV, MEV-Boost, and MEV Rewards](../mev.mdx) erklärt **Maximum-Extractable Value** (MEV), seine Rolle im Staking-Ökosystem und wie Sie es nach Ihren Wünschen mit dem Smart Node konfigurieren können.
