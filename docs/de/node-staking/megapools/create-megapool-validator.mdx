...
Nachdem dein `Prelaunch`-Validator von der Beacon Chain verarbeitet und der erste 1 ETH-Deposit gutgeschrieben wurde, führt dein Node automatisch eine
`stake`-Transaktion durch, um den vollständigen 32 ETH Beacon Chain-Deposit abzuschließen. Die `stake`-Transaktion versetzt deinen `Prelaunch`-Validator
in einen `Staking`-Validator. An diesem Punkt hat dein `Staking`-Validator:
- 32 ETH in die Beacon Chain eingezahlt
- Eine Validator-Indexnummer zugewiesen bekommen
- Ausstehende Aktivierung auf der Beacon Chain
