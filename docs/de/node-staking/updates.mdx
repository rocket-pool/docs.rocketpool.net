...
## Aktualisierung des Smart Node Stacks

Gelegentlich wird Rocket Pool eine neue Version des Smart Node Stacks veröffentlichen.
...
::: tip TIPP
Wenn Sie das Smart Node-Upgrade abgeschlossen haben, wird das Grafana-Dashboard weiterhin anzeigen, dass ein Update verfügbar ist.
...
Die Schritte zum Upgrade hängen davon ab, welchen Modus Ihr Node verwendet. Am besten aktualisieren Sie den Smart Node mit dem integrierten Update-Befehl der CLI. Wählen Sie aus den folgenden Optionen:

<div className="p-3">
  <Tabs>
    <Tab label="CLI-Update-Befehl (Docker-Modus - Linux/macOS)">

      Um den Smart Node mit dem integrierten Update-Befehl zu aktualisieren, führen Sie aus:

      ```shell
      rocketpool update
      ```
      Dieser Befehl wird:
      1. Die neueste CLI-Binärdatei herunterladen und verifizieren
      2. Optional den Dienst automatisch stoppen, aktualisieren und neu starten

      Folgen Sie den interaktiven Abfragen, um das Update abzuschließen.
      ```
      ...
      Anschließend lädt der Smart Node die neueste CLI-Binärdatei herunter und verifiziert sie:
      ...
      y
      ```
      An dieser Stelle können Sie bei allen interaktiven Abfragen `y` wählen, damit der Smart Node Sie durch die restlichen Schritte führt.
      Wenn Sie fertig sind, sehen Sie eine Meldung, die das erfolgreiche Upgrade bestätigt:
      ```
    ...
    <Tab label="Manuelles Update - Linux (Docker oder Hybrid-Modus)">
      ...
      Laden Sie die neue Smart Node CLI herunter:
      ...
      Überprüfen Sie schließlich die Version, um sicherzustellen, dass sowohl CLI als auch Smart Node Stack auf dem neuesten Stand sind:
    ...
    <Tab label="Manuelles Update - Linux (Native-Modus)">
      <p className="rspress-directive warning">
        <p className="rspress-directive-title">HINWEIS</p>
        Dies aktualisiert nur den Smart Node Stack selbst - es aktualisiert **nicht** Ihre Execution oder Consensus Clients.
      ...
      Laden Sie die neue Smart Node CLI herunter und sichern Sie die alten CLI- und Daemon-Binaries, falls Sie sie zurückbringen müssen:
    ...
    <Tab label="Manuelles Update - macOS (Docker oder Hybrid-Modus)">
      Stoppen Sie die Rocket Pool Dienste:
...
Jede neue Smart Node-Version liefert gepinnte Docker-Image-Digests für die Execution-, Consensus- und Smart Node-Container aus (`repo:tag@sha256:…` in `~/.rocketpool/mainnet.env`). `rocketpool service install` überschreibt diese offiziellen Pin-Dateien. In einigen Fällen möchten Sie möglicherweise ein anderes Client-Image ausführen, _bevor_ Sie auf eine neue Smart Node-Version warten. Dieser Abschnitt zeigt Ihnen, wie das geht.
...
<div className="p-3">
  <Tabs>
    <Tab label="Docker-Modus">
      ...
      Der Smart Node pinnt Client- und Smart Node-Images jetzt per Digest.
      Wenn Sie **Container Tag** in der TUI ändern, können Sie weiterhin einen anderen Tag oder Digest ausführen, bevor die nächste Smart Node-Version erscheint.
      Nach `rocketpool service install` oder `rocketpool update` werden die offiziellen Pins in `mainnet.env` ersetzt; eine Überschreibung aus der TUI hat trotzdem Vorrang, bis Sie sie entfernen.
      ...
      Der Smart Node wird anbieten, alle betroffenen Container automatisch neu zu starten.

      ::: tip HINWEIS
      Die offiziellen Pins stehen in `~/.rocketpool/mainnet.env`. Bearbeiten Sie diese Datei nicht für dauerhafte Änderungen; die Installation überschreibt sie. Optionale netzwerkspezifische Überschreibungen gehören in `~/.rocketpool/testnet.env` oder `~/.rocketpool/devnet.env` (nur die von Ihnen gesetzten Schlüssel ersetzen den Mainnet-Pin). Ein **Container Tag**, den Sie im Settings Manager setzen, überschreibt den Pin für diesen Service. Belassen Sie ihn beim Standardwert, um den offiziellen Digest zu behalten.
      :::
    </Tab>
