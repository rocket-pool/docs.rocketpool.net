...
Wenn Sie sich während des Smart Node-Konfigurationsprozesses dafür entscheiden, Metriken zu aktivieren, fügt Ihr Node die folgenden Prozesse hinzu:
...
Die Standardkonfiguration erstellt Docker-Container mit all diesen Diensten, die neben den restlichen Docker-Containern der Smart Node leben.
...
<div className="p-3">
  <Tabs>
    <Tab label="Docker">
      ...
      Führen Sie zunächst den Smart Node-Konfigurationsbefehl erneut aus:
      ...
      Speichern und beenden Sie, und die Smart Node startet die Prometheus-, Node Exporter- und Grafana-Docker-Container für Sie.
      ...
      Während dieser Überprüfung vergleicht es auch Ihre installierte Rocket Pool Smart Node-Version mit der neuesten Version und informiert Sie, wenn eine neue Version verfügbar ist.
...
Beziehen Sie sich auf die nachstehenden Tabs für Ihren Smart Node-Installationsmodus.
...
Wenn Sie nach der Dashboard-ID im Feld **Import via grafana.com** gefragt werden, geben Sie `24900` ein oder verwenden Sie die vollständige URL ([https://grafana.com/grafana/dashboards/24900-rocket-pool-dashboard-v1-4-0/](https://grafana.com/grafana/dashboards/24900-rocket-pool-dashboard-v1-4-0/)) und klicken Sie auf die Schaltfläche **Load**.
...
::: tip HINWEIS
Smart Node v1.23.0 liefert **Rocket Pool Dashboard v1.4.2** mit seinem Metrics-Stack aus. Diese Revision ergänzt node-bezogene Minipool- und Megapool-Validator-Zahlen nach Status und entfernt netzwerkweite Megapool-Status-Panels, die von entfernten Supply-Collector-Metriken abhingen. Wenn Ihr Grafana bereits ein älteres Dashboard importiert hat, importieren Sie es erneut (oder laden Sie das mit Smart Node bereitgestellte JSON), damit diese Panels zu den von Ihrem Node exportierten Metriken passen.
:::
...
Sie werden in einer zukünftigen Version der Smart Node, die historische Belohnungen ordnungsgemäß verfolgen kann, wieder aktiviert.
:::
...
Wenn Sie Dockers Standardspeicherort während Ihrer Smart Node-Installation nicht auf ein anderes Laufwerk geändert haben, ist das Laufwerk, das Sie verfolgen möchten, dasjenige, auf dem Ihr Betriebssystem installiert ist.
