...
Ab **Version 1.5.0** des Smart Node-Stacks können Sie ein "Fallback"-Execution-Client- und Consensus-Client-Paar angeben, das für Ihre primären Clients übernehmen kann, falls diese jemals offline gehen (z.B. weil [Sie Geth verwenden und es prunen müssen](./pruning)).
...
Sobald die primären Clients Ihrer Node wieder online sind, werden der Smart Node und Ihr Validator-Client automatisch zu ihnen zurückwechseln.
...
Dieses Beispiel zeigt Ihnen, wie Sie einen zweiten Smart Node auf einer anderen Maschine im Docker-Modus erstellen, der als Fallback-Node dienen kann.
...
2. Sobald die Maschine bereit ist, [installieren Sie den Smart Node-Stack](./docker).
3. Führen Sie `rocketpool service config` aus, um anzugeben, welche Clients Sie verwenden möchten.
   ...
   6. Speichern Sie die Einstellungen und starten Sie den Smart Node.
...
5. Aktivieren Sie optional **Prefer Fallback Clients** (standardmäßig aus). Wenn aktiviert, sendet der Smart Node API-, Node- und Watchtower-Anfragen zuerst an Ihre Fallback-Execution- und Consensus-Clients und verwendet das primäre Paar nur, wenn der Fallback nicht verfügbar ist. Dadurch bleibt die Smart Node-Last von den primären Clients fern, sodass diese sich auf die Validator-Aufgaben konzentrieren können. Ihr Validator Client ist davon nicht betroffen und verwendet weiterhin zuerst das primäre Paar.
...
Beachten Sie, dass dies nur dem **Smart Node selbst** (dem Daemon-Service) Fallback-Unterstützung bietet; **Sie müssen die Argumente Ihres Validator-Client-Service manuell aktualisieren, um ihm Zugriff auf die Fallback-Clients zu geben.**
:::
