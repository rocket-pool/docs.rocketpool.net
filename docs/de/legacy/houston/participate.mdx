...
Bitte lesen Sie den [Lebenszyklus eines Vorschlags](/de/legacy/houston/pdao#lebenszyklus-eines-pdao-vorschlags), bevor Sie fortfahren. Er erklärt die Unterschiede zwischen allen Abstimmungsperioden und den Aktionen, die während jeder Periode durchgeführt werden können.
...
- Wenn Sie mit `direct` antworten, wird die Abstimmungskraft auf Ihre Node initialisiert und Sie können direkt über Protocol DAO-Vorschläge abstimmen.
- Wenn Sie mit `delegate` antworten, haben Sie die Möglichkeit, etwas Gas zu sparen, indem Sie die Abstimmung initialisieren und [einen Delegierten festlegen](/de/legacy/houston/participate#abstimmungskraft-delegieren) innerhalb derselben Transaktion.

Sie müssen dies nur einmal tun. Es konfiguriert die anfänglichen Snapshot-Informationen für eine Node. Nachdem Sie die Abstimmung initialisiert haben, aktualisiert jede durchgeführte Aktion die Snapshot-Informationen Ihrer Node. Sobald Ihre Abstimmungskraft initialisiert ist, können Sie überprüfen, wie viel Sie haben, indem Sie den folgenden Smart Node-Befehl verwenden:
...
:::tip TIPP
Machen Sie sich keine Sorgen, wenn Sie die Website versehentlich schließen oder den Befehl verlieren. Sie können einfach die Schritte wiederholen und **erneut mit derselben Node-Adresse und Signalisierungsadresse signieren**. Das Frontend verwendet `signer.Signmessage()` aus der ethers-Bibliothek, was bedeutet, dass Ihre Signatur bei gleicher Eingabe deterministisch ist. Klicken Sie [hier](https://docs.ethers.org/v6/api/providers/#cid_865), um mehr zu erfahren.
:::
...
- Im Snapshotting enthalten (entweder durch [Abstimmung initialisieren](/de/legacy/houston/participate#abstimmung-initialisieren) oder durch Registrierung nach Houston)
- Muss mindestens einen Minipool haben
- Hat eine von Null verschiedene Abstimmungskraft
- Hat [RPL-Sperrung erlaubt](/de/legacy/houston/participate#rpl-sperrung-erlauben)
- Hat einen RPL-Stake (abzüglich bereits gesperrtem RPL), der größer ist als die Vorschlagsbindung

Vorschläge existieren, um Parameter zu ändern und Code auf Protokollebene auszuführen! Es sollte Diskussion und Konsens durch den [Governance](/de/legacy/houston/participate#governance-prozess)-Prozess geben, bevor ein Vorschlag on-chain erstellt wird.
...
Nachdem dies in einem Block enthalten ist, wird ein pDAO-Vorschlag erstellt! Der Vorschlag tritt bei Erstellung in die [Vote Delay Period](/de/legacy/houston/pdao#vote-delay-period) ein.
...
<div className="p-3">
  <Tabs>
    <Tab label="Liste der Vorschläge anzeigen">
      ...
      Hier können wir sehen, dass unser Vorschlag `invite test-member` eine ID von 71 hat und sich im ausstehenden Zustand befindet. In diesem Zustand können [Anfechtende](/de/legacy/houston/pdao#challenge-process) die Gültigkeit des vom Einbringer bereitgestellten Merkle Pollard (zur Berechnung der Abstimmungskraft) bestreiten. Wenn `proposal.vote.delay.time` endet, geht der Vorschlag in aktive Abstimmungsphasen über. Sie können gerne [Lebenszyklus eines Vorschlags](/de/legacy/houston/pdao#lebenszyklus-eines-pdao-vorschlags) zur Auffrischung lesen.
    </Tab>
    <Tab label="Vorschlagsdetails anzeigen">
      ...
      Hier können Sie alle Informationen zu einem bestimmten Vorschlag einsehen. Zum Beispiel die Ausführungspayload, in welcher Phase sich der Vorschlag befindet und die Abstimmungsrichtung.
      ...
      <p className="rspress-directive tip">
        ...
        Der Zustand dieses Beispielvorschlags ist `Pending`. Dies zeigt an, dass der Vorschlag angefochten werden kann, bevor er zu `Active (Phase 1)` übergeht. Während des `Pending`-Zustands kann keine Abstimmung stattfinden.
      </p>
    </Tab>
...
```
1. Abstain: Die Abstimmungskraft des Wählers trägt zum Quorum bei, ist aber weder für noch gegen den Vorschlag.
2. For: Der Wähler stimmt für die Ausführung des Vorschlags.
3. Against: Der Wähler stimmt gegen die Ausführung des Vorschlags.
4. Veto: Der Wähler stimmt gegen den Vorschlag und zeigt an, dass er den Vorschlag als Spam oder bösartig einstuft.
```

Ihre Abstimmungskraft wird auf die Option ihrer Wahl angewendet. Abstimmungskraft ist eine Funktion des "effektiven RPL-Stakes". Eine detailliertere Lektüre finden Sie im [rocketpool-research Repo](https://github.com/rocket-pool/rocketpool-research/blob/master/Protocol%20DAO/kane/pDAO%20Replacement/pDAO.md#overview-of-on-chain-voting).
...
Um einen Vorschlag zur Einrichtung einer wiederkehrenden Treasury-Ausgabe einzubringen, verwenden Sie den folgenden Smart Node-Befehl und folgen Sie den Aufforderungen:
