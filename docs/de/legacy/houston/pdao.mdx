...
Ein Vorschlag sollte durch den [Governance-Prozess](../houston/participate#governance-prozess) angekündigt werden, bevor er On-Chain geht. Er besteht aus 4 Perioden, die alle pDAO-[kontrollierbare Parameter](https://rpips.rocketpool.net/RPIPs/RPIP-33#parameter-table) sind:
...
Nachdem der Vorschlag die Abstimmungsperioden bestanden hat, kann der Antragsteller seine gesperrte RPL-Kaution [beanspruchen](../houston/participate#bindungen-und-belohnungen-beanspruchen), es sei denn, der Vorschlag wurde durch eine Challenge besiegt oder abgelehnt.
...
Jeder Node kann an der Verfolgung und Überprüfung der Richtigkeit von Vorschlägen teilnehmen. Um sich für diese Verantwortung anzumelden, verwenden Sie den Befehl `rocketpool service config`, navigieren Sie zum Menü **Smart Node and TX Fee Settings** und aktivieren Sie das Kontrollkästchen `Enable PDAO Proposal Checker`.

<img src={proposal_checker} width="100%" height="auto" />

Wenn diese Einstellung aktiviert ist, überprüft der Node neue Vorschläge, verifiziert ihre Richtigkeit und reicht Challenges für ungültige Vorschläge ein. Die einzige Voraussetzung ist, dass [RPL Locking](../houston/participate#rpl-sperrung-erlauben) aktiviert ist.

Diese Überprüfung läuft alle 5 Minuten in Verbindung mit einigen anderen Node-bezogenen Aufgaben. Wir werden ein Beispiel durchgehen, wie das Anfechten eines betrügerischen Vorschlags aussieht. Wir können den Smart Node-Befehl `rocketpool service logs node` verwenden, um den Fortschritt zu überwachen:
...
Da der Vorschlag nun besiegt ist, kann unser Node (der Herausforderer) seine ursprüngliche RPL-Kaution sowie die RPL-Kaution des Antragstellers als Belohnung für die Besiegung eines betrügerischen Vorschlags [beanspruchen](../houston/participate#bindungen-und-belohnungen-beanspruchen).
