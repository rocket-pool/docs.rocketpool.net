...
<div className="p-3">
  <Tabs>
    <Tab label="Running on the Holesky Test Network">
      f you're running on the Holesky test network, please see the [Practicing with the Test Network](/guides/testnet/overview) section to learn how to acquire test ETH.

      For test RPL, we have added a similar faucet function directly to the CLI.
      Please see the [Getting Test RPL on Holesky](/guides/testnet/overview#getting-test-rpl-on-holesky) guide to acquire some.
    </Tab>
  ...
  <Tabs>
    <Tab label="Method 1">
      **Use this method if your new primary withdrawal address can be used to sign transactions via MetaMask or WalletConnect.**
      ...
      1. The easiest way to do this is to navigate to the Rocket Pool primary withdrawal address site (for the [Holesky Testnet](https://testnet.node.rocketpool.net/manage/primary-withdrawal-address) or for [Mainnet](https://node.rocketpool.net/manage/primary-withdrawal-address)).
