...
import { useStabiliseScrollPosition } from "./useStabiliseScrollPosition";
...
const { selected, select } = useTabs(toRef(props, "tabLabels"), toRef(props, "sharedStateKey"));

const tablist = ref<HTMLDivElement>();
const { stabiliseScrollPosition } = useStabiliseScrollPosition(tablist);
const selectStable = stabiliseScrollPosition(select);
...
const onKeydown = (e: KeyboardEvent) => {
  ...
  if (e.key === "ArrowLeft") {
    selectIndex = currentIndex >= 1 ? currentIndex - 1 : props.tabLabels.length - 1;
  } else if (e.key === "ArrowRight") {
    selectIndex = currentIndex < props.tabLabels.length - 1 ? currentIndex + 1 : 0;
  }
...
<template>
  <div class="plugin-tabs">
    ...
    <template v-for="tabLabel in tabLabels" :key="tabLabel">
      <div v-if="tabLabel === selected" :id="`panel-${tabLabel}-${uid}`" class="plugin-tabs--content" role="tabpanel" tabindex="0" :aria-labelledby="`tab-${tabLabel}-${uid}`">
        <slot :name="tabLabel" />
      </div>
    </template>
  </div>
