...
Une proposition devrait être anticipée par le [processus de gouvernance](./participate#processus-de-gouvernance) avant qu'elle ne se retrouve on-chain. Elle se compose de 4 périodes, toutes étant des [paramètres contrôlables](https://rpips.rocketpool.net/RPIPs/RPIP-33#parameter-table) de la pDAO :
...
Les contestataires fournissent un index dans l'arbre Merkle-sum qu'ils allèguent être incorrect. Tout opérateur de nœud peut participer à la contestation de propositions frauduleuses (et gagner une récompense en le faisant). N'hésitez pas à lire sur le [processus de contestation de la pDAO](./pdao#processus-de-contestation) si vous êtes intéressé à y participer. Une proposition non vaincue lors d'une contestation à la fin de la période de délai de vote entrera dans les étapes de vote.
...
Après que la proposition ait passé les périodes de vote, le proposeur peut [réclamer](./participate#réclamer-les-cautions-et-récompenses) sa caution RPL verrouillée, sauf si la proposition a été vaincue par une contestation ou véto.
...
Tout nœud peut participer au suivi et à la vérification de l'exactitude des propositions. Pour participer à cette responsabilité, utilisez la commande `rocketpool service config`, naviguez vers le menu **Smart Node and TX Fee Settings**, et cochez la case `Enable PDAO Proposal Checker`.

<img src={proposal_checker} width="100%" height="auto" />

Lorsque ce paramètre est activé, le nœud vérifiera les nouvelles propositions, vérifiera leur exactitude et soumettra des contestations aux propositions invalides. Le seul prérequis est que le [verrouillage de RPL](./participate#autoriser-le-verrouillage-de-rpl) soit activé.
...
Maintenant que la proposition est vaincue, notre nœud (le contestataire) peut [réclamer](./participate#réclamer-les-cautions-et-récompenses) sa caution RPL originale ainsi que la caution RPL du proposeur en tant que récompense pour avoir vaincu une proposition frauduleuse.
