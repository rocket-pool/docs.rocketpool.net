...
Veuillez lire le [cycle de vie d'une proposition](./pdao#cycle-de-vie-dune-proposition-pdao) avant de continuer. Cela expliquera les différences entre toutes les périodes de vote et les actions qui peuvent être prises pendant chaque période.
...
- Si vous répondez avec `delegate`, vous aurez l'opportunité d'économiser un peu de gaz en initialisant le vote et en [définissant un délégué](./participate#déléguer-le-pouvoir-de-vote) dans la même transaction.
...
La version `1.13.8` du Smart Node introduit une nouvelle fonctionnalité **Initialisation automatique du pouvoir de vote**, conçue pour initialiser automatiquement le pouvoir de vote sur les nœuds qui ne l'ont pas encore fait. Le pouvoir de vote initialisé automatiquement est auto-délégué. Cette fonctionnalité peut être configurée dans les paramètres du Smart Node en exécutant `rocketpool service config` et en naviguant vers la section **Smart Node and TX Fees**.
...
- Inclus dans le snapshotting (soit en [initialisant le vote](./participate#initialiser-le-vote) soit en s'enregistrant après Houston)
- Doit avoir au moins un minipool
- A un pouvoir de vote non nul
- A autorisé le [verrouillage de RPL](./participate#autoriser-le-verrouillage-de-rpl)
- A un stake RPL (moins tout RPL déjà verrouillé) supérieur à la caution de proposition

Les propositions existent pour changer les paramètres et exécuter du code au niveau du protocole ! Il devrait y avoir discussion et consensus à travers le [processus de gouvernance](./participate#processus-de-gouvernance) avant qu'une proposition ne soit créée on-chain.
...
COMMANDES :
   ...
   submit-batch, sb            Soumettre une seule proposition qui modifie plusieurs paramètres de la DAO de Protocole à partir d'un fichier JSON
```

Chacune de ces commandes vous invitera avec une liste d'entrées pour créer votre proposition souhaitée. Les commandes de proposition de paramètre acceptent `--to-json` pour écrire ou ajouter une modification de paramètre dans un fichier JSON au lieu de soumettre une transaction. Lorsque le fichier est prêt, soumettez-le avec `rocketpool pdao propose submit-batch`.

Dans ce guide, nous inviterons un nœud au conseil de sécurité pour servir d'exemple. Pour créer une proposition pour inviter un nœud au conseil de sécurité, vous utiliseriez la commande :
...
Après que cela soit inclus dans un bloc, une proposition pDAO sera créée ! La proposition entrera dans la [période de délai de vote](./pdao#période-de-délai-de-vote) dès sa création.
