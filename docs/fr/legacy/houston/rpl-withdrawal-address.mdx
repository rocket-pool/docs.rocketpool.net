...
Il est important de comprendre la différence entre votre [adresse de retrait principale](../../node-staking/prepare-node#définition-de-votre-adresse-de-retrait-principale) et votre adresse de retrait RPL. Par défaut, votre adresse de retrait RPL n'est pas définie et votre adresse de retrait principale est l'endroit où toutes vos récompenses de checkpoint RPL, votre RPL staké et votre ETH de la Beacon Chain seront envoyés.

Vous pouvez définir une [adresse de retrait séparée pour votre RPL](https://rpips.rocketpool.net/RPIPs/RPIP-31) si vous le souhaitez. Votre adresse de retrait RPL si elle est définie pourra déclencher et réclamer le RPL des récompenses d'inflation et n'aura aucun effet sur les récompenses de consensus ETH ou quoi que ce soit lié à l'ETH. Dans ce cas, votre adresse de retrait principale n'aura plus aucune autorité sur les actions liées au RPL.

Cela crée des opportunités intéressantes où le RPL peut être fourni par une entité à un opérateur de nœud qui ne souhaite pas avoir d'exposition au RPL. Cette entité peut ensuite réclamer les récompenses RPL pour avoir fourni la garantie d'assurance requise pour le nœud. En d'autres termes, cela permet à différents acteurs de fournir chaque type de garantie.

::: warning NOTE
Lors de la configuration initiale de votre nœud, votre adresse de retrait RPL n'est pas définie par défaut.
...
De cette façon, si votre portefeuille de nœud est compromis, l'attaquant n'aura pas accès à votre RPL staké en vous forçant à sortir car tous ces fonds seront envoyés à votre portefeuille froid séparé (qu'il n'a espérons-le pas).
...
[Voir ici](https://ethereum.org/en/security/#wallet-security) pour quelques bases sur la sécurité des portefeuilles de la Fondation Ethereum.
:::
    ...
    <Tab label="Method 1">
      *Utilisez cette méthode si votre nouvelle adresse de retrait RPL peut être utilisée pour signer des transactions via MetaMask ou WalletConnect.**
      ...
      <p className="rspress-directive warning">
        ...
        Cette méthode vous obligera à **soumettre une transaction** depuis votre nouvelle adresse de retrait RPL, vous **devez donc avoir une petite quantité d'ETH dans cette adresse.**
      </p>
      ...
      <p className="rspress-directive warning">
        ...
        Suivez [les instructions officielles Ledger](https://www.ledger.com/academy/security/the-safest-way-to-use-metamask) pour ce faire.
        ...
        Vous devrez également **activer "blind signing"** pour la session en cours ; vous pouvez le trouver dans la section Paramètres de l'application ETH sur l'appareil.
        La signature aveugle sera automatiquement désactivée après la fermeture de la session.

        Si vous utilisez Hoodi Testnet et souhaitez utiliser votre Ledger comme adresse de retrait RPL, **vous devez créer un nouveau portefeuille Ethereum sur votre Ledger** d'abord pour vous assurer de ne pas connecter votre adresse en direct au réseau de test, ce qui a tendance à créer de la confusion.
        ...
        Notez que Ledger Live n'affichera pas votre solde sur le réseau de test, mais d'autres applications prenant en charge le réseau de test (telles que MetaMask et Etherscan) pourront l'afficher.
      </p>

      1. Exécutez `rocketpool node set-rpl-withdrawal-address 'your cold wallet address or ENS name'`. Votre nouvelle adresse de retrait RPL sera marquée comme "en attente". Jusqu'à ce que vous la confirmiez, **votre ancienne adresse de retrait RPL sera toujours utilisée**.
      ...
      1. La façon la plus simple de le faire est de naviguer vers le site d'adresse de retrait RPL de Rocket Pool (pour le [Hoodi Testnet](https://testnet.node.rocketpool.net/rpl-withdrawal-address) ou pour [Mainnet](https://node.rocketpool.net/rpl-withdrawal-address)).
      ...
      Cliquez sur **Confirm** pour envoyer la transaction au réseau. Cela prendra du temps jusqu'à ce qu'elle soit incluse dans un bloc, mais une fois que c'est fait, vous verrez une boîte de dialogue de confirmation :

      <img src={rplWithdrawalConfirmation} width="100%" height="auto"/>

      5. Votre nouvelle adresse de retrait RPL sera maintenant confirmée et activée. Vous pouvez la voir avec `rocketpool node status`.
    </Tab>
    <Tab label="Method 2">
      **Utilisez cette méthode uniquement si votre adresse de retrait RPL _ne peut pas_ être utilisée pour signer des transactions via MetaMask ou WalletConnect.**
      ...
      <p className="rspress-directive danger">
        ...
        Si vous faites une faute de frappe ici, il n'y a aucun moyen de l'annuler et **les récompenses de votre minipool seront perdues pour toujours**.

        Nous vous **encourageons fortement** à utiliser le mécanisme de transaction de test avant de confirmer cela, et si possible, utilisez la Méthode 1 à la place.
      </p>
    </Tab>
...
Pour la changer, vous devrez envoyer une transaction signée depuis votre adresse de retrait RPL **active** (celle vers laquelle vous venez de passer).
Le site Web Rocket Pool a une fonction pour vous aider à le faire.

Si vous souhaitez désactiver votre adresse de retrait RPL, le processus est assez simple. Tout d'abord, vous voudrez retourner à la page d'adresse de retrait RPL sur le site Web Rocket Pool.
...
Si vous êtes connecté avec l'adresse de retrait RPL du nœud, vous devriez pouvoir cliquer sur le bouton **unset** à droite ici :

<img src="./images/unset_rpl_withdrawal.png" width="100%" height="auto" />

Cliquer sur **unset** vous amènera à une page où vous pouvez confirmer les détails. Si vous êtes satisfait de ce que vous voyez, cliquez sur le bouton orange **unset** pour envoyer la transaction au réseau.
...
N'hésitez pas à passer sur [notre serveur Discord](https://discord.gg/G46XgK264a) pour nous faire savoir ce que vous en avez pensé et suivre le projet au fur et à mesure qu'il évolue.
