...
Lorsque vous décidez qu'il est temps, le Smart Node est livré avec la capacité de l'élaguer pour vous sur demande.
...
Si vous utilisez votre propre client d'exécution, comme un client externe en mode Hybrid ou en mode Native, vous ne pouvez pas utiliser le Smart Node pour élaguer le client d'exécution.
...
<div className="p-3">
  <Tabs>
    <Tab label="Geth">
      ...
      Lorsque cela se produit, le Smart Node (et votre client de consensus) aura besoin d'une autre façon d'accéder à la chaîne ETH1 pour fonctionner correctement.

      Le moyen le plus simple de fournir cela est avec un **nœud de secours**.
      Si vous [avez configuré un nœud de secours](./fallback) en utilisant `rocketpool service config` déjà, alors le Smart Node basculera automatiquement vers celui-ci lorsque votre conteneur Geth sera arrêté pour maintenance.
...
<div className="p-3">
  <Tabs>
    <Tab label="Geth">
      ...
      Si vous acceptez, vous verrez quelques détails pendant que le Smart Node prépare les choses ; cela devrait se terminer par un message de succès :
      ...
      Une fois l'élagage terminé, il redémarrera automatiquement et le Smart Node recommencera à l'utiliser au lieu de votre secours.
    </Tab>
    <Tab label="Nethermind">
