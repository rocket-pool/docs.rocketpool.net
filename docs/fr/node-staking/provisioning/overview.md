...
Cette section couvre les détails de la façon de provisionner votre nœud pour le staking avec Rocket Pool une fois que vous avez installé et configuré le Smart Node.
...
[Starting Rocket Pool](../starting-rp) vous montrera comment démarrer les services du Smart Node pour chaque mode et comment vérifier la progression de synchronisation de vos clients d'exécution et de consensus.

[Creating a New Wallet](../wallet-init) vous guide à travers le processus de création d'un tout nouveau portefeuille avec le Smart Node si c'est la première fois que vous configurez un nœud.
...
[MEV, MEV-Boost, and MEV Rewards](../mev.mdx) explique la **Maximum-Extractable Value** (MEV), son rôle dans l'écosystème de staking, et comment vous pouvez la configurer à votre goût en utilisant le Smart Node.
