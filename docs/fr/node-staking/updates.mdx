...
## Mise à jour de la pile Smart Node

Occasionnellement, Rocket Pool publiera une nouvelle version de la pile Smart Node.
...
::: tip ASTUCE
Lorsque vous avez terminé la mise à niveau du Smart Node, le tableau de bord Grafana indiquera toujours qu'une mise à jour est disponible.
...
Les étapes de mise à niveau dépendent du mode utilisé par votre nœud. La meilleure façon de mettre à jour le Smart Node est d'utiliser la commande de mise à jour intégrée de la CLI. Sélectionnez parmi les options ci-dessous :

<div className="p-3">
  <Tabs>
    <Tab label="Commande de mise à jour CLI (Mode Docker - Linux/macOS)">

      Pour mettre à jour le Smart Node à l'aide de la commande de mise à jour intégrée, exécutez :

      ```shell
      rocketpool update
      ```
      Cette commande va :
      1. Télécharger et vérifier le dernier binaire de la CLI
      2. Éventuellement arrêter, mettre à niveau et redémarrer le service automatiquement

      Suivez les invites interactives pour terminer la mise à jour.
      ```
      ...
      Ensuite, le Smart Node téléchargera et vérifiera le dernier binaire de la CLI :
      ...
      Updating from 1.24.0 to 1.24.2
      ...
      y
      ```
      À ce stade, vous pouvez sélectionner `y` à toutes les invites interactives pour laisser le Smart Node vous guider dans le reste des étapes.
      Une fois terminé, vous verrez un message confirmant que la mise à niveau a réussi :
      ```
      The upgrade to Smart Node 1.24.2 has been completed.
    ...
    <Tab label="Mise à jour manuelle - Linux (Mode Docker ou Hybride)">
      ...
      Téléchargez la nouvelle CLI Smart Node :
      ...
      Enfin, vérifiez la version pour vous assurer que la CLI et la pile Smart Node sont toutes les deux à jour :
      ...
      Rocket Pool client version: 1.24.2
      Rocket Pool service version: 1.24.2
    ...
    <Tab label="Mise à jour manuelle - Linux (Mode natif)">
      <p className="rspress-directive warning">
        <p className="rspress-directive-title">NOTE</p>
        Cela ne mettra à jour que la pile Smart Node elle-même - cela **ne mettra pas** à jour vos clients d'exécution ou de consensus.
      ...
      Téléchargez la nouvelle CLI Smart Node et sauvegardez les anciens binaires CLI et daemon au cas où vous auriez besoin de les rétablir :
    ...
    <Tab label="Mise à jour manuelle - macOS (Mode Docker ou Hybride)">
      Arrêtez les services Rocket Pool :
...
Chaque nouvelle version du Smart Node est livrée avec des digests d'images Docker épinglés pour les conteneurs d'exécution, de consensus et du Smart Node (`repo:tag@sha256:…` dans `~/.rocketpool/mainnet.env`). `rocketpool service install` écrase ces fichiers d'épinglage officiels. Dans certains cas, vous pourriez vouloir exécuter une image de client différente _avant_ d'attendre une nouvelle version du Smart Node. Cette section vous montre comment faire.
...
<div className="p-3">
  <Tabs>
    <Tab label="Mode Docker">
      ...
      Le Smart Node épingle désormais par digest les images des clients et du Smart Node.
      Modifier **Container Tag** dans la TUI vous permet toujours d'exécuter un tag ou un digest différent avant la prochaine version du Smart Node.
      Après `rocketpool service install` ou `rocketpool update`, les épinglages officiels de `mainnet.env` sont remplacés ; une surcharge définie dans la TUI l'emporte toujours jusqu'à ce que vous la supprimiez.
      ...
      Le Smart Node proposera de redémarrer automatiquement tous les conteneurs affectés.

      ::: tip NOTE
      Les épinglages officiels se trouvent dans `~/.rocketpool/mainnet.env`. N'éditez pas ce fichier pour des modifications durables ; l'installation l'écrase. Les surcharges facultatives par réseau vont dans `~/.rocketpool/testnet.env` ou `~/.rocketpool/devnet.env` (seules les clés que vous définissez remplacent l'épinglage de mainnet). Un **Container Tag** défini dans le gestionnaire de paramètres remplace l'épinglage pour ce service. Laissez-le à sa valeur par défaut pour conserver le digest officiel.
      :::
    </Tab>
