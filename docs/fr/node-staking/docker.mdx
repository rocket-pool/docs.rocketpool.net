...
Dans cette section, nous allons parcourir le processus d'installation de la pile Smart Node Rocket Pool en utilisant la configuration standard basée sur [Docker](https://www.docker.com/resources/what-container).
Cela installera et configurera tout ce dont vous avez besoin pour exécuter un nœud complet, y compris :

- Le logiciel **Smart Node** Rocket Pool
...
1. Utiliser la CLI pour installer la pile Smart Node
1. Configurer la pile Smart Node avec une interface utilisateur facile à utiliser dans le terminal
1. Terminé !
...
L'installateur Ubuntu obtient docker du gestionnaire de paquets snap, qui entrera en conflit avec la version de docker installée par Smart Node.
...
## Installation de la pile Smart Node

Maintenant que vous avez installé la CLI, vous pouvez déployer la pile Smart Node.
Cela préparera votre système avec Docker, [docker-compose](https://docs.docker.com/compose/), et chargera les fichiers Smart Node afin qu'ils soient prêts à être utilisés.
Cela n'exécutera rien pour le moment ; cela viendra plus tard.

Pour déployer la pile Smart Node, vous devrez exécuter la commande suivante sur votre machine de nœud (soit en vous connectant localement, soit à distance via SSH) :

```shell
rocketpool service install
```

Cela récupérera la dernière version de la pile Smart Node et la configurera.
...
::: warning NOTE
Notez que l'installateur Smart Node ne peut pas installer `docker` et `docker-compose` automatiquement sur toutes les plateformes.
...
Une fois cela terminé, la pile Smart Node sera prête à fonctionner.
...
## Configuration de la pile Smart Node
