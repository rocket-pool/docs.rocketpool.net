...
Si vous choisissez d'activer les métriques pendant le processus de configuration Smart Node, votre nœud ajoutera les processus suivants :
...
La configuration par défaut créera des conteneurs Docker avec tous ces services qui vivent aux côtés du reste des conteneurs Docker du Smart Node.
...
<div className="p-3">
  <Tabs>
    <Tab label="Docker">
      ...
      Commencez par exécuter à nouveau la commande de configuration Smart Node :
      ...
      Pendant cette vérification, il comparera également votre version installée de Rocket Pool Smart Node avec la dernière version, et vous informera s'il y a une nouvelle version disponible.
...
Référez-vous aux onglets ci-dessous pour votre mode d'installation Smart Node.
...
Lorsqu'on vous demande l'ID du tableau de bord dans la boîte **Import via grafana.com**, entrez `24900` ou utilisez l'URL complète ([https://grafana.com/grafana/dashboards/24900-rocket-pool-dashboard-v1-4-0/](https://grafana.com/grafana/dashboards/24900-rocket-pool-dashboard-v1-4-0/)) et appuyez sur le bouton **Load**.
...
::: tip NOTE
Smart Node v1.23.0 est livré avec **Rocket Pool Dashboard v1.4.2** dans sa pile de métriques. Cette révision ajoute les comptages de validateurs minipool et megapool par statut à l'échelle du nœud et supprime les panneaux d'état megapool à l'échelle du réseau qui dépendaient de métriques du supply-collector désormais retirées. Si votre Grafana a déjà importé un tableau de bord plus ancien, réimportez-le (ou chargez le JSON fourni avec Smart Node) afin que ces panneaux correspondent aux métriques exportées par votre nœud.
:::
...
Elles seront réactivées dans une future version du Smart Node qui pourra suivre correctement les récompenses historiques.
:::
...
Si vous n'avez pas modifié l'emplacement par défaut de Docker sur un autre disque lors de l'installation de votre Smart Node, alors le disque que vous voulez suivre sera celui sur lequel votre système d'exploitation est installé.
