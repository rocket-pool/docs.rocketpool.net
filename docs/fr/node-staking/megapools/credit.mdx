...
- **Réduction de votre montant de bond**. Si votre bond actuel dépasse l'exigence minimale du protocole, vous pouvez réduire votre bond excédentaire et le recevoir sous forme de crédit de dépôt. Notez que les deux premiers validateurs nécessitent toujours des bonds de 4 ETH chacun
(indépendamment des paramètres pDAO). La réduction de bond bénéficie principalement aux opérateurs de nœuds avec 3+ validateurs lorsque la pDAO abaisse le paramètre `reduced_bond`. Utilisez `rocketpool megapool reduce-bond` pour effectuer une réduction de bond.
