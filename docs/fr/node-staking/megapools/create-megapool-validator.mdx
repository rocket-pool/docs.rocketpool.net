...
Si un validateur en prelaunch ne parvient pas à staker dans les 365 jours, le validateur sera dissous. Le 1 ETH (sur un bond de 4 ETH) envoyé à la Beacon Chain pendant le processus de prelaunch n'est **pas récupérable**.
L'opérateur de nœud reçoit les 3 ETH restants de son bond (moins une pénalité de dissolution de 0,05 ETH appliquée en dette) sous forme de remboursement dans son megapool, pour un remboursement net de 2,95 ETH.
Cela peut être réclamé en utilisant `rocketpool megapool claim` ou `rocketpool claims claim-all`.
...
dépôt complet de 32 ETH sur la Beacon Chain. La transaction `stake` fera progresser votre validateur `Prelaunch` vers un validateur `Staking`. À ce stade, votre validateur `Staking` a :

- 32 ETH déposés sur la Beacon Chain
- Un numéro d'index de validateur attribué
- Activation en attente sur la Beacon Chain
