...
Après que votre validateur `Prelaunch` est traité par la Beacon Chain et que le dépôt initial de 1 ETH est crédité, votre nœud effectuera automatiquement une
transaction `stake` pour compléter le dépôt complet de 32 ETH sur la Beacon Chain. La transaction `stake` fera progresser votre validateur `Prelaunch`
vers un validateur `Staking`. À ce stade, votre validateur `Staking` a :
