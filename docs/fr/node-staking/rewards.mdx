...
Le Merkle Tree est construit dans un fichier JSON et hébergé sur [un dépôt GitHub que nous maintenons](https://github.com/rocket-pool/rewards-trees/). Le Smart Node peut également charger les **événements** d'intervalle de récompenses depuis ce dépôt GitHub, ce qui vous permet de réclamer même après qu'une expiration agressive de l'historique a supprimé les anciens journaux de la couche d'exécution.
...
- Si votre client Execution principal n'est pas un nœud d'archive, vous pouvez ajouter une URL pour un nœud d'archive séparé dans la boîte `Archive-Mode EC URL`.
  ...
  - L'Archive EC ne sera utilisé que pour la génération d'arbres ; il ne sera utilisé pour aucune autre tâche du Smart Node.
...
Maintenant, votre Smart Node construira l'arbre complètement indépendamment de l'Oracle DAO, en utilisant uniquement les données fournies par vos clients Execution et Consensus.
...
Si vous n'êtes **pas** inscrit au Smoothing Pool, la portion de la couche Execution de vos récompenses des propositions de blocs (y compris les frais de transaction et MEV) sera envoyée au contrat [Fee Distributor](./fee-distrib-sp#distributeurs-de-frais-et-la-smoothing-pool) de votre nœud à la place.
