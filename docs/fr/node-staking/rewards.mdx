...
Le Merkle Tree est construit dans un fichier JSON et hébergé sur [un dépôt GitHub que nous maintenons](https://github.com/rocket-pool/rewards-trees/). Le Smartnode peut également charger les **événements** d'intervalle de récompenses depuis ce dépôt GitHub, ce qui vous permet de réclamer même après qu'une expiration agressive de l'historique a supprimé les anciens journaux de la couche d'exécution.
