...
La route CoW Swap ne fait pas partie du protocole Rocket Pool. Les cotations et les solvers sont déterminés par des systèmes **tiers** qui échappent au contrôle ou à la visibilité du protocole Rocket Pool. Les utilisateurs doivent noter que l'utilisation de CoW Swap peut les exposer à des risques supplémentaires tels que le slippage, l'impact sur les prix, des solvers qui dirigent les fonds de manière incorrecte, etc. Pour un contrôle précis,
nous recommandons fortement d'utiliser CoW Swap directement.
:::
...
<div className="p-3">
  <Tabs>
    <Tab label="Préparation sur le Réseau de Test Hoodi">
      ...
      Rendez-vous sur la page [Pratique avec le Réseau de Test](../node-staking/testnet/overview#obtenir-de-leth-de-test-sur-hoodi) pour un guide rapide sur la façon d'utiliser un robinet de réseau de test pour obtenir de l'ETH de test sur Hoodi.
