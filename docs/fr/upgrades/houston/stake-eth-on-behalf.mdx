...
Bien que l'objectif principal de cette fonctionnalité soit de faciliter les scénarios à déposant unique, il convient de noter que plusieurs déposants indépendants peuvent également tirer parti de cette capacité en créant des contrats intelligents superposés. Rocket Pool a également introduit la possibilité de [staker du RPL au nom](../../node-staking/create-validator#mettre-en-liste-blanche-une-adresse-pour-staker-au-nom-de) d'un nœud lors de notre précédente version Atlas.

### Prérequis

Assurez-vous d'avoir correctement configuré l'[adresse de retrait principale](../../node-staking/prepare-node#définition-de-votre-adresse-de-retrait-principale) du nœud que vous avez l'intention de financer. Suivez le lien ci-dessus pour un guide sur le fonctionnement des adresses de retrait après Houston.
...
<div className="p-3">
  <Tabs>
    <Tab label="Préparation sur le Hoodi Testnet">
      ...
      Rendez-vous sur la page [Pratiquer avec le réseau de test](../../node-staking/testnet/overview#obtenir-de-leth-de-test-sur-hoodi) pour un guide rapide sur l'utilisation d'un faucet de testnet pour obtenir des ETH de test sur Hoodi.
...
<div className="p-3">
  <Tabs>
    <Tab label="Préparation sur le Hoodi Testnet">
      Lorsque vous êtes prêt à retirer l'ETH que vous avez staké au nom d'un nœud, rendez-vous sur [https://testnet.node.rocketpool.net/withdraw-eth](https://testnet.node.rocketpool.net/withdraw-eth).
      Après vous être connecté en tant qu'[adresse de retrait principale](../../node-staking/prepare-node#définition-de-votre-adresse-de-retrait-principale) du nœud, vous devriez être accueilli par un
      menu familier. Il vous sera demandé d'entrer l'adresse du nœud depuis lequel vous souhaitez retirer :
    </Tab>
    <Tab label="Préparation sur le Ethereum Mainnet">
      Lorsque vous êtes prêt à retirer l'ETH que vous avez staké au nom d'un nœud, rendez-vous sur [https://node.rocketpool.net/withdraw-eth](https://node.rocketpool.net/withdraw-eth). Après vous être
      connecté en tant qu' [adresse de retrait principale](../../node-staking/prepare-node#définition-de-votre-adresse-de-retrait-principale) du nœud, vous devriez être accueilli par un menu familier.
      Il vous sera demandé d'entrer l'adresse du nœud depuis lequel vous souhaitez retirer :
    </Tab>
  </Tabs>
</div>
