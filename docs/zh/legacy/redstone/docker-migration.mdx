...
本指南将涵盖您需要了解的所有内容,以便在使用 **Docker 模式**时为您的节点准备 Redstone 更新和合并。

## 升级到 v1.5.0 之前要做的事情

在升级到 Smart Node 的 v1.5.0 及更高版本之前,请检查以下清单以确保您已准备好:
...
由于此更改,您应该在仍使用 v1.4 时切换到完整客户端,让它同步完成,然后升级到 v1.5。

Docker 模式使切换客户端变得非常容易。
[本指南](../../node-staking/change-clients#更改-execution-客户端)提供了该过程的演练。

## 升级到 v1.5.0

将 Smart Node 堆栈升级到 v1.5.0 与任何其他升级没有区别。
只需遵循[此处的正常说明](../../node-staking/updates#更新-smartnode-堆栈)。

## Smart Node 自动处理的事项

在 Docker 模式下,一旦您更新到 v1.5.0,Smart Node 将自动处理支持 Redstone 和合并所需的大部分更改。
...
Smart Node 将自动处理身份验证令牌的设置以及执行客户端和共识客户端上的 Engine API。

### 您的费用接收者

**费用接收者**是执行层链上的地址,将接收您提议的区块的所有优先费用。
这是在验证器客户端首次启动时提供给它的设置。

当您升级到 v1.5 时,Smart Node 将处理将其设置为正确的地址,并将不断检查以确保您使用正确的地址,这样您就不会[意外受到惩罚](https://github.com/rocket-pool/rocketpool-research/blob/master/Penalties/penalty-system)。
...
Rocket Pool 已将 MEV-Boost 内置到 Smart Node 中,并自动配置您的节点使用它,因此您的提议可以获得最大奖励。

## 升级后您应该做的事情

虽然 Smart Node 为您处理了大部分更改,但您仍应手动执行一些其他操作:
...
Smart Node 仍然能够提供后备执行客户端(现在还有后备共识客户端),但您现在需要使用您控制的执行客户端和共识客户端。

有关设置后备节点的更多信息,请参阅[后备节点指南](../../node-staking/fallback)。

### 初始化您的费用分配器

如果您不打算选择加入[平滑池](./whats-new#smoothing-pool)并将所有优先费用和 MEV 奖励申领到您的[费用分配器合约](./whats-new#费用接收者和您的分配器),您最终将必须初始化它(在链上创建合约实例),以便将奖励从其申领到您的提款地址。
...
Redstone 升级用[全新系统](./whats-new#新的奖励系统)取代了昂贵且有问题的旧奖励系统,该系统更便宜,支持 RPL 的自动重新质押(部分和全额),最重要的是 - **让您随时申领奖励**。

由于申领奖励不再有时间限制,而且一次申领多个奖励间隔更便宜,Smart Node 的自动奖励申领功能**已被移除**。
您现在可以通过以下命令申领奖励:
...
如果出于任何原因,某些内容不符合您的喜好,并且您想恢复到以前的 Smart Node 版本,您可以轻松做到这一点。
Smart Node 在升级时会自动备份您以前版本的设置,因此只需获取以前的版本(这里我们演示 **v1.4.3**)并用备份替换设置:
