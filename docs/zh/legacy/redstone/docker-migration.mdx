...
Smartnode 将自动处理身份验证令牌的设置以及执行客户端和共识客户端上的 Engine API。
...
当您升级到 v1.5 时,Smartnode 将处理将其设置为正确的地址,并将不断检查以确保您使用正确的地址,这样您就不会[意外受到惩罚](https://github.com/rocket-pool/rocketpool-research/blob/master/Penalties/penalty-system)。
...
Rocket Pool 已将 MEV-Boost 内置到 Smartnode 中,并自动配置您的节点使用它,因此您的提议可以获得最大奖励。
