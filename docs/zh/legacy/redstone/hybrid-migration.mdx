...
::: tip 提示
如果您有兴趣让 Smartnode 维护自己的执行客户端和共识客户端,但希望保持对自己的验证器客户端的控制(例如,如果您有自己的单独质押验证器密钥附加到它),您可能需要考虑[反向混合模式](../../node-staking/advanced-config#允许外部验证者客户端连接到-smartnode),它正是这样做的!
:::
