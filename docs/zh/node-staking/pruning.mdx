...
::: tip 注意
本页面介绍 `geth` 和 `nethermind` 的**离线／在线修剪**路径（`rocketpool service prune-eth1`）。

控制磁盘占用的常规方式是 **Rolling History Expiry**（默认的 Pruning Mode）。Nethermind、Besu、Reth 和 Erigon 会删除大约一年以前的区块体和收据；Geth 则使用 Prague 之前的历史过期方式。参见[执行客户端历史过期](./maintenance/history-expiry)。
...
当您决定是时候修剪时,Smart Node 可以根据您的请求为您修剪。
请阅读下文以了解它的工作原理以及预期结果。

::: warning 注意
修剪执行客户端仅在 Docker 模式下可用。

如果您使用自己的执行客户端,例如混合模式或原生模式中的外部客户端,则无法使用 Smart Node 修剪执行客户端。
您需要手动进行。
...
<div className="p-3">
  <Tabs>
    <Tab label="Geth">
      ...
      当这种情况发生时,Smart Node(和您的共识客户端)将需要其他方式来访问 ETH1 链以正常运行。

      提供这一点的最简单方法是使用**后备节点**。
      如果您已经使用 `rocketpool service config` [配置了后备节点](./fallback),那么当您的 Geth 容器停机维护时,Smart Node 将自动切换到它。
      它还会通知您的共识客户端使用后备节点。
...
<div className="p-3">
  <Tabs>
    <Tab label="Geth">
      ...
      如果您接受,您将看到 Smart Node 准备过程的一些详细信息;它应该以成功消息结束:
      ...
      修剪完成后,它将自动重启,Smart Node 将恢复使用它而不是您的后备节点。
    </Tab>
    <Tab label="Nethermind">
...
## 将 Geth 迁移到 Pebble v2

如果您运行 **Geth** 且数据库仍使用 Pebble v1，可以使用以下命令就地迁移：

```shell
rocketpool service migrate-geth
```

该命令会关闭 Geth，将数据库从 Pebble v1 迁移到 v2，然后重新启动它。它与 `prune-eth1` **不同**，并且命令名称是 **`migrate-geth`**（不是 `migrate-eth1`）。

::: warning 警告
迁移期间 Geth 处于离线状态。如果没有[备用节点](./fallback)，在迁移完成之前您将错过证明和区块提议。
:::

请确认提示，或传入 `--yes` 跳过提示。使用以下命令查看进度：
