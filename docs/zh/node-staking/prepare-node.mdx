...
如果您来到这里,说明您已成功启动 Smart Node 服务,创建了钱包,并完成了执行层和共识层客户端的同步。
...
<div className="p-3">
  <Tabs>
    <Tab label="在 Hoodi 测试网络上运行">
      ...
      请参阅[在 Hoodi 上获取测试 RPL](/zh/node-staking/testnet/overview#在-hoodi-上获取测试-rpl)指南以获取一些。
    </Tab>
    <Tab label="在主网上运行">
...
重要的是要了解[主提款地址](./prepare-node#设置主提款地址)和 RPL 提款地址之间的区别。默认情况下,您的 RPL 提款地址未设置,您的主提款地址是所有 RPL 检查点奖励、质押的 RPL 和 Beacon Chain ETH 将被发送到的地方。
...
3. 在确认前一笔交易后,您可以使用 Smart Node CLI 设置**反向解析**。这将允许应用程序从您的节点钱包地址映射回您的 `.eth` 名称。
