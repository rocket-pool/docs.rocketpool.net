...
如果是这样,那么您已准备好在 Rocket Pool 网络上注册节点并创建带有验证者的 minipool!
如果还没有,请回顾前面的部分,完成这些步骤后再返回这里。

在创建新的 minipool 和验证者之前,还需要采取几个步骤来完成节点的准备工作。
不过这些步骤只需执行一次;完成后,如果您想在节点上创建多个 minipool,可以直接跳到[创建新的 Minipool](./create-validator.mdx)部分。

## 加载节点钱包

注册节点和启动验证者都涉及从节点钱包向以太坊网络提交交易。
这意味着**您需要在钱包中有一些 ETH** 来支付这些交易的 gas 费用。
在创建 minipool 之前,您还需要质押一些 **RPL 代币**作为抵押品;您可以直接在节点上执行此操作,或者(更好的方式)您可以使用 Rocket Pool 网站的**代表质押**功能,用冷钱包中的 RPL 为节点质押。
我们将在本指南后面讨论代表质押功能,届时将质押 RPL。

设置一个 minipool 的节点所需的 gas 费用约为 0.0025 ETH 乘以当前 gas 价格(以 gwei 为单位)。
...
这是您在领取检查点奖励或退出验证者并从 minipool 提款时,所有 RPL 检查点奖励、质押的 RPL 和信标链 ETH 将被发送到的地址。
...
主提款地址在节点操作者级别设置。
如果您创建多个 minipool,它们都将引用相同的主提款地址,因此您只需执行一次此设置。
  ...
  <Tabs>
    <Tab label="方法 1">
      **如果您的新主提款地址可以通过 MetaMask 或 WalletConnect 签署交易,请使用此方法。**
      ...
      2. 要确认它,您必须**从新的主提款地址**向 minipool 合约发送特殊交易,以验证您拥有主提款地址。
    ...
    <Tab label="方法 2">
      **仅当您的主提款地址_无法_通过 MetaMask 或 WalletConnect 签署交易时,才使用此方法。**
      ...
      如果在此处输入错误,无法撤销,**您的 minipool 奖励将永远丢失**。
...
重要的是要了解[主提款地址](./prepare-node#setting-your-primary-withdrawal-address)和 RPL 提款地址之间的区别。默认情况下,您的 RPL 提款地址未设置,您的主提款地址是所有 RPL 检查点奖励、质押的 RPL 和信标链 ETH 将被发送到的地方。
...
RPL 提款地址在节点操作者级别设置。
如果您创建多个 minipool,它们都将引用相同的 RPL 提款地址,因此您只需执行一次此设置。
  ...
  <Tabs>
    <Tab label="方法 1">
      *如果您的新 RPL 提款地址可以通过 MetaMask 或 WalletConnect 签署交易,请使用此方法。**
      ...
      2. 要确认它,您必须**从新的 RPL 提款地址**向 minipool 合约发送特殊交易,以验证您拥有 RPL 提款地址。
...
完成这些主题后,您将被引导完成创建 minipool 和赚取质押奖励的过程。
