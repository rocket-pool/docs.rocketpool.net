...
如果您选择[将验证者从 Rocket Pool 存款队列中退出](/zh/node-staking/megapools/create-megapool-validator#从-rocket-pool-存款队列中退出验证者)，您的节点将获得退还的快速队列票据。
...
如果预启动验证者在 365 天内未能质押，该验证者将被解散。在预启动过程中发送到信标链的 1 ETH（4 ETH 质押金的一部分）**无法恢复**。
节点运营者将收到其质押金中剩余的 3 ETH（扣除 0.05 ETH 的解散罚款作为债务）作为退款至其 megapool，净退款为 2.95 ETH。
可以使用 `rocketpool megapool claim` 或 `rocketpool claims claim-all` 来领取。

[了解如何确认质押成功](/zh/node-staking/megapools/create-megapool-validator#确认质押成功)
...
下一个提示将显示 [Rocket Pool 存款队列](/zh/node-staking/megapools/create-megapool-validator#rocket-pool-存款队列和快速队列) 的状态。
...
一旦您的验证者从 [Rocket Pool 存款队列](/zh/node-staking/megapools/create-megapool-validator#rocket-pool-存款队列和快速队列) 获得 ETH 分配，它将进入 `Prelaunch` 状态。此时，您 megapool 余额中的 1 ETH 将存入信标链。您的验证者公钥也将在信标链上注册，这意味着您可以在 https://beaconcha.in/（如果您使用测试网，则访问 https://hoodi.beaconcha.in/）等浏览器上查看 `Prelaunch` 验证者的状态。
...
在您的 `Prelaunch` 验证者被信标链处理并且初始 1 ETH 存款被记入后，您的节点将自动执行 `stake` 交易以完成完整的 32 ETH 信标链存款。`stake` 交易将把您的 `Prelaunch` 验证者推进为 `Staking`
验证者。此时您的 `Staking` 验证者已：
