...
如果您在 Smart Node 配置过程中选择启用指标,您的节点将添加以下进程:
...
默认配置将创建包含所有这些服务的 Docker 容器,这些容器与 Smart Node 的其余 Docker 容器一起运行。
...
<div className="p-3">
  <Tabs>
    <Tab label="Docker">
      ...
      在此检查期间,它还会将您安装的 Rocket Pool Smart Node 版本与最新版本进行比较,如果有新版本可用,它会通知您。
...
当提示在 **Import via grafana.com** 框中输入仪表板 ID 时,输入 `24900` 或使用完整 URL ([https://grafana.com/grafana/dashboards/24900-rocket-pool-dashboard-v1-4-0/](https://grafana.com/grafana/dashboards/24900-rocket-pool-dashboard-v1-4-0/)),然后按 **Load** 按钮。
...
::: tip 注意
Smart Node v1.23.0 随其指标堆栈一起提供 **Rocket Pool Dashboard v1.4.2**。该版本新增了按状态划分的节点级 minipool 和 megapool 验证者数量，并移除了依赖已删除的 supply-collector 指标的全网 megapool 状态面板。如果您的 Grafana 已经导入了旧版仪表板，请重新导入（或加载随 Smart Node 提供的 JSON），以便这些面板与您的节点导出的指标相匹配。
:::
...
它们将在可以正确跟踪历史奖励的 Smart Node 未来版本中再次启用。
:::
...
如果您在 Smart Node 安装期间没有将 Docker 的默认位置更改为其他驱动器,那么您要跟踪的磁盘将是安装操作系统的磁盘。
