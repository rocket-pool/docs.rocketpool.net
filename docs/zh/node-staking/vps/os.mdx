...
在 **Add Tags** 步骤中,您应该单击 **click to add a Name tag** 按钮并为其提供一个易于记住的名称,以便以后用于识别服务器;如果您需要灵感,可以使用 **Rocket Pool Smart Node** 之类的名称。
完成后单击 **Next**。

在 **Security Group** 部分,保持选中 **Create a new security group** 选项。
输入名称和描述,例如 **Smart Node** 和 **Smart Node Group**。
删除表中的所有现有规则并添加以下规则:
...
给它一个易于记忆的名称,例如 **Smart Node Key**,然后单击 **Download Key Pair**。
...
将资源类型保持设置为 **Instance**,然后从列表中选择您刚刚创建的 Rocket Pool Smart Node 实例。
