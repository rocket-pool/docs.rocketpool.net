...
此信息被编译成[默克尔树](https://en.wikipedia.org/wiki/Merkle_tree) - 一种极其高效的方式，使所有详细信息可供智能合约使用。默克尔树被构建成 JSON 文件并托管在[我们维护的 GitHub 存储库](https://github.com/rocket-pool/rewards-trees/)上。Smartnode 还可以从该 GitHub 存储库加载奖励周期**事件**，因此即使激进的历史过期已删除旧的执行层日志，您仍然可以领取奖励。
