...
为了代表节点质押，必须将地址列入白名单。您的提款地址始终列入白名单，如果您的 RPL 由提款地址持有，则可以跳过此步骤。您只需将地址列入白名单一次即可从中质押。您可以通过以下 Smart Node 命令执行此操作：
...
如果您想从代表质押白名单中删除地址，可以使用以下 Smart Node 命令执行此操作：
...
默认情况下，当您创建新的 minipool 时，Rocket Pool 将为其生成随机唯一地址。但是，Smart Node 提供了为 minipool 搜索自定义**虚荣地址**的能力。
...
[了解有关如何确认成功质押的更多信息](./create-validator#确认成功质押)
...
您首先会看到一条注释，说明存入新 minipool 将**自动分配**节点的[费用分配器](./fee-distrib-sp)合约中的任何余额（如果您未选择加入 [Smoothing Pool](./fee-distrib-sp#平滑池)，则用于捕获 MEV 奖励）：
