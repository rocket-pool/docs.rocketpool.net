...
::: warning 注意
如果你在本地使用 Smart Node（通过直接连接键盘和显示器物理登录），那么**本节与你无关 - 你可以跳过它**。
:::

大多数 Smart Node 运营者通过使用 `ssh` 从另一台计算机连接到其终端来远程与节点交互：

- 你连接**到**的机器（在本例中为你的节点机器）称为**服务器**。
- 你连接**自**的机器（例如你的笔记本电脑、台式机，甚至手机）是**客户端**。

保护 Smart Node 最重要的事情之一就是**保持客户端机器的安全**。
...
无论你是在家运行 Smart Node 还是在远程数据中心使用 VPS，你很可能通过 SSH 访问它，或者即使你不使用它，SSH 也**已启用**。
...
通常，你的机器应该只接受执行客户端、共识客户端和 Smart Node 堆栈使用的端口上的网络流量。
...
以下命令将为你的 Smart Node 设置 `ufw` 的良好默认配置。
**在节点机器上运行这些命令**。
...
支持通过 QUIC 进行 P2P gossip 的 Smart Node 共识层客户端默认监听 UDP 端口 **8001**。如果您启用该功能，请在主机防火墙中开放该端口：
