...
Rocket Pool 的 Smart Node 安装程序可以将您的机器转换为完整的以太坊节点,因为它需要执行层和共识层客户端才能正常运行。

ETH1/ETH2 术语已被弃用。
在本指南的其余部分,这些链将被称为**执行层 (ETH1)** 和**信标链或共识层 (ETH2)**。

如果您已经在另一台机器上运行执行层和共识层客户端(例如,如果您已经在进行单独质押),那么请跳过本节,转到[使用外部客户端配置混合 Rocket Pool 节点](./install-modes#使用外部客户端的混合配置)部分。
...
对于想要快速启动和运行的用户,Smart Node 安装程序提供了 `Random Client` 选项,这可能是最佳选择。
对于有特定客户端选择的用户,我们提供在 Rocket Pool 安装过程中轻松选择的能力。
下面的选项有助于描述每个客户端,以便您在想要指定所需客户端时做出明智的决定。

## 执行层客户端

Rocket Pool 支持五种执行层客户端：**Geth**、**Besu**、**Nethermind**、**Reth** 和 **Erigon**。
...
[Erigon](https://erigon.tech/) 是使用 [Go](https://go.dev/) 编写的执行层客户端。它采用分阶段同步架构，并且可以在首次启动时通过 BitTorrent 拉取快照，如果 torrent 端口可达，初始同步会相对较快。

在设置管理器中，除了常用的 P2P 端口（`30303`）之外，Erigon 还会暴露一个 **Torrent Port**（默认 `42069`，TCP 和 UDP）。如果您希望快照同步和对等连接在局域网之外正常工作，请同时转发这两个端口。

Erigon 支持 **Rolling History Expiry**（删除大约一年以前的区块体和收据）。它**不**支持仅删除合并前历史的 History Expiry；如果您选择该模式，它会保留剩余的区块历史，并采用完整模式的状态修剪。如果您以**全节点**方式运行，它会保留收据。

与其他客户端一样，RAM 需求较为灵活，而磁盘质量很重要。请像为 Geth 的 `30303` 端口那样，在您的路由器（以及主机防火墙）中开放 torrent 和 P2P 端口。
...
| Geth       | 完整 | ~8 GB      |
| Besu       | 完整 | ~10 GB     |
| Nethermind | 完整 | ~7-15 GB   |
| Reth       | 完整 | ~14 GB     |
| Erigon     | 完整 | ~10 GB     |

_RAM 使用量数据来自 [eth-docker](https://ethdocker.com/Usage/ResourceUsage/)。实际使用量可能因配置和网络状况而异。_
...
这使其能够在嵌入式系统和资源受限的设备上表现良好。如果您计划在节点上运行总共 16GB RAM 的执行层和共识层客户端组合，建议使用 nimbus。
...
| 客户端     | RAM 使用量 | 同步时间           |
| ---------- | ---------- | ------------------ |
| Lighthouse | ~14 GB     | 使用检查点同步即时 |
| Lodestar   | ~12 GB     | 使用检查点同步即时 |
| Nimbus     | ~5 GB      | 使用检查点同步即时 |
| Prysm      | ~9 GB      | 使用检查点同步即时 |
| Teku       | ~10 GB     | 使用检查点同步即时 |
