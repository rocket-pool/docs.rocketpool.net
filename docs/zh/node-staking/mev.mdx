...
Rocket Pool Smart Node 默认直接捆绑了 MEV-Boost,使我们的 **Docker 模式**和**混合模式**节点运营者能够无缝利用它。
**原生模式**用户需要手动设置它。
...
节点运营者可以选择使用上面列出的一个或多个可信中继,但不能使用自定义(不受信任的)中继。
选择退出 MEV-boost 的节点运营者必须在启动 Smart Node 之前明确这样做。
...
<div className="p-3">
  <Tabs>
    <Tab label="Docker 模式">
      使用 Smart Node 的配置 TUI 配置 MEV-Boost 很容易。
      ...
      启用后,屏幕将如下所示(_截至 Smart Node v1.17.2_):
      ...
      - `Container Tag` 框对于手动升级 Smart Node 运行的 MEV-Boost 版本很有用,如果 Flashbots 发布了一个新的高优先级版本,您想在包含它的 Smart Node 更新发布之前使用它。
      - `Additional Flags` 框用于如果您想直接向 MEV-Boost 容器添加补充配置标志或参数。通常,它不会有用。

      启用 MEV-Boost 并启用您想要的中继后,只需保存并退出。
      Smart Node 将为您重新启动相关容器,并自动为您设置所有内容。
