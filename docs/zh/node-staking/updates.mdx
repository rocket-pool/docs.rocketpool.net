...
Rocket Pool 偶尔会发布新版本的 Smart Node 堆栈。
...
::: tip 提示
完成 Smart Node 升级后,Grafana 仪表板仍会指示有可用更新。
...
升级步骤取决于节点使用的模式。更新 Smart Node 的最佳方式是使用 CLI 内置的更新命令。从以下选项中选择:

<div className="p-3">
  <Tabs>
    <Tab label="CLI 更新命令(Docker 模式 - Linux/macOS)">

      要使用内置的更新命令更新 Smart Node,请运行:

      ```shell
      rocketpool update
      ```
      此命令将:
      1. 下载并验证最新的 CLI 二进制文件
      2. 可选地自动停止、升级并重启服务
      ...
      之后,Smart Node 将下载并验证最新的 CLI 二进制文件:
      ...
      y
      ```
      此时,您可以在所有交互式提示中选择 `y`,让 Smart Node 引导您完成其余步骤。
      完成后,您将看到一条确认升级成功的消息:
      ```
    ...
    <Tab label="手动更新 - Linux(Docker 或混合模式)">
      停止 Rocket Pool 服务:
      ...
      最后,检查版本以确保 CLI 和 Smart Node 堆栈都是最新的:
    ...
    <Tab label="手动更新 - Linux(原生模式)">
      <p className="rspress-directive warning">
        <p className="rspress-directive-title">注意</p>
        这只会更新 Smart Node 堆栈本身 - **不会**更新执行层或共识层客户端。
      ...
      下载新的 Smart Node CLI 并备份旧的 CLI 和守护程序二进制文件,以防需要恢复它们:
    ...
    <Tab label="手动更新 - macOS(Docker 或混合模式)">
      停止 Rocket Pool 服务:
...
每个新的 Smart Node 版本都会为 Execution、Consensus 和 Smart Node 容器提供固定的 Docker 镜像摘要（在 `~/.rocketpool/mainnet.env` 中以 `repo:tag@sha256:…` 的形式存在）。`rocketpool service install` 会覆盖这些官方固定文件。在某些情况下，您可能希望在等待新的 Smart Node 版本发布*之前*运行不同的客户端镜像。本节将向您展示如何操作。
...
<div className="p-3">
  <Tabs>
    <Tab label="Docker 模式">
      ...
      Smart Node 现在通过摘要固定客户端镜像和 Smart Node 镜像。
      在 TUI 中更改 **Container Tag** 仍然可以让您在下一个 Smart Node 版本发布之前运行不同的标签或摘要。
      执行 `rocketpool service install` 或 `rocketpool update` 后，`mainnet.env` 中的官方固定值会被替换；在您清除之前，TUI 中的覆盖设置仍然优先。
      ...
      对更改满意后,像往常一样保存并退出。
      Smart Node 将提供自动重启所有受影响的容器。

      ::: tip 注意
      官方固定值位于 `~/.rocketpool/mainnet.env` 中。请勿通过编辑该文件来做持久修改；安装时会覆盖它。可选的按网络覆盖应放在 `~/.rocketpool/testnet.env` 或 `~/.rocketpool/devnet.env` 中（只有您设置的键才会替换 mainnet 的固定值）。您在设置管理器中设置的 **Container Tag** 会覆盖该服务的固定值。保持默认值即可继续使用官方摘要。
      :::
    </Tab>
    <Tab label="原生模式">
