...
请注意,正如[示例数学](./create-validator.mdx)部分所示,14% 的 8-ETH minipool _仍然比_ 20% 的 16-ETH minipool 更有利可图,因此保持高佣金率并不是保留 16 ETH 债券的充分理由。
:::
...
一旦您选择了一个或多个要减少的 minipool,Smart Node 将检查这些 minipool 是否符合债券减少的资格。
...
此信用可用于免费[创建另一个 8-ETH minipool](./create-validator.mdx#存入-eth-并创建-minipool)(除 gas 外,无需节点钱包提供 ETH)!
