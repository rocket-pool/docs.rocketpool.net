...
- 您已质押足够的 RPL 来支持债券减少_后_所需的最低 RPL 抵押水平。
...
在债券减少期间,Rocket Pool **分配您的 minipool 的现有余额**,使用 minipool 债券减少前的债券和佣金,以确保您和 rETH staker 都能获得现有余额的公平份额,并且债券减少不会改变任何一方_本应_从该余额中获得的奖励。
...
- 另一方面,_节点运营者_的份额不会被发送到节点运营者的提款地址。它被标记为**可用退款**,这就是为什么退款金额从 0 ETH 增加到之前 minipool 在 EL 上余额的"您的份额"金额。
  - 您可以随时使用以下命令领取此退款:
    ```shell
