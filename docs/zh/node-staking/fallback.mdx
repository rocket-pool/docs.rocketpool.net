...
从Smart Node stack的**1.5.0**版本开始,您可以提供一对"备用"Execution客户端和Consensus客户端,在您的主客户端离线时接管(例如[您使用Geth并需要裁剪它](./pruning))。
...
一旦您节点的主客户端重新上线,Smart Node和您的Validator客户端将自动切换回主客户端。
...
此示例向您展示如何使用Docker模式在不同的机器上创建第二个Smart Node,它可以作为备用节点。
...
2. 一旦机器准备就绪,[安装Smart Node stack](./docker)。
...
5. 可选勾选 **Prefer Fallback Clients**（默认关闭）。启用后，Smart Node 会优先将 API、节点和 watchtower 请求发送到您的备用 Execution 和 Consensus 客户端，只有在备用客户端不可用时才使用主客户端。这样可以减轻主客户端上的 Smart Node 负载，使其能够专注于验证者职责。您的验证者客户端不受影响，仍会优先使用主客户端。
...
请注意,这只会为**Smart Node本身**(守护进程服务)提供备用支持;**您必须手动更新Validator客户端服务的参数才能让它访问备用客户端。**
:::
