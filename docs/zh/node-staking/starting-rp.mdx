...
此时,您应该已经运行了完整的 Rocket Pool 基础设施,包括 Smart Node 堆栈、执行层和共识层客户端。
...
<div className="p-3">
  <Tabs>
    <Tab label="Docker(默认)模式">
      ...
      如果是这样,那么 Smart Node 堆栈已成功初始化并正在运行。
    ...
    <Tab label="原生模式">
      在原生模式下,您已经在[配置不使用 Docker 的原生 Rocket Pool 节点](./native)部分中设置并启动了 Smart Node 服务。
...
第一行将告诉您 Smart Node 是配置为以太坊主网还是 Hoodi 测试网。
...
现在 Smart Node 服务已启动,最好检查它们是否正常运行而没有错误。
  ...
  <Tabs>
    <Tab label="Docker(默认)模式">
      ...
      <p className="rspress-directive warning">
        ...
        这没关系!
        验证者容器将失败,直到使用 `rocketpool node deposit` 命令创建了新的验证者,我们将在[创建 Megapool（验证者）](/node-staking/megapools/create-megapool-validator)部分介绍。
    ...
    <Tab label="原生模式">
      在原生模式下检查状态和日志的最简单方法是使用您首次设置 Smart Node 服务时创建的日志记录脚本。
