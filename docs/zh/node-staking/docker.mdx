...
在本节中，我们将逐步介绍使用基于标准 [Docker](https://www.docker.com/resources/what-container) 的设置来安装 Rocket Pool Smart Node 堆栈的过程。
这将安装和配置运行完整节点所需的一切，包括：

- Rocket Pool **Smart Node** 软件
...
1. 使用 CLI 安装 Smart Node 堆栈
1. 在终端中使用易于使用的 UI 配置 Smart Node 堆栈
1. 完成！
...
Ubuntu 安装程序从 snap 软件包管理器获取 docker，这将与 Smart Node 安装的 docker 版本冲突。
...
现在您已经安装了 CLI，可以部署 Smart Node 堆栈了。
这将使用 Docker、[docker-compose](https://docs.docker.com/compose/) 准备您的系统，并加载 Smart Node 文件，使它们准备就绪。
它实际上还不会运行任何东西；那将在稍后进行。

要部署 Smart Node 堆栈，您需要在节点机器上运行以下命令（通过本地登录或通过 SSH 等方式远程连接）：

```shell
rocketpool service install
```

这将获取最新版本的 Smart Node 堆栈并进行设置。
...
::: warning 注意
请注意，Smart Node 安装程序无法在所有平台上自动安装 `docker` 和 `docker-compose`。
如果您在安装过程中收到这样的错误消息：
