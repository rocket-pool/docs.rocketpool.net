...
Layer 2由多个独立网络组成,这些网络位于Ethereum之下,像Ethereum通常那样执行大量交易,但将所有交易汇总到一个"超级"交易中,然后将*该交易*发布到主Ethereum链上。
...
::: warning 注意
在使用 MetaMask 进行任何交易之前,*始终*通过打开 MetaMask 并查看顶部的网络下拉菜单来验证它当前使用的是哪个网络!
很容易意外切换到不同的网络并忘记!
:::
