...
::: danger 警告
CoW Swap 路线不属于 Rocket Pool 协议的一部分。报价和求解器由**第三方**系统决定，这些系统不在 Rocket Pool 协议的控制或可见范围之内。用户应注意，使用 CoW Swap 可能会使其面临额外风险，如滑点、价格影响、求解器错误路由资金等。为了精细控制，
我们强烈建议您直接使用 CoW Swap。
:::
  ...
  <Tabs>
    <Tab label="在 Hoodi 测试网上准备">
      ...
      **Token Symbol** 应该自动填充为 `rETH`，**Decimals of Precision** 应该自动填充为 `18`。
      ...
      前往[使用测试网络进行练习](../node-staking/testnet/overview#在-hoodi-上获取测试-eth)页面，快速了解如何使用测试网水龙头在 Hoodi 上获取一些测试 ETH。
