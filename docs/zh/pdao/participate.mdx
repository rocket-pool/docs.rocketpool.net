...
请在继续之前阅读[提案的生命周期](./pdao#lifecycle-of-a-pdao-proposal)。它将解释所有投票期之间的差异以及在每个期间可以采取的行动。
...
- 如果您回复 `delegate`，您将有机会通过在同一笔交易中初始化投票和[设置委托人](./participate#delegating-voting-power)来节省一些 gas。
...
- 包含在快照中（通过[初始化投票](./participate#initializing-voting)或在 Houston 后注册）
- 必须至少有一个 minipool
- 具有非零投票权
- 已允许 [RPL 锁定](./participate#allowing-rpl-locking)
- 拥有大于提案保证金的 RPL 质押（减去任何已锁定的 RPL）

提案的存在是为了在协议层面更改参数和执行代码！在链上创建提案之前，应该通过[治理](./participate#governance-process)流程进行讨论并达成共识。
...
在此包含在区块中后，将创建 pDAO 提案！提案将在创建时进入[投票延迟期](./pdao#vote-delay-period)。
...
<div className="p-3">
  <Tabs>
    <Tab label="查看提案列表">
      ...
      在这里，我们可以看到我们的提案 `invite test-member` 的 ID 为 71，处于待处理状态。在此状态下，[挑战者](/zh/pdao/pdao#challenge-process)可以对提案者提供的 merkle pollard（用于计算投票权）的有效性提出异议。当 `proposal.vote.delay.time` 结束时，提案将过渡到活跃投票阶段。请随时查看[提案的生命周期](./pdao#lifecycle-of-a-pdao-proposal)以进行复习。
    </Tab>
    <Tab label="查看提案详情">
