...
Smart Node 将自动处理在您的执行客户端和共识客户端上设置身份验证令牌和引擎 API。
...
当您升级到 v1.5 时，Smart Node 将处理将其设置为正确的地址，并将不断检查以确保您使用的是正确的地址，这样您就不会[意外受到惩罚](https://github.com/rocket-pool/rocketpool-research/blob/master/Penalties/penalty-system)。
...
Rocket Pool 在 Smart Node 中内置了 MEV-Boost，并自动配置您的节点以使用它，因此您的提议获得最大的奖励。
