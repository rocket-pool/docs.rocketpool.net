...
Smartnode 堆栈的先前版本允许您在本地管理一个客户端，而外部管理另一个客户端。
例如，您可以拥有一个 Smartnode 管理的执行客户端，并将其连接到您外部管理的共识客户端。

从 v1.5 开始，不再支持这种配置。
您必须切换到本地管理的执行客户端和共识客户端（也称为 [Docker 模式](./docker-migration.mdx)），或者设置您自己管理的执行客户端和共识客户端。

::: tip 提示
如果您希望让 Smartnode 维护自己的执行客户端和共识客户端，但想保持对自己验证器客户端的控制（例如，如果您有自己的单独质押验证器密钥附加到它），您可能想要考虑 [反向混合模式](../../node-staking/advanced-config#允许外部验证者客户端连接到-smartnode)，它正好做到这一点！
:::
...
在混合模式下，一旦您更新到 v1.5.0，Smartnode 将自动处理支持 Redstone 所需的一些更改，但在混合模式下您需要手动处理其他更改。
...
当您升级到 v1.5 时，Smartnode 将处理将其设置为验证器客户端上的正确地址，并将不断检查以确保您使用的是正确的地址，这样您就不会[意外受到惩罚](https://github.com/rocket-pool/rocketpool-research/blob/master/Penalties/penalty-system)。
