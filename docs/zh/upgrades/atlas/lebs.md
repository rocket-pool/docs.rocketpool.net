...
前者代表了协议的最低保证金金额,因为它保证如果节点运营商使用 Rocket Pool 攻击以太坊协议并且其*整个保证金*被罚没,他们将损失与 rETH 质押者一样多,并且不会获利。
...
要创建 8-ETH minipool,节点运营商只需要提供**8 个自己的 ETH**(加上足够的 RPL 来满足抵押要求 - 有关详细信息,请参阅 [RPL 抵押品](#rpl-抵押品))。
...
- 如何在不退出的情况下将*现有的* 16-ETH minipool 迁移到 8-ETH minipool
...
从盈利能力的角度来看(纯粹看 ETH 奖励并忽略 RPL),佣金为 14% 的 8-ETH 保证金 minipool 为节点运营商提供*更多奖励*,甚至超过*佣金为 20% 的 16-ETH 保证金 minipool*(截至 Redstone,这是最高可能的奖励配置)。
同时,由于节点运营商更有效地利用 rETH 持有者的资本,它们也为 *rETH 持有者*提供更多奖励。
...
- 每个 minipool 的**最低 RPL** 是***借入*金额的 10%**
- 每个 minipool 的**最高 RPL** 是***保证金*金额的 150%**
