...
Smart Node будет предоставлять вам индикаторы состояния о своём прогрессе по ходу дела, как вы можете видеть в примере ниже.
:::
...
::: danger ПРИМЕЧАНИЕ
Хотя это доказывает, что вы можете построить дерево, вы _должны_ убедиться, что ваш токен API Web3.Storage был введён в конфигурацию Smart Node, чтобы он мог загрузить результирующее дерево в IPFS.
:::
