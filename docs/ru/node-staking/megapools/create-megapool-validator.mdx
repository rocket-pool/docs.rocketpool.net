...
Если валидатор в состоянии prelaunch не выполнит стейкинг в течение 365 дней, валидатор будет аннулирован. 1 ETH (из залога в 4 ETH), отправленный в Beacon Chain в процессе prelaunch, **не подлежит возврату**.
Оператор ноды получает оставшиеся 3 ETH из своего залога (за вычетом штрафа за аннулирование в 0,05 ETH, применяемого как долг) в качестве возврата в свой мегапул, с чистым возвратом в 2,95 ETH.
Это можно получить с помощью `rocketpool megapool claim` или `rocketpool claims claim-all`.
...
После того как ваш `Prelaunch` валидатор будет обработан Beacon Chain и зачтён первоначальный депозит в 1 ETH, ваша нода автоматически выполнит транзакцию `stake` для завершения полного депозита в 32
ETH в Beacon Chain. Транзакция `stake` переведёт ваш `Prelaunch` валидатор в состояние `Staking`. На этом этапе ваш `Staking` валидатор: - Имеет 32 ETH, депонированных в Beacon Chain - Получил
индексный номер валидатора - Ожидает активации в Beacon Chain
