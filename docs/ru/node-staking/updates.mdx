...
## Обновление стека Smart Node

Иногда Rocket Pool выпускает новую версию стека Smart Node.
...
::: tip СОВЕТ
Когда вы завершите обновление Smart Node, панель Grafana все еще будет указывать, что доступно обновление.
...
Шаги для обновления зависят от того, какой режим использует ваш узел. Лучший способ обновить Smart Node — использовать встроенную команду обновления CLI. Выберите из вариантов ниже:

<div className="p-3">
  <Tabs>
    <Tab label="Команда обновления CLI (Docker Mode - Linux/macOS)">

      Чтобы обновить Smart Node с помощью встроенной команды обновления, выполните:

      ```shell
      rocketpool update
      ```
      Эта команда:
      1. Загрузит и проверит последний бинарный файл CLI
      2. При желании автоматически остановит, обновит и перезапустит сервис

      Следуйте интерактивным подсказкам, чтобы завершить обновление.
      ```
      ...
      Затем Smart Node загрузит и проверит последний бинарный файл CLI:
      ...
      y
      ```
      На этом этапе вы можете выбрать `y` во всех интерактивных подсказках, чтобы Smart Node провёл вас через остальные шаги.
      По завершении вы увидите сообщение, подтверждающее успешное обновление:
      ```
    ...
    <Tab label="Ручное обновление - Linux (Docker или Hybrid Mode)">
      ...
      Загрузите новый Smart Node CLI:
      ...
      Наконец, проверьте версию, чтобы убедиться, что CLI и стек Smart Node оба обновлены:
    ...
    <Tab label="Ручное обновление - Linux (Native Mode)">
      <p className="rspress-directive warning">
        <p className="rspress-directive-title">ПРИМЕЧАНИЕ</p>
        Это обновит только сам стек Smart Node - это **не** обновит ваши клиенты Execution или Consensus.
      ...
      Загрузите новый Smart Node CLI и создайте резервную копию старых бинарных файлов CLI и демона, если вам нужно вернуть их:
    ...
    <Tab label="Ручное обновление - macOS (Docker или Hybrid Mode)">
      Остановите службы Rocket Pool:
...
Каждый новый релиз Smart Node поставляется с закреплёнными digest'ами Docker-образов для контейнеров Execution, Consensus и Smart Node (`repo:tag@sha256:…` в `~/.rocketpool/mainnet.env`). Команда `rocketpool service install` перезаписывает эти официальные файлы закреплений. В некоторых случаях вы можете захотеть запустить другой образ клиента _до_ выхода нового релиза Smart Node. В этом разделе показано, как это сделать.
...
<div className="p-3">
  <Tabs>
    <Tab label="Docker Mode">
      ...
      Теперь Smart Node закрепляет образы клиентов и самого Smart Node по digest.
      Изменение **Container Tag** в TUI по-прежнему позволяет запустить другой тег или digest до следующего релиза Smart Node.
      После `rocketpool service install` или `rocketpool update` официальные закрепления в `mainnet.env` заменяются; переопределение из TUI всё равно имеет приоритет, пока вы его не уберёте.
      ...
      Smart Node предложит автоматически перезапустить все затронутые контейнеры.

      ::: tip ПРИМЕЧАНИЕ
      Официальные закрепления находятся в `~/.rocketpool/mainnet.env`. Не редактируйте этот файл для постоянных изменений — установка перезапишет его. Необязательные переопределения для отдельных сетей помещайте в `~/.rocketpool/testnet.env` или `~/.rocketpool/devnet.env` (закрепление mainnet заменяют только заданные вами ключи). **Container Tag**, заданный в менеджере настроек, переопределяет закрепление для этого сервиса. Оставьте значение по умолчанию, чтобы сохранить официальный digest.
      :::
    </Tab>
