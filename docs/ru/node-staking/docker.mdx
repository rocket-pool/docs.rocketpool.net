...
В этом разделе мы рассмотрим процесс установки стека Smart Node от Rocket Pool с использованием стандартной установки на основе [Docker](https://www.docker.com/resources/what-container).
Это установит и настроит все необходимое для запуска полного узла, включая:

- Программное обеспечение **Smart Node** от Rocket Pool
...
1. Использовать CLI для установки стека Smart Node
1. Настроить стек Smart Node с помощью простого в использовании интерфейса в терминале
1. Готово!
...
Установщик Ubuntu получает docker из менеджера пакетов snap, который будет конфликтовать с версией docker, установленной Smart Node.
...
## Установка стека Smart Node

Теперь, когда у вас установлен CLI, вы можете развернуть стек Smart Node.
Это подготовит вашу систему с Docker, [docker-compose](https://docs.docker.com/compose/) и загрузит файлы Smart Node, чтобы они были готовы к работе.
Это еще не запустит ничего; это произойдет позже.

Чтобы развернуть стек Smart Node, вам нужно будет выполнить следующую команду на вашей машине узла (либо войдя в систему локально, либо подключившись удаленно, например, через SSH):

```shell
rocketpool service install
```

Это загрузит последнюю версию стека Smart Node и настроит его.
...
::: warning ПРИМЕЧАНИЕ
Обратите внимание, что установщик Smart Node не может автоматически установить `docker` и `docker-compose` на всех платформах.
...
После завершения стек Smart Node будет готов к запуску.
