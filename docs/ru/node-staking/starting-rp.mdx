...
На этом этапе у вас должна быть запущена полная инфраструктура Rocket Pool, включая стек Smart Node, клиенты Execution и Consensus.
...
<div className="p-3">
  <Tabs>
    <Tab label="Режим Docker (по умолчанию)">
      Теперь, когда у вас установлен Smart Node, запустить стек легко.
      ...
      Если это так, то стек Smart Node был успешно инициализирован и теперь работает.
    ...
    <Tab label="Нативный режим">
      В нативном режиме вы уже настроили и запустили сервисы Smart Node в рамках раздела [Настройка нативной ноды Rocket Pool без Docker](./native).
...
Первая строка сообщит вам, настроен ли ваш Smart Node для основной сети Ethereum или для тестовой сети Hoodi.
...
Теперь, когда сервисы Smart Node запущены, хорошей идеей будет проверить, что они работают правильно без ошибок.
  ...
  <Tabs>
    <Tab label="Режим Docker (по умолчанию)">
      ...
      <p className="rspress-directive warning">
        ...
        Это нормально!
        Контейнер валидатора будет давать сбой до тех пор, пока не будет создан новый валидатор с помощью команды `rocketpool node deposit`, которую мы рассмотрим в разделе [Создание мегапула (валидатора)](/node-staking/megapools/create-megapool-validator).
    ...
    <Tab label="Нативный режим">
      Самый простой способ проверить статус и логи в нативном режиме - использовать скрипты логирования, которые вы создали, когда впервые настраивали сервисы Smart Node.
