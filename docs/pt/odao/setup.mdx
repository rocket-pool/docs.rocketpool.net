...
- O Smart Node CLI
- O daemon da API do Smart Node
- O daemon `node` do Smart Node
- O daemon `watchtower` do Smart Node
...
Cada um desses componentes pode ser executado como contêineres Docker na configuração padrão, como serviços `systemd` na configuração "Modo Nativo", ou executados como entidades gerenciadas externamente às quais o Smart Node pode se conectar através de suas respectivas rotas de API.
...
As outras etapas, como registro do nó, estabelecimento de uma boa postura de segurança, monitoramento da saúde e desempenho do nó e atualização do Smart Node após uma atualização, ainda se aplicam a você e devem ser revisadas cuidadosamente antes de prosseguir.
...
<div className="p-3">
  <Tabs>
    ...
    <Tab label="Arquivo de Configuração Direto">
      Abra o arquivo `user-settings.yml` no diretório do seu Smart Node (`~/.rocketpool` por padrão) em um editor de texto.
...
## Comandos do Smart Node do Oracle DAO

O grupo de comandos `odao` do Smart Node CLI é usado para interagir com os contratos e funções do Oracle DAO on-chain:
