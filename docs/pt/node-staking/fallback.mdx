...
A partir da versão **1.5.0** da stack do Smart Node, você pode fornecer um par de cliente de Execution "fallback" e cliente de Consensus que podem assumir o controle dos seus clientes primários se eles ficarem offline (como porque [você usa Geth e precisa fazer poda](./pruning)).
Nessa situação, sua máquina de nó primária ainda será responsável por atestar e propor blocos com as chaves de validador do seu megapool, mas se conectará a uma máquina externa para interagir com a camada de Execution e as cadeias Beacon.
...
Uma vez que os clientes primários do seu nó estejam novamente online, o Smart Node e seu cliente Validator voltarão para eles automaticamente.
...
Este exemplo mostra como criar um segundo Smart Node em uma máquina diferente usando o modo Docker, que pode servir como um nó de fallback.
...
2. Quando a máquina estiver pronta, [instale a stack do Smart Node](./docker).
3. Execute `rocketpool service config` para especificar quais clientes você gostaria de usar.
   ...
   6. Salve as configurações e inicie o Smart Node.
...
5. Opcionalmente, marque **Prefer Fallback Clients** (desativado por padrão). Se ativado, o Smart Node enviará as requisições de API, de nó e do watchtower primeiro para seus clientes de Execution e Consensus de fallback, e só usará o par primário se o fallback estiver indisponível. Isso mantém a carga do Smart Node longe dos clientes primários, para que possam se concentrar nas tarefas de validação. Seu Validator Client não é afetado e continua usando o par primário primeiro.
...
Observe que isso fornecerá apenas ao **Smart Node em si** (o serviço daemon) o suporte de fallback; **você terá que atualizar os argumentos do serviço do seu cliente Validator manualmente para dar a ele acesso aos clientes de fallback.**
:::
