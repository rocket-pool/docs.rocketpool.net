...
A Árvore de Merkle é construída em um arquivo JSON e hospedada em [um repositório GitHub que mantemos](https://github.com/rocket-pool/rewards-trees/). O Smartnode também pode carregar os **eventos** de intervalo de recompensas desse repositório do GitHub, então você ainda pode reivindicar depois que uma expiração agressiva de histórico tiver descartado logs antigos da camada de Execução.
