...
A Árvore de Merkle é construída em um arquivo JSON e hospedada em [um repositório GitHub que mantemos](https://github.com/rocket-pool/rewards-trees/). O Smart Node também pode carregar os **eventos** de intervalo de recompensas desse repositório do GitHub, então você ainda pode reivindicar depois que uma expiração agressiva de histórico tiver descartado logs antigos da camada de Execução.
...
- Se seu cliente Execution primário não for um node de arquivo, você pode adicionar uma URL para um node de arquivo separado na caixa `Archive-Mode EC URL`.
  ...
  - O EC de Arquivo será usado apenas para geração de árvore; não será usado para nenhuma outra tarefa do Smart Node.
...
Agora seu Smart Node construirá a árvore completamente independente do Oracle DAO, usando apenas os dados fornecidos por seus clientes Execution e Consensus.
...
Se você **não está** optado no Smoothing Pool, a porção da Execution-layer de suas recompensas de propostas de bloco (incluindo taxas de transação e recompensas MEV) será enviada para o contrato [Fee Distributor](./fee-distrib-sp#distribuidores-de-taxas-e-o-smoothing-pool) do seu node.
