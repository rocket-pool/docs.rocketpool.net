...
para fazer staking a partir dele. Você pode fazer isso através do seguinte comando Smart Node:
...
Se você quiser remover um endereço da sua whitelist de stake-on-behalf, você pode fazer isso com o seguinte comando Smart Node:
...
No entanto, o Smart Node fornece a capacidade de procurar um **endereço de vaidade** personalizado para o minipool.
...
[Saiba mais sobre como confirmar um staking bem-sucedido](/pt/node-staking/create-validator#confirmando-um-staking-bem-sucedido)
...
Você verá primeiro uma nota de que depositar um novo minipool **automaticamente distribuirá** qualquer saldo no contrato [fee distributor](./fee-distrib-sp) do seu node (usado para capturar recompensas MEV se você não estiver optado pelo [Smoothing Pool](./fee-distrib-sp#o-smoothing-pool)):
