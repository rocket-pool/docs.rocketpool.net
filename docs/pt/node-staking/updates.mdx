...
## Atualizando o Smart Node Stack

Ocasionalmente, Rocket Pool lançará uma nova versão do Smart Node stack.
...
::: tip DICA
Quando você completar a atualização do Smart Node, o dashboard Grafana ainda indicará que há uma atualização disponível.
...
As etapas para atualizar dependem de qual modo seu nó usa. A melhor forma de atualizar o Smart Node é usando o comando de atualização integrado da CLI. Selecione entre as opções abaixo:

<div className="p-3">
  <Tabs>
    <Tab label="Comando de atualização da CLI (Modo Docker - Linux/macOS)">

      Para atualizar o Smart Node usando o comando de atualização integrado, execute:

      ```shell
      rocketpool update
      ```
      Este comando irá:
      1. Baixar e verificar o binário mais recente da CLI
      2. Opcionalmente parar, atualizar e reiniciar o serviço automaticamente

      Siga os prompts interativos para concluir a atualização.
      ```
      ...
      Em seguida, o Smart Node vai baixar e verificar o binário mais recente da CLI:
      ...
      y
      ```
      Neste ponto, você pode selecionar `y` em todos os prompts interativos para deixar o Smart Node guiá-lo pelo restante das etapas.
      Ao terminar, você verá uma mensagem confirmando que a atualização foi bem-sucedida:
      ```
    ...
    <Tab label="Atualização manual - Linux (Docker ou Modo Híbrido)">
      ...
      Baixe o novo CLI do Smart Node:
      ...
      Finalmente, verifique a versão para ter certeza de que o CLI e o Smart Node stack estão ambos atualizados:
    ...
    <Tab label="Atualização manual - Linux (Modo Nativo)">
      <p className="rspress-directive warning">
        <p className="rspress-directive-title">NOTA</p>
        Isso atualizará apenas o próprio Smart Node stack - **não** atualizará seus clientes Execution ou Consensus.
      ...
      Baixe o novo CLI do Smart Node e faça backup dos binários antigos do CLI e daemon caso você precise trazê-los de volta:
    ...
    <Tab label="Atualização manual - macOS (Docker ou Modo Híbrido)">
      Pare os serviços do Rocket Pool:
...
Cada novo lançamento do Smart Node vem com digests de imagens Docker fixados para os containers Execution, Consensus e Smart Node (`repo:tag@sha256:…` em `~/.rocketpool/mainnet.env`). O `rocketpool service install` sobrescreve esses arquivos oficiais de fixação. Em alguns casos você pode querer executar uma imagem de cliente diferente _antes_ de esperar por um novo lançamento do Smart Node. Esta seção mostra como.
...
<div className="p-3">
  <Tabs>
    <Tab label="Modo Docker">
      ...
      O Smart Node agora fixa por digest as imagens dos clientes e do próprio Smart Node.
      Alterar **Container Tag** na TUI ainda permite que você execute uma tag ou digest diferente antes do próximo lançamento do Smart Node.
      Após `rocketpool service install` ou `rocketpool update`, as fixações oficiais em `mainnet.env` são substituídas; uma sobreposição feita na TUI continua prevalecendo até que você a remova.
      ...
      O Smart Node oferecerá reiniciar todos os containers afetados automaticamente.

      ::: tip NOTA
      As fixações oficiais ficam em `~/.rocketpool/mainnet.env`. Não edite esse arquivo para mudanças duradouras; a instalação o sobrescreve. Sobreposições opcionais por rede vão em `~/.rocketpool/testnet.env` ou `~/.rocketpool/devnet.env` (somente as chaves que você definir substituem a fixação da mainnet). Um **Container Tag** definido no Settings Manager sobrepõe a fixação para aquele serviço. Deixe-o no valor padrão para manter o digest oficial.
      :::
    </Tab>
