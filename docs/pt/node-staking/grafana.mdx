...
Se você optar por habilitar métricas durante o processo de configuração do Smart Node, seu nó adicionará os seguintes processos:
...
A configuração padrão criará contêineres Docker com todos esses serviços que vivem ao lado dos outros contêineres Docker do Smart Node.
...
<div className="p-3">
  <Tabs>
    <Tab label="Docker">
      ...
      Comece executando o comando de configuração do Smart Node novamente:
      ...
      Durante essa verificação, ele também comparará sua versão instalada do Rocket Pool Smart Node com a versão mais recente, e informará se há uma nova versão disponível.
...
Consulte as abas abaixo para o seu modo de instalação do Smart Node.
...
Quando solicitado para o ID do dashboard na caixa **Import via grafana.com**, digite `24900` ou use a URL completa ([https://grafana.com/grafana/dashboards/24900-rocket-pool-dashboard-v1-4-0/](https://grafana.com/grafana/dashboards/24900-rocket-pool-dashboard-v1-4-0/)) e pressione o botão **Load**.
...
::: tip NOTA
O Smart Node v1.23.0 inclui o **Rocket Pool Dashboard v1.4.2** com sua stack de métricas. Essa revisão adiciona contagens de validadores de minipool e megapool por status no escopo do node e remove os painéis de status de megapool em escala de rede que dependiam de métricas do supply-collector já removidas. Se o seu Grafana já importou um dashboard mais antigo, importe-o novamente (ou carregue o JSON provisionado com o Smart Node) para que esses painéis correspondam às métricas que o seu node exporta.
:::
...
Elas serão habilitadas novamente em uma versão futura do Smart Node que pode rastrear recompensas históricas adequadamente.
:::
...
Se você não mudou a localização padrão do Docker para um drive diferente durante sua instalação do Smart Node, então o disco que você quer rastrear será aquele em que seu Sistema Operacional está instalado.
