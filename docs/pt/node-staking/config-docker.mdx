...
# Configurando a Stack do Smart Node (Docker / Modo Híbrido)
...
Felizmente, o Smart Node foi projetado para ocultar toda essa complexidade, tornando-o rápido e fácil de configurar, enquanto ainda oferece a liberdade de personalizar tudo, se assim desejar.

Nesta seção, abordaremos os vários métodos para configurar o Smart Node se você estiver usando a **configuração baseada em Docker** ou uma **configuração Híbrida**, onde você se conecta a clientes de Execução ou Consenso gerenciados externamente (por exemplo, clientes que você gerencia fora do Smart Node para staking solo).
...
- Via [Interface do Gerenciador de Configurações](#configurando-via-gerenciador-de-configurações) - isso oferece acesso a todas as configurações do Smart Node para que você possa personalizar tudo o quanto quiser.
- Sem interface via [Linha de Comando](#configurando-via-linha-de-comando) - esta é uma opção para pessoas que executam o Smart Node em um ambiente sem interface (não interativo) e precisam configurá-lo automaticamente.
...
::: tip NOTA
Se você já configurou o Smart Node, em vez disso, você será recebido com o [Gerenciador de Configurações](#configurando-via-gerenciador-de-configurações).
...
::: tip NOTA
As redes oficiais (Mainnet, Hoodi Testnet e Devnet) vêm de `networks-default.yml`. Você pode adicionar ou substituir redes em `~/.rocketpool/networks-extra.yml` sem recompilar o Smart Node. Consulte [Redes personalizadas e de substituição](./advanced-config#redes-personalizadas-e-de-substituição) para mais detalhes.
:::
...
Use-o se você ainda não tiver um par de clientes e quiser que o Smart Node gerencie um para você.
Ao escolher isso, o Smart Node criará, configurará e gerenciará um par de cliente de Execução e Consenso como contêineres Docker.
...
Ao escolher isso, o Smart Node simplesmente se conectará aos seus clientes existentes e não executará um próprio.
...
<div className="p-3">
  <Tabs>
    <Tab label="Gerenciado Localmente">
      Se você quiser que o Smart Node gerencie um cliente de Execução para você, a próxima tela pedirá que você escolha um cliente:

      <img src={ECSelection} width="100%" height="auto"/>

      Consulte a seção [Escolhendo seus clientes ETH](/pt/node-staking/eth-clients#clientes-de-execution) para uma descrição de cada opção.
      Depois de fazer sua escolha, clique na aba apropriada abaixo para aprender como configurá-lo:

      Se você escolher **Geth**, **Besu**, **Nethermind**, **Reth** ou **Erigon**, o Assistente cuidará de toda a configuração para você.
      ...
      Se você escolher **Erigon**, encaminhe também a **Torrent Port** dele (padrão **42069**, TCP e UDP) além da porta P2P **30303**, para que a sincronização por snapshot e o peering funcionem. O Assistente continua usando padrões sensatos para todo o resto.
      ...
      <p className="rspress-directive warning">
        <p className="rspress-directive-title">NOTA</p>
        Como o Smart Node será executado em seu próprio contêiner Docker, ele usará a rede interna do Docker.
        Você não poderá usar nomes de host como `localhost` ou `127.0.0.1` aqui; se o seu cliente de Execução estiver rodando na mesma máquina que o Smart Node, você precisará fornecer o endereço IP LAN da máquina.
      </p>
    ...
    <Tab label="Externo">
      ...
      O Smart Node usará a URL HTTP para se comunicar com ele e executar atividades de blockchain, como consultar o estado da cadeia e enviar transações.
      **Se você ainda não tiver a porta da API do seu cliente habilitada e acessível a partir da sua máquina Smart Node, você precisará configurá-la agora.**
      ...
      A URL do Websocket só é relevante se você precisar dela para seu próprio uso; o Smart Node não a utiliza.
      Você pode deixá-la em branco se não precisar dela.

      <p className="rspress-directive warning">
        <p className="rspress-directive-title">NOTA</p>
        Como o Smart Node será executado em seu próprio contêiner Docker, ele usará a rede interna do Docker.
        Você não poderá usar nomes de host como `localhost` ou `127.0.0.1` aqui; se o seu cliente de Execução alternativo estiver rodando na mesma máquina que o Smart Node, você precisará fornecer o endereço IP LAN da máquina.
      </p>
...
<div className="p-3">
  <Tabs>
    <Tab label="Gerenciado Localmente">
      Se você quiser que o Smart Node gerencie um cliente de Consenso para você, a próxima tela pedirá que você escolha um cliente:
      ...
      **A escolha preferida** para a saúde e diversidade geral da rede é a escolha **Aleatório (Recomendado)**, que escolherá aleatoriamente entre um dos quatro clientes de Consenso suportados para você.
      ...
      Se você preferir escolher um cliente explícito, consulte a seção [Escolhendo seus clientes ETH](./eth-clients#clientes-de-consensus) para uma descrição de cada opção para que você possa tomar uma decisão informada.
        ...
        <Tabs>
          <Tab label="Lighthouse">
            ...
            **Observe que o comprimento máximo do graffiti é de 16 caracteres.**
            ...
            Veja [a seção abaixo sobre Checkpoint Syncing](#sincronização-de-checkpoint-da-beacon-chain) se você gostaria de usá-lo.
          ...
          <Tab label="Nimbus">
            ...
            **Observe que o comprimento máximo do graffiti é de 16 caracteres.**
            ...
            O Nimbus suporta gossip P2P sobre o [protocolo QUIC](https://en.wikipedia.org/wiki/QUIC). No Gerenciador de Configurações, defina **P2P QUIC Port** nas configurações do Nimbus (padrão **8001**, UDP), encaminhe essa porta para o seu node no port forwarding do seu roteador e abra-a no firewall do host, se você usar um (`ufw status`). Evite colisões com outros nodes na mesma máquina (ou com nodes de fallback / testnet).
          ...
          <Tab label="Prysm">
            ...
            **Note the maximum length of the graffiti is 16 characters.**
            ...
            See [the section below on Checkpoint Syncing](#sincronização-de-checkpoint-da-beacon-chain) if you'd like to use it.
    ...
    <Tab label="Gerenciado Externamente">
      Como cada cliente de Consenso tem comportamento ligeiramente diferente, o Smart Node precisa saber qual você está usando externamente para que possa adaptar seu próprio comportamento de acordo.
      ...
      <div className="p-3">
        <Tabs>
          <Tab label="Lighthouse">
            ...
            O Smart Node usará a URL HTTP para se comunicar com ele e executar atividades de blockchain, como consultar o estado da cadeia e enviar mensagens.
            **Se você ainda não tiver a porta da API habilitada e acessível a partir da sua máquina Smart Node, você precisará configurá-la agora.**
            Veja a [documentação do Lighthouse](https://lighthouse-book.sigmaprime.io/api-bn.html) para instruções sobre como fazer isso.

            <p className="rspress-directive warning">
              <p className="rspress-directive-title">NOTA</p>
              Como o Smart Node será executado em seu próprio contêiner Docker, ele usará a rede interna do Docker.
              Você não poderá usar nomes de host como `localhost` ou `127.0.0.1` aqui; se o seu cliente estiver rodando na mesma máquina que o Smart Node, você precisará fornecer o endereço IP LAN da máquina.
            </p>
          ...
          <Tab label="Prysm">
            ...
            **Se você ainda não tiver as portas da API habilitadas e acessíveis a partir da sua máquina Smart Node, você precisará configurá-las agora.**
...
A partir da versão 1.5.0 da stack do Smart Node, você pode fornecer um par de cliente de Execução e cliente de Consenso "alternativo" que pode assumir seus clientes primários se eles ficarem offline (como quando você usa Geth e precisa podá-lo).
...
A partir do Smart Node v1.7.0, MEV agora é _opt-out_, então sua configuração é apresentada como parte da configuração inicial, como você vê na próxima tela:
...
Após esta pergunta, você terminou de configurar o Smart Node!
Você verá o seguinte diálogo:

<img src="./images/tui-finished.png" width="100%" height="auto" />

Se você está satisfeito com sua configuração e está pronto para iniciar o Smart Node, clique em `Save and Exit` aqui e vá para a seção [Protegendo seu Node](./securing-your-node) em seguida.

Se você gostaria de revisar todas as configurações e personalizar muitas configurações adicionais que não foram incluídas no Assistente, clique em `Review All Settings` e vá para a [próxima seção](#configurando-via-gerenciador-de-configurações).
...
1. A **Lista de Categorias**, que permite detalhar as configurações de cada categoria da stack do Smart Node
...
3. O botão **Open the Config Wizard**, que levará você ao [Assistente de Configuração](#configurando-via-assistente) se você preferir essa interface
...
Por exemplo, aqui está a tela para a categoria **Smart Node e Taxas TX**:
...
Você é encorajado a explorar todas as configurações para aprender sobre elas e ver o que seu Smart Node pode fazer!
:::
...
Se você usar o Smart Node em um ambiente sem interface onde não pode interagir com a Interface de Terminal, pode configurar seu node através da linha de comando.
