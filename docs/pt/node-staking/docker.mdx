...
Nesta seção, vamos percorrer o processo de instalação da stack Smart Node do Rocket Pool usando a configuração padrão baseada em [Docker](https://www.docker.com/resources/what-container).
Isso instalará e configurará tudo o que você precisa para executar um node completo, incluindo:

- O software **Smart Node** do Rocket Pool
...
1. Usar a CLI para instalar a stack Smart Node
1. Configurar a stack Smart Node com uma interface de usuário fácil de usar no terminal
1. Pronto!
...
O instalador do Ubuntu obtém o docker do gerenciador de pacotes snap, que entrará em conflito com a versão do docker instalada pelo Smart Node.
...
## Instalando a Stack Smart Node

Agora que você tem a CLI instalada, pode implantar a stack Smart Node.
Isso preparará seu sistema com Docker, [docker-compose](https://docs.docker.com/compose/), e carregará os arquivos do Smart Node para que estejam prontos para uso.
Ainda não executará nada; isso vem depois.

Para implantar a stack Smart Node, você precisará executar o seguinte comando na sua máquina de node (fazendo login localmente ou conectando remotamente, como através de SSH):

```shell
rocketpool service install
```

Isso obterá a versão mais recente da stack Smart Node e a configurará.
...
::: warning NOTA
Note que o instalador do Smart Node não pode instalar `docker` e `docker-compose` em todas as plataformas automaticamente.
...
Quando isso for concluído, a stack Smart Node estará pronta para executar.
...
## Configurando a Stack Smart Node
