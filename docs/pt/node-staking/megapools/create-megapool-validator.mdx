...
Seu nó receberá um ticket de fila expressa de volta se você optar por [retirar seu validador da fila expressa de depósitos do Rocket Pool](./create-megapool-validator#remover-um-validador-da-fila-de-depósitos-do-rocket-pool).
...
Se um validador em prelaunch não conseguir fazer stake dentro de 365 dias, o validador será dissolvido. O 1 ETH (de um bond de 4 ETH) enviado à Beacon Chain durante o processo de prelaunch **não é recuperável**.
O operador do nó recebe os 3 ETH restantes do seu bond (menos uma penalidade de dissolução de 0,05 ETH aplicada como débito) como reembolso no seu megapool, para um reembolso líquido de 2,95 ETH.
Isso pode ser reivindicado usando `rocketpool megapool claim` ou `rocketpool claims claim-all`.

[Saiba mais sobre como confirmar um stake bem-sucedido](./create-megapool-validator#confirmando-um-stake-bem-sucedido)
...
O próximo prompt exibirá o status da [fila de depósitos do Rocket Pool](/pt/node-staking/megapools/create-megapool-validator#fila-de-depósitos-e-fila-expressa-do-rocket-pool).
...
Quando seu validador receber ETH da [fila de depósitos do Rocket Pool](/pt/node-staking/megapools/create-megapool-validator#fila-de-depósitos-e-fila-expressa-do-rocket-pool), ele será movido para o estado `Prelaunch`. Nesse ponto, 1 ETH do seu saldo de megapool será depositado
...
completo de 32 ETH na Beacon Chain. A transação `stake` fará seu validador `Prelaunch` progredir para um validador `Staking`. Nesse ponto, seu validador `Staking` tem:

- 32 ETH depositados na Beacon Chain
- Um número de índice de validador atribuído
- Ativação pendente na Beacon Chain
