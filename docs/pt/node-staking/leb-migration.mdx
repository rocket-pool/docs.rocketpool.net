...
Observe que, como demonstrado pela seção de [exemplo de cálculo](./create-validator.mdx), um minipool de 8 ETH a 14% é _ainda mais lucrativo_ do que um minipool de 16 ETH a 20%, então manter uma taxa de comissão alta não é uma razão convincente para reter um bond de 16 ETH.
:::
...
Depois de selecionar um ou mais minipools para reduzir, o Smart Node verificará se esses minipools são elegíveis para redução de bond.
...
Este crédito pode ser usado para [fazer outro minipool de 8 ETH](./create-validator.mdx#depositando-eth-e-criando-um-minipool) gratuitamente (nenhum ETH necessário da carteira do nó, exceto para gas)!
