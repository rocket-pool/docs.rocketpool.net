...
Por favor, leia o [ciclo de vida de uma proposta](./pdao#ciclo-de-vida-de-uma-proposta-pdao) antes de prosseguir. Ele explicará
...
- Se você responder com `delegate`, você terá a oportunidade de economizar um pouco de gas ao inicializar a votação
  e [definir um delegado](./participate#delegando-o-poder-de-voto) dentro da mesma transação.
...
A versão `1.13.8` do Smart Node introduz um novo recurso **Auto Inicializar Poder de Voto**, que foi projetado para inicializar automaticamente
o poder de voto em nós que ainda não o fizeram. O poder de voto inicializado automaticamente é auto-delegado.
Este recurso pode ser configurado nas configurações do Smart Node executando `rocketpool service config` e navegando para a seção \*
\*Smart Node e Taxas TX\*\*.
...
endereço de retirada primário. Você pode fazê-lo usando este comando no Smart Node:
...
- Incluído no snapshot (seja ao [inicializar a votação](./participate#inicializando-votação) ou ao
  registrar pós Houston)
- Deve ter pelo menos um minipool
- Tem poder de voto não-zero
- Permitiu [bloqueio RPL](./participate#permitindo-bloqueio-rpl)
...
através do processo de [governança](./participate#processo-de-governança) antes que uma proposta seja criada on chain.
...
COMANDOS:
   ...
   submit-batch, sb            Enviar uma única proposta que altera várias configurações da DAO do Protocolo a partir de um arquivo JSON
```

Cada um destes comandos irá solicitá-lo com uma lista de inputs para criar a sua proposta desejada. Os comandos de proposta de configuração aceitam `--to-json` para escrever ou anexar uma alteração de configuração a um arquivo JSON em vez de enviar uma transação. Quando o arquivo estiver pronto, envie-o com `rocketpool pdao propose submit-batch`.
...
[período de atraso de voto](./pdao#período-de-atraso-de-voto) após a criação.
...
<div className="p-3">
  <Tabs>
    <Tab label="Visualizando uma Lista de Propostas">
      ...
      [desafiantes](/pt/pdao/pdao#processo-de-desafio) podem disputar a validade da
      ...
      proposta](./pdao#ciclo-de-vida-de-uma-proposta-pdao) para uma atualização.
    </Tab>
