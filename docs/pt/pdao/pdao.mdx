...
Uma proposta deve ser prevista pelo [processo de governança](./participate#processo-de-governança) antes de acabar on-chain. Ela consiste em 4 Períodos, todos os quais são [parâmetros controláveis](https://rpips.rocketpool.net/RPIPs/RPIP-33#parameter-table) pDAO:
...
Depois que a proposta tiver passado pelos períodos de votação, o proponente pode [reivindicar](./participate#reivindicando-garantias-e-recompensas) sua garantia RPL bloqueada, a menos que a proposta tenha sido derrotada por um desafio ou vetada.
...
Qualquer nó pode participar no rastreamento e verificação da correção das propostas. Para optar por essa responsabilidade, use o comando `rocketpool service config`, navegue até o menu **Configurações de Smart Node e Taxa TX**, e marque a caixa `Habilitar Verificador de Proposta PDAO`.

<img src={proposal_checker} width="100%" height="auto" />

Quando esta configuração está habilitada, o nó verificará novas propostas, verificará sua correção e enviará desafios a propostas inválidas. O único pré-requisito é que o [Bloqueio RPL](./participate#permitindo-bloqueio-rpl) esteja habilitado.
...
Agora que a proposta está derrotada, nosso nó (o desafiante) pode [reivindicar](./participate#reivindicando-garantias-e-recompensas) sua garantia RPL original, bem como a garantia RPL do proponente como uma recompensa por derrotar uma proposta fraudulenta.
