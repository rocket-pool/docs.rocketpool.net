...
Para maior detalhe e fundamentação, consulte [tipos de propostas](https://rpips.rocketpool.net/RPIPs/RPIP-33#proposal-types). É importante entender que uma proposta da pDAO é uma entidade on-chain que existe para executar mudanças no nível do protocolo.

## Ciclo de vida de uma proposta da pDAO

Uma proposta deve ser prevista pelo [processo de governança](../houston/participate#processo-de-governança) antes de acabar on-chain. Consiste em 4 Períodos, todos eles [parâmetros controláveis](https://rpips.rocketpool.net/RPIPs/RPIP-33#parameter-table) pela pDAO:
...
4. Veto: O eleitor vota contra a proposta e também indica que considera a proposta como spam ou maliciosa.
```
...
Após a proposta ter passado pelos períodos de votação, o proponente pode [reivindicar](../houston/participate#reivindicando-bonds-e-recompensas) seu vínculo de RPL travado, a menos que a proposta tenha sido derrotada por um desafio ou vetada.
...
E é isso! Lembre-se de que todas as variáveis mencionadas acima são parâmetros controláveis pela pDAO. Clique [aqui](https://rpips.rocketpool.net/RPIPs/RPIP-33#parameter-table) para uma lista abrangente de cada parâmetro que a pDAO tem autoridade para alterar usando propostas on-chain.
...
Qualquer nó pode participar no rastreamento e verificação da correção das propostas. Para participar dessa responsabilidade, use o comando `rocketpool service config`, navegue até o menu **Smart Node and TX Fee Settings** e marque a caixa `Enable PDAO Proposal Checker`.

<img src={proposal_checker} width="100%" height="auto" />

Quando esta configuração está habilitada, o nó verificará novas propostas, verificará sua correção e enviará desafios para propostas inválidas. O único pré-requisito é que o [Bloqueio de RPL](../houston/participate#permitindo-bloqueio-de-rpl) esteja habilitado.
