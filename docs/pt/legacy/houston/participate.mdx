...
Por favor, leia o [ciclo de vida de uma proposta](../houston/pdao#ciclo-de-vida-de-uma-proposta-da-pdao) antes de prosseguir. Isso explicará
...
- Se você responder com `delegate`, terá a oportunidade de economizar um pouco de gas inicializando a votação
  e [definindo um delegado](../houston/participate#delegando-poder-de-voto) na mesma transação.
...
A versão `1.13.8` do Smart Node introduz um novo recurso **Auto Initialize Vote Power**, que foi projetado para inicializar automaticamente
o poder de voto em nós que ainda não o fizeram. O poder de voto inicializado automaticamente é auto-delegado.
Este recurso pode ser configurado nas configurações do Smart Node executando `rocketpool service config` e navegando até a seção **Smart Node and TX Fees**.
...
retirada primário. Você pode fazer isso usando este comando no Smart Node:
...
- Incluído no snapshotting (seja [inicializando a votação](../houston/participate#inicializando-a-votação) ou
  registrando-se após Houston)
- Deve ter pelo menos um minipool
- Tem poder de voto diferente de zero
- Permitiu [bloqueio de RPL](../houston/participate#permitindo-bloqueio-de-rpl)
...
através do processo de [governança](../houston/participate#processo-de-governança) antes que uma proposta seja criada on chain.
...
[período de atraso de voto](../houston/pdao#período-de-atraso-de-voto) após a criação.
...
<div className="p-3">
  <Tabs>
    <Tab label="Visualizando uma Lista de Propostas">
      ...
      [contestadores](/pt/legacy/houston/pdao#processo-de-desafio) podem contestar a validade do
      ...
      proposta](../houston/pdao#ciclo-de-vida-de-uma-proposta-da-pdao) para uma atualização.
    </Tab>
