...
[The default configuration of Rocket Pool using Docker](../node-staking/install-modes) handles all of the settings involved in using a testnet for you automatically.
There's nothing special you need to do.

If you're using [a different setup](../node-staking/install-modes) with an external execution or consensus client, or are running your clients [natively without Docker](../node-staking/install-modes), then you will need to set this up manually:
...
If you are planning to run a node, please go through the [Node Operator Guide](../node-staking/responsibilities) to set up a node and create a new wallet first.
You will be prompted to return here at the appropriate step.
:::
...
2. Register your node (see the [Preparing your Node](../node-staking/prepare-node) page for instructions)
...
To learn how to swap legacy RPL for new RPL, follow the steps in the [Creating a Minipool (Validator)](../node-staking/create-validator) guide.
