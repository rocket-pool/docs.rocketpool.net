...
진행하기 전에 [제안의 라이프사이클](./pdao#lifecycle-of-a-pdao-proposal)을 읽어보세요. 모든 투표 기간의 차이점과 각 기간 동안 수행할 수 있는 작업을 설명합니다.
...
- `delegate`로 응답하면 동일한 트랜잭션 내에서 투표를 초기화하고 [위임자를 설정](./participate#delegating-voting-power)하여 약간의 가스를 절약할 수 있습니다.
...
- 스냅샷에 포함됨 ([투표 초기화](./participate#initializing-voting) 또는 Houston 이후 등록)
- 최소 하나의 minipool 보유
- 0이 아닌 투표권 보유
- [RPL 잠금](./participate#allowing-rpl-locking) 허용
- RPL 스테이크(이미 잠긴 RPL 제외)가 제안 보증금보다 큼

제안은 프로토콜 수준에서 매개변수를 변경하고 코드를 실행하기 위해 존재합니다! 제안이 체인에 생성되기 전에 [거버넌스](./participate#governance-process) 프로세스를 통해 논의와 합의가 있어야 합니다.
...
이것이 블록에 포함되면 pDAO 제안이 생성됩니다! 제안은 생성 시 [투표 지연 기간](./pdao#vote-delay-period)에 들어갑니다.
