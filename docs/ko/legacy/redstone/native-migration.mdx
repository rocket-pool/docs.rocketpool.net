...
Smart Node 소프트웨어에 필요한 올바른 fee recipient를 사용하도록 Validator client를 설정하는 방법은 아래에서 보여드리겠습니다.
...
- Execution client, Consensus client, Validator client 및 Smart Node 데몬(`rp-node` 서비스)에 대한 로그 스크립트를 확인하여 모두 정상적으로 작동하고 오류가 없는지 확인합니다.
...
[개요 문서](./whats-new#fee-recipients-및-distributor)에 설명된 대로 다음 두 값 중 하나일 수 있습니다:

- Smoothing Pool에 **가입한 경우** **Smoothing Pool contract**의 주소여야 합니다. [공식 계약 페이지](/ko/protocol/contracts-integrations)에서 주소를 얻을 수 있습니다.
...
Native mode에서는 Smart Node 데몬 서비스인 `rp-node`를 사용하는 경우 Smart Node가 이를 관리하도록 하거나 데몬을 사용하지 않는 경우 직접 관리할 수 있습니다.

#### 데몬을 통한 자동 관리

Smart Node 데몬은 노드에 적합한 fee recipient를 자동으로 결정하고 변경되는 경우(예: Smoothing Pool에 가입 및 탈퇴) 관리합니다.
이것이 **가장 안전한** 옵션입니다. Smart Node는 항상 [벌금](https://github.com/rocket-pool/rocketpool-research/blob/master/Penalties/penalty-system)을 **방지하는** 값으로 설정되도록 보장합니다.
...
<div className="p-3">
  <Tabs>
    <Tab label="Lighthouse">
      ...
      이제 VC는 Smart Node 데몬이 관리하는 파일을 사용하며 fee recipient가 변경될 때마다 자동으로 재시작됩니다.
    </Tab>
    <Tab label="Nimbus">
      Validator Client 서비스를 수정하여 `ExecStart` 줄 _이전에_ 다음 줄을 추가합니다:
...
**Redstone이 배포되기 전에는** 사용 중인 네트워크의 rETH 주소를 사용하면 됩니다([공식 계약 페이지](/ko/protocol/contracts-integrations)에서 찾을 수 있습니다).
...
<div className="p-3">
  <Tabs>
    <Tab label="Lighthouse">
      ...
      - Redstone 업데이트가 배포되기 **전**의 [rETH address](/ko/protocol/contracts-integrations/#token-contracts) (예: Mainnet에서 `0xae78736Cd615f374D3085123A210448E74Fc6393`)
      - Redstone이 배포된 후 노드의 **fee distributor**이며, 계약 업그레이드가 발생하면 `rocketpool node status`로 검색할 수 있습니다
      - Smoothing Pool에 가입한 경우 [Smoothing Pool address](/ko/protocol/contracts-integrations/#주요-프로토콜-컨트랙트)
