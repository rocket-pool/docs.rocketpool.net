...
Docker mode에서 Smartnode는 v1.5.0으로 업데이트하면 Redstone 및 The Merge를 지원하는 데 필요한 대부분의 변경 사항을 자동으로 처리합니다.
...
Smartnode는 Execution 및 Consensus client 모두에서 인증 토큰 및 Engine API 설정을 자동으로 처리합니다.
...
Smartnode는 v1.5로 업그레이드할 때 올바른 주소로 설정하고 [실수로 벌금을 받지](https://github.com/rocket-pool/rocketpool-research/blob/master/Penalties/penalty-system) 않도록 올바른 주소를 사용하고 있는지 지속적으로 확인합니다.
...
Rocket Pool은 Smartnode에 MEV-Boost를 내장하고 이를 사용하도록 노드를 자동으로 구성하므로 제안이 최대 보상을 받습니다.
