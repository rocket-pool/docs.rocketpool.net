...
# [Docker Mode] Redstone 업데이트 및 The Merge 가이드

이 가이드는 **Docker Mode**를 사용하는 경우 Redstone 업데이트 및 The Merge를 위해 노드를 준비하는 데 필요한 모든 정보를 다룹니다.

## v1.5.0으로 업그레이드하기 전에 해야 할 일

Smart Node를 v1.5.0 이상으로 업그레이드하기 전에 다음 체크리스트를 확인하여 준비가 되었는지 확인하세요:

### 전체 Execution Client로 전환

The Merge는 자체 Execution client를 실행해야 하므로 Infura 또는 Pocket과 같은 원격 공급자를 더 이상 사용할 수 없습니다.
v1.5.0에는 더 이상 해당 공급자가 없으며 전체 Execution client를 선택할 때까지 스택을 시작할 수 없습니다.

이러한 변경으로 인해 v1.4를 사용하는 동안 전체 클라이언트로 전환하고 동기화가 완료되면 v1.5로 업그레이드해야 합니다.

Docker Mode는 클라이언트 전환을 매우 쉽게 만듭니다.
[이 가이드](../../node-staking/change-clients#execution-클라이언트-변경하기)에서 프로세스에 대한 안내를 제공합니다.

## v1.5.0으로 업그레이드

Smart Node 스택을 v1.5.0으로 업그레이드하는 것은 다른 업그레이드와 다르지 않습니다.
[여기의 일반 지침](../../node-staking/updates#smartnode-스택-업데이트)을 따르기만 하면 됩니다.

## Smart Node가 자동으로 처리하는 사항

Docker mode에서 Smart Node는 v1.5.0으로 업데이트하면 Redstone 및 The Merge를 지원하는 데 필요한 대부분의 변경 사항을 자동으로 처리합니다.
...
Smart Node는 Execution 및 Consensus client 모두에서 인증 토큰 및 Engine API 설정을 자동으로 처리합니다.

### Fee Recipient

**fee recipient**는 제안한 블록에 대한 모든 우선 수수료를 받을 Execution layer 체인의 주소입니다.
이는 Validator client가 처음 시작할 때 제공되는 설정입니다.

Smart Node는 v1.5로 업그레이드할 때 올바른 주소로 설정하고 [실수로 벌금을 받지](https://github.com/rocket-pool/rocketpool-research/blob/master/Penalties/penalty-system) 않도록 올바른 주소를 사용하고 있는지 지속적으로 확인합니다.

[Smoothing Pool](./whats-new#smoothing-pool)에 가입한 경우 해당 주소가 fee recipient가 됩니다.
그렇지 않은 경우 [fee distributor contract](./whats-new#fee-recipients-및-distributor)가 fee recipient가 됩니다.

### MEV-Boost

[MEV-boost](https://boost.flashbots.net/)는 Flashbots가 The Merge 이후 Proof-of-Stake validator에게 MEV 보상을 제공하는 시스템입니다.
Rocket Pool은 Smart Node에 MEV-Boost를 내장하고 이를 사용하도록 노드를 자동으로 구성하므로 제안이 최대 보상을 받습니다.

## 업그레이드 후 수행해야 할 작업

Smart Node가 대부분의 변경 사항을 처리하지만 수동으로 수행해야 하는 몇 가지 추가 작업이 있습니다:
...
- `rocketpool service logs eth1`, `rocketpool service logs eth2`, `rocketpool service logs validator`, `rocketpool service logs node`로 로그의 오류를 확인합니다.
...
Smart Node는 여전히 fallback Execution client(및 이제 fallback Consensus client도)를 제공할 수 있지만 이제 제어하는 Execution 및 Consensus client를 사용해야 합니다.

fallback node 설정에 대한 자세한 내용은 [Fallback node 가이드](../../node-staking/fallback)를 참조하세요.

### Fee Distributor 초기화

[Smoothing Pool](./whats-new#smoothing-pool)에 가입하지 않고 모든 우선 수수료 및 MEV 보상을 [fee distributor contract](./whats-new#fee-recipients-및-distributor)로 청구하려는 경우, 출금 주소로 보상을 청구하려면 결국 초기화(체인에 계약 인스턴스 생성)해야 합니다.
...
Redstone 업그레이드는 비용이 많이 들고 문제가 있는 기존 보상 시스템을 [새로운 시스템](./whats-new#새로운-보상-시스템)으로 교체합니다. 이 시스템은 훨씬 저렴하고 RPL의 자동 재스테이킹(부분 및 전체 금액 모두)을 지원하며 가장 중요한 것은 **원하는 때에 보상을 청구할 수 있다**는 것입니다.

보상 청구에 더 이상 시간 제한이 없고 많은 보상 기간을 한 번에 청구하는 것이 더 저렴하기 때문에 Smart Node의 자동 보상 청구 기능이 **제거되었습니다**.
...
어떤 이유로든 마음에 들지 않아 이전 Smart Node 릴리스로 되돌리고 싶다면 쉽게 할 수 있습니다.
Smart Node는 업그레이드 시 이전 버전의 설정을 자동으로 백업하므로 이전 버전(여기서는 **v1.4.3**을 시연합니다)을 가져와서 백업으로 설정을 교체하기만 하면 됩니다:
...
1. 문제가 없으면 Smart Node 스택을 시작합니다:
