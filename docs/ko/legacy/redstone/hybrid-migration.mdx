...
이전 버전의 Smart Node 스택에서는 하나의 클라이언트를 로컬로 관리하고 다른 클라이언트를 외부에서 관리할 수 있었습니다.
예를 들어, Smart Node가 관리하는 Execution client를 가지고 외부에서 관리하는 Consensus client에 연결할 수 있었습니다.
...
::: tip 팁
Smart Node가 자체 Execution 및 Consensus client를 유지하도록 하면서 자체 Validator client를 제어하고 싶다면(예: 자체 solo staking validator 키가 연결되어 있는 경우), 정확히 이를 수행하는 [Reverse Hybrid Mode](../../node-staking/advanced-config#외부-검증자-클라이언트의-smartnode-연결-허용)를 고려할 수 있습니다!
:::
...
Hybrid mode에서 Smart Node는 v1.5.0으로 업데이트하면 Redstone을 지원하는 데 필요한 일부 변경 사항을 자동으로 처리하지만 Hybrid Mode에서는 다른 것들을 수동으로 처리해야 합니다.
...
Smart Node는 v1.5로 업그레이드할 때 관리하는 Validator client에서 올바른 주소로 설정하고 [실수로 벌금을 받지](https://github.com/rocket-pool/rocketpool-research/blob/master/Penalties/penalty-system) 않도록 올바른 주소를 사용하고 있는지 지속적으로 확인합니다.
