...
Smart Node 소프트웨어는 정직한 사용자가 패널티를 받지 않도록 설계되었으며, 이를 위해 Validator 클라이언트를 오프라인으로 전환해야 하는 경우에도 그렇게 합니다.
이 경우 증명을 중지하고 Smart Node가 fee recipient를 올바르게 설정할 수 없는 이유에 대한 오류 메시지가 로그 파일에 표시됩니다.
