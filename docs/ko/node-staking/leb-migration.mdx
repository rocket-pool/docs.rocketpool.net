...
[예시 계산](./create-validator.mdx) 섹션에서 보여주듯이 14%의 8-ETH minipool이 20%의 16-ETH minipool보다 _여전히 더 수익성이 높으므로_ 높은 수수료율을 유지하는 것은 16 ETH 본드를 유지할 설득력 있는 이유가 아닙니다.
:::
...
감소할 하나 이상의 minipool을 선택하면 Smart Node가 해당 minipool이 본드 감소 자격이 있는지 확인합니다.
...
이 크레딧은 무료로 [다른 8-ETH minipool을 만드는 데](./create-validator.mdx#eth-예치-및-minipool-생성) 사용할 수 있습니다(가스비를 제외하고 노드 지갑에서 ETH가 필요하지 않음)!
