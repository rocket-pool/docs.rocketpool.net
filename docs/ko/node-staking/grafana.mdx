...
Smart Node 구성 과정에서 메트릭을 활성화하도록 선택하면 노드에 다음 프로세스가 추가됩니다:
...
기본 구성은 Smart Node의 나머지 Docker 컨테이너와 함께 이러한 모든 서비스가 포함된 Docker 컨테이너를 생성합니다.
...
<div className="p-3">
  <Tabs>
    <Tab label="Docker">
      ...
      Smart Node 구성 명령을 다시 실행하여 시작합니다:
      ...
      이 검사 중에 설치된 Rocket Pool Smart Node 버전을 최신 릴리스와 비교하고 새 릴리스가 있으면 알려줍니다.
...
Smart Node 설치 모드에 대한 아래 탭을 참조하세요.
...
**Import via grafana.com** 상자에서 대시보드 ID를 입력하라는 메시지가 표시되면 `24900`을 입력하거나 전체 URL([https://grafana.com/grafana/dashboards/24900-rocket-pool-dashboard-v1-4-0/](https://grafana.com/grafana/dashboards/24900-rocket-pool-dashboard-v1-4-0/))을 사용하고 **Load** 버튼을 누릅니다.
...
::: tip 참고
Smart Node v1.23.0에는 메트릭 스택과 함께 **Rocket Pool Dashboard v1.4.2**가 포함되어 있습니다. 이 리비전에서는 상태별 노드 단위 minipool 및 megapool 검증자 수가 추가되었으며, 제거된 supply-collector 메트릭에 의존하던 네트워크 전체 megapool 상태 패널이 삭제되었습니다. Grafana에 이미 이전 대시보드를 가져왔다면, 해당 패널이 노드가 내보내는 메트릭과 일치하도록 다시 가져오거나 Smart Node와 함께 제공되는 JSON을 로드하세요.
:::
...
역사적 보상을 올바르게 추적할 수 있는 Smart Node의 향후 버전에서 다시 활성화됩니다.
:::
...
Smart Node 설치 중에 Docker의 기본 위치를 다른 드라이브로 변경하지 않은 경우 추적하려는 디스크는 운영 체제가 설치된 디스크입니다.
