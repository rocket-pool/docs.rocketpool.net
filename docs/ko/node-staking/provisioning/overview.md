...
이 섹션에서는 Smart Node를 설치하고 구성한 후 Rocket Pool로 staking하기 위해 노드를 프로비저닝하는 방법에 대한 세부 사항을 다룹니다.
다룰 staking에 대한 정보가 많기 때문에 긴 섹션입니다. **첫 번째 메가풀 검증자를 생성하기 전에 각 가이드를 읽어주십시오!**
...
- Smart Node를 [설치](../installing/overview)하고 [구성](../config/overview)했습니다

## 가이드

[Starting Rocket Pool](../starting-rp)은 각 모드에 대한 Smart Node 서비스를 시작하는 방법과 Execution 및 Consensus client의 동기화 진행 상황을 확인하는 방법을 보여줍니다.

[Creating a New Wallet](../wallet-init)은 노드를 처음 설정하는 경우 Smart Node로 새 wallet을 생성하는 프로세스를 안내합니다.
...
[MEV, MEV-Boost, and MEV Rewards](../mev.mdx)는 **최대 추출 가능 가치(Maximum-Extractable Value)** (MEV), staking 생태계에서의 역할 및 Smart Node를 사용하여 원하는 대로 구성하는 방법을 설명합니다.
