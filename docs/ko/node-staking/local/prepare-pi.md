...
::: warning 참고
이 시점에서 라우터를 구성하여 Pi의 IP 주소를 **정적**으로 만드는 것을 _강력히 고려_해야 합니다.
...
1. Pi를 끕니다.
2. microSD 카드를 빼냅니다.
3. microSD 어댑터를 사용하여 카드를 다른 Linux 컴퓨터에 연결합니다.
   \*참고: **다른 Linux 컴퓨터여야 합니다**. Windows는 SD 카드가 사용하는 `ext4` 파일 시스템을 읽을 수 없으므로 Windows 컴퓨터에 연결해도 작동하지 않습니다!\*\*
