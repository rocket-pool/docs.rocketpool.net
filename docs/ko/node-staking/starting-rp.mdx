...
이 시점에서 Smart Node 스택, Execution 및 Consensus 클라이언트를 포함한 완전한 Rocket Pool 인프라가 실행되고 있어야 합니다.
...
<div className="p-3">
  <Tabs>
    <Tab label="Docker (Default) Mode">
      이제 Smart Node가 설치되었으므로 스택을 시작하는 것은 쉽습니다.
      다음 명령을 입력하기만 하면 됩니다:
      ...
      이와 같이 표시되면 Smart Node 스택이 성공적으로 초기화되어 현재 실행 중입니다.
    ...
    <Tab label="Native Mode">
      Native 모드에서는 [Configuring a Native Rocket Pool Node without Docker](./native) 섹션의 일부로 Smart Node 서비스를 이미 설정하고 시작했습니다.
...
첫 번째 줄은 Smart Node가 Ethereum 메인넷 또는 Hoodi 테스트넷으로 구성되어 있는지 알려줍니다.
...
이제 Smart Node 서비스가 시작되었으므로 오류 없이 올바르게 실행되고 있는지 확인하는 것이 좋습니다.
  ...
  <Tabs>
    <Tab label="Docker (Default) Mode">
      ...
      <p className="rspress-directive warning">
        ...
        괜찮습니다!
        validator 컨테이너는 [메가풀 생성 (검증자)](/node-staking/megapools/create-megapool-validator) 섹션에서 다룰 `rocketpool node deposit` 명령을 사용하여 새 validator가 생성될 때까지 실패합니다.
    ...
    <Tab label="Native Mode">
      Native 모드에서 상태와 로그를 확인하는 가장 쉬운 방법은 Smart Node 서비스를 처음 설정할 때 만든 로깅 스크립트를 사용하는 것입니다.
