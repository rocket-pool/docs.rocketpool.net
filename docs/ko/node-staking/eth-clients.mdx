...
Rocket Pool의 Smart Node 설치 프로그램은 머신을 전체 Ethereum 노드로 변환할 수 있습니다. 올바르게 작동하려면 Execution 및 Consensus 클라이언트가 모두 필요합니다.

ETH1/ETH2 용어는 더 이상 사용되지 않습니다.
이 가이드의 나머지 부분에서는 체인을 **Execution Layer (ETH1)** 및 **Beacon Chain 또는 Consensus Layer (ETH2)**라고 합니다.

별도의 머신에서 이미 Execution 및 Consensus 클라이언트를 실행 중인 경우 (예: 이미 solo-staking을 하고 있는 경우) 이 섹션을 건너뛰고 [외부 클라이언트를 사용한 하이브리드 Rocket Pool 노드 구성](./install-modes#외부-클라이언트를-사용한-하이브리드-구성) 섹션으로 이동하십시오.
...
빠르게 시작하고 실행하려는 사용자의 경우 Smart Node 설치 프로그램은 가장 좋은 선택일 수 있는 `Random Client` 옵션을 제공합니다.
...
Rocket Pool은 다섯 가지 Execution 클라이언트를 지원합니다: **Geth**, **Besu**, **Nethermind**, **Reth** 및 **Erigon**.
...
최소 RAM 요구 사항은 8 GB이지만 16 GB를 권장합니다.
...
[Erigon](https://erigon.tech/)은 [Go](https://go.dev/)로 작성된 Execution 클라이언트입니다. 단계별 동기화 아키텍처를 사용하며 첫 시작 시 BitTorrent를 통해 스냅샷을 가져올 수 있어, torrent 포트에 접근할 수 있다면 초기 동기화가 비교적 빠릅니다.

Settings Manager에서 Erigon은 일반적인 P2P 포트(`30303`) 외에 **Torrent Port**(기본값 `42069`, TCP 및 UDP)를 노출합니다. LAN 외부에서도 스냅샷 동기화와 피어링이 잘 작동하도록 하려면 두 포트를 모두 포워딩하세요.

Erigon은 **Rolling History Expiry**(약 1년보다 오래된 바디와 영수증 삭제)를 지원합니다. pre-merge 전용 History Expiry는 지원하지 **않습니다**. 해당 모드를 선택하면 남은 블록 히스토리를 full 모드 상태 가지치기와 함께 유지합니다. **풀 노드**로 실행하면 영수증을 유지합니다.

다른 클라이언트와 마찬가지로 RAM 요구 사항은 유연하며 디스크 품질이 중요합니다. Geth의 `30303`과 동일한 방식으로 라우터(및 호스트 방화벽)에서 torrent 및 P2P 포트를 개방하세요.
...
_RAM 사용량 데이터는 [eth-docker](https://ethdocker.com/Usage/ResourceUsage/)에서 가져왔습니다. 실제 사용량은 구성 및 네트워크 상태에 따라 달라질 수 있습니다._
...
이를 통해 임베디드 시스템 및 리소스 제한 장치에서도 잘 작동할 수 있습니다. 노드에서 총 16GB RAM으로 Execution 및 Consensus 클라이언트 쌍을 실행할 계획이라면 nimbus를 권장합니다.
...
| 클라이언트 | RAM 사용량 | 동기화 시간            |
| ---------- | ---------- | ---------------------- |
| Lighthouse | ~14 GB     | checkpoint sync로 즉시 |
| Lodestar   | ~12 GB     | checkpoint sync로 즉시 |
| Nimbus     | ~5 GB      | checkpoint sync로 즉시 |
| Prysm      | ~9 GB      | checkpoint sync로 즉시 |
| Teku       | ~10 GB     | checkpoint sync로 즉시 |
