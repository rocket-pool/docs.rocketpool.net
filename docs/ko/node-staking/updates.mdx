...
또한 짧은 시간 동안 오프라인 상태가 되어 *슬래싱*될 가능성은 **절대 없습니다**.
...
때때로 Rocket Pool은 Smart Node 스택의 새 버전을 릴리스합니다.
...
::: tip 팁
Smart Node 업그레이드를 완료하면 Grafana 대시보드에 여전히 업데이트를 사용할 수 있다고 표시됩니다.
...
업그레이드 단계는 노드가 사용하는 모드에 따라 다릅니다. Smart Node를 업데이트하는 가장 좋은 방법은 CLI에 내장된 업데이트 명령을 사용하는 것입니다. 아래 옵션에서 선택하세요:

<div className="p-3">
  <Tabs>
    <Tab label="CLI 업데이트 명령 (Docker Mode - Linux/macOS)">

      내장 업데이트 명령으로 Smart Node를 업데이트하려면 다음을 실행하세요:

      ```shell
      rocketpool update
      ```
      이 명령은 다음을 수행합니다:
      1. 최신 CLI 바이너리를 다운로드하고 검증합니다
      2. 선택적으로 서비스를 자동으로 중지, 업그레이드, 재시작합니다

      대화형 프롬프트를 따라 업데이트를 완료하세요.
      ```
      ...
      그런 다음 Smart Node가 최신 CLI 바이너리를 다운로드하고 검증합니다:
      ...
      y
      ```
      이 시점에서 모든 대화형 프롬프트에 `y`를 선택하면 Smart Node가 나머지 단계를 안내합니다.
      완료되면 업그레이드가 성공했음을 확인하는 메시지가 표시됩니다:
      ```
    ...
    <Tab label="수동 업데이트 - Linux (Docker or Hybrid Mode)">
      ...
      마지막으로 버전을 확인하여 CLI와 Smart Node 스택이 모두 최신 상태인지 확인합니다:
    ...
    <Tab label="수동 업데이트 - Linux (Native Mode)">
      <p className="rspress-directive warning">
        <p className="rspress-directive-title">참고</p>
        이것은 Smart Node 스택 자체만 업데이트합니다. Execution 또는 Consensus 클라이언트를 업데이트하지 **않습니다**.
      ...
      새 Smart Node CLI를 다운로드하고 필요한 경우 이전 CLI 및 데몬 바이너리를 백업합니다:
    ...
    <Tab label="수동 업데이트 - macOS (Docker or Hybrid Mode)">
      Rocket Pool 서비스를 중지합니다:
...
Smart Node의 각 새 릴리스에는 Execution, Consensus 및 Smart Node 컨테이너에 대해 고정된 Docker 이미지 다이제스트가 포함됩니다(`~/.rocketpool/mainnet.env`의 `repo:tag@sha256:…`). `rocketpool service install`은 이러한 공식 고정 파일을 덮어씁니다. 경우에 따라 새 Smart Node 릴리스를 기다리기 _전에_ 다른 클라이언트 이미지를 실행하고 싶을 수 있습니다. 이 섹션에서는 그 방법을 설명합니다.
...
<div className="p-3">
  <Tabs>
    <Tab label="Docker Mode">
      ...
      Smart Node는 이제 클라이언트 이미지와 Smart Node 이미지를 다이제스트로 고정합니다.
      TUI에서 **Container Tag**를 변경하면 다음 Smart Node 릴리스 전에도 다른 태그나 다이제스트를 실행할 수 있습니다.
      `rocketpool service install` 또는 `rocketpool update`를 실행하면 `mainnet.env`의 공식 고정값이 교체됩니다. TUI에서 지정한 재정의는 이를 지우기 전까지 계속 우선합니다.
      ...
      **Beacon Node Container Tag** 설정을 수정합니다:
      ...
      Smart Node는 영향을 받는 모든 컨테이너를 자동으로 재시작할 것을 제안합니다.

      ::: tip 참고
      공식 고정값은 `~/.rocketpool/mainnet.env`에 있습니다. 지속적인 변경을 위해 이 파일을 편집하지 마십시오. 설치 시 덮어써집니다. 선택적인 네트워크별 재정의는 `~/.rocketpool/testnet.env` 또는 `~/.rocketpool/devnet.env`에 넣습니다(설정한 키만 mainnet 고정값을 대체합니다). Settings Manager에서 설정한 **Container Tag**는 해당 서비스의 고정값을 재정의합니다. 공식 다이제스트를 유지하려면 기본값으로 두십시오.
      :::
    </Tab>
