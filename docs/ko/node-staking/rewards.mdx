...
Merkle Tree는 JSON 파일로 구축되어 [우리가 유지 관리하는 GitHub 저장소](https://github.com/rocket-pool/rewards-trees/)에 호스팅됩니다. Smartnode는 해당 GitHub 저장소에서 보상 인터벌 **이벤트**를 불러올 수도 있으므로, 공격적인 히스토리 만료로 오래된 Execution 레이어 로그가 삭제된 후에도 청구할 수 있습니다.
