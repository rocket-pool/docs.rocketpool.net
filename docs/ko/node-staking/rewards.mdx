...
Merkle Tree는 JSON 파일로 구축되어 [우리가 유지 관리하는 GitHub 저장소](https://github.com/rocket-pool/rewards-trees/)에 호스팅됩니다. Smart Node는 해당 GitHub 저장소에서 보상 인터벌 **이벤트**를 불러올 수도 있으므로, 공격적인 히스토리 만료로 오래된 Execution 레이어 로그가 삭제된 후에도 청구할 수 있습니다.
...
- 기본 Execution 클라이언트가 아카이브 노드가 아닌 경우 `Archive-Mode EC URL` 상자에 별도의 아카이브 노드 URL을 추가할 수 있습니다.
  ...
  - Archive EC는 트리 생성에만 사용됩니다. 다른 Smart Node 작업에는 사용되지 않습니다.
...
이제 Smart Node는 Execution 및 Consensus 클라이언트가 제공하는 데이터만 사용하여 Oracle DAO와 완전히 독립적으로 트리를 구축합니다.
...
Smoothing Pool에 참여하지 **않은** 경우 블록 제안의 Execution-layer 부분 보상 (거래 수수료 및 MEV 포함)이 노드의 [Fee Distributor](./fee-distrib-sp#fee-distributor와-smoothing-pool) 계약으로 전송됩니다.
...
Fee Distributor의 잔액에 액세스하려면 **분배**합니다 (따라서 *Fee Distributor*라는 이름입니다).
