...
기존 노드 운영자는 레거시 미니풀에 본드된 ETH를 기준으로 급행 대기열 티켓을 받습니다: 본드된 4 ETH마다 티켓 1개.
...
급행 대기열에서 [검증자를 대기열에서 제거](./create-megapool-validator#rocket-pool-입금-대기열에서-검증자-제거)하기로 선택하면 급행 대기열 티켓이 환급됩니다.
...
사전 출시 검증자가 365일 이내에 스테이킹에 실패하면 검증자가 해산됩니다. 사전 출시 과정에서 Beacon Chain으로 전송된 1 ETH(4 ETH 본드 중)는 **복구할 수 없습니다**.
노드 운영자는 본드의 나머지 3 ETH를 (0.05 ETH의 해산 페널티가 부채로 차감된 후) 메가풀에서 환불로 받으며, 순 환불액은 2.95 ETH입니다.
이는 `rocketpool megapool claim` 또는 `rocketpool claims claim-all`을 사용하여 청구할 수 있습니다.

[성공적인 스테이킹 확인 방법 자세히 알아보기](./create-megapool-validator#성공적인-스테이킹-확인)
...
다음 프롬프트에서는 [Rocket Pool 입금 대기열](/ko/node-staking/megapools/create-megapool-validator#rocket-pool-입금-대기열-및-급행-대기열) 상태를 표시합니다.
...
`Prelaunch` 검증자를 `Staking` 검증자로 진행시킵니다. 이 시점에서 `Staking` 검증자는 다음 상태가 됩니다:

- Beacon Chain에 32 ETH 입금 완료
- 검증자 인덱스 번호 할당
- Beacon Chain에서 활성화 대기 중
