...
`Prelaunch` 검증자가 Beacon Chain에 의해 처리되고 초기 1 ETH 입금이 인정된 후, 노드는 자동으로 전체 32 ETH Beacon Chain 입금을 완료하기 위한 `stake` 트랜잭션을 수행합니다. `stake` 트랜잭션은 `Prelaunch` 검증자를 `Staking` 검증자로 진행시킵니다. 이 시점에서 `Staking` 검증자는 다음 상태가 됩니다:
