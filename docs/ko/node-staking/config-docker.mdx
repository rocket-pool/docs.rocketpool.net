...
::: warning 참고
Checkpoint Sync는 _Beacon Chain 데이터가 아직 없는 경우_에만 발생합니다.
...
Smartnode v1.7.0부터 MEV는 이제 _opt-out_이므로 다음 화면에서 볼 수 있듯이 초기 설정의 일부로 구성이 제공됩니다:
...
확인란이 _선택 해제_되면 다음과 같이 단순히 비어 있습니다:
