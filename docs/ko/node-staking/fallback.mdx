...
Smart Node 스택의 **1.5.0** 버전부터 기본 클라이언트가 오프라인 상태가 될 경우([Geth를 사용하고 프루닝이 필요한 경우](./pruning)처럼) 이를 대체할 수 있는 "fallback" Execution 클라이언트 및 Consensus 클라이언트 쌍을 제공할 수 있습니다.
...
노드의 기본 클라이언트가 다시 온라인 상태가 되면 Smart Node와 Validator 클라이언트가 자동으로 다시 전환됩니다.
...
이 예제는 Docker 모드를 사용하여 다른 머신에 두 번째 Smart Node를 생성하는 방법을 보여주며, 이를 백업 노드로 사용할 수 있습니다.
...
2. 머신이 준비되면 [Smart Node 스택을 설치](./docker)합니다.
...
5. 선택적으로 **Prefer Fallback Clients**(기본값 꺼짐)를 체크합니다. 활성화하면 Smart Node가 API, node, watchtower 요청을 백업 Execution 및 Consensus 클라이언트로 먼저 보내고, 백업을 사용할 수 없을 때만 기본 클라이언트를 사용합니다. 이렇게 하면 기본 클라이언트에서 Smart Node 부하를 덜어내어 validator 업무에 집중할 수 있습니다. Validator 클라이언트는 영향을 받지 않으며 계속해서 기본 클라이언트를 먼저 사용합니다.
...
이것은 **Smart Node 자체**(데몬 서비스)에만 백업 지원을 제공한다는 점에 유의하세요. **백업 클라이언트에 대한 액세스를 제공하려면 Validator 클라이언트 서비스의 인수를 수동으로 업데이트해야 합니다.**
:::
