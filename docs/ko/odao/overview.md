...
- 표준 Smart Node CLI.
- Smart Node 데몬이 `watchtower` 모드에서 구성되고 실행 중입니다(모든 사용자를 위한 표준 Smart Node 번들에 포함되어 있지만 Oracle DAO 노드에 대해서만 적극적으로 업무를 수행합니다).
...
2. **수동 업무**: 귀하 자신의 의사 결정과 Oracle DAO의 나머지 멤버와의 대역 외 통신이 필요한 업무입니다. 컨트랙트 업그레이드에 대한 투표, 매개변수 변경, Oracle DAO에 멤버 초대 또는 제거와 같은 것들이 포함됩니다. 이들은 모두 표준 Smart Node CLI를 통해 수행할 수 있습니다.
